...
import {
  ...
  getJobCards, getOperations, getWorkstations, getPendingPreDispatchApprovals,
} from '@/lib/data';
...
export default async function ProductionWorkersPage({ searchParams }) {
  ...
  const [sheet, workers, projects, trades, jobCards, operations, workstations, preDispatchApprovals] = await Promise.all([
    getWorkerSheet(date),
    getWorkers(),
