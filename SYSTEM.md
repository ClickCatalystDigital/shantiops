...
## 5ct. Sales Payment Tracker + legacy Excel import (2026-09-19)

Sales sidebar → **Payment Tracker** (nested Orders / Payments, `components/SalesPaymentTracker.jsx`, wired in `SalesWorkspace.jsx`). Orders = one row per `sale_orders` row: click-to-edit cells (`EditCell`), a colored `track_status` dropdown, six stage checkboxes, Current Stage = first *unticked* step (else "Completed"), six paired KPI cards, pagination (10/25/50). Payments = log (`sale_order_payments`, `POST /api/sale-order-payments`, right-side Sheet; cells editable and rows deletable via `PATCH`/`DELETE /api/sale-order-payments/[id]`); it never posts to the ledger — Accounts' `customer_receipts` (§5w) stays the GL settlement.
- **Schema (additive on `sale_orders`):** `remarks`, `stage_*` ×6, `track_status` (Pending|Ready|WIP|Dispatched|Closed — deliberately separate from legacy `status` open|fulfilled|cancelled, which the Sales flow tile and payment route read), `order_date`, `invoice_ref`, `sales_person_override` (each falls back to the derived value when NULL). `sale_order_payments.received_on` is nullable (legacy rows with no date) and carries a free-text `invoice_ref`. Unique index on `sale_orders.so_no` (guarded in `migrate()`).
- **Order Value** is editable only for orders with no `sale_order_items` (a value derived from items would be overwritten on the next items save) — enforced server-side in `PATCH /api/sale-orders/[id]`.
- **Contracts untouched:** SAS (`purchase-requisitions`, Sales-only) and SoS (`POST /api/projects` with `sale_order_id`) still read the same `sale_orders`/`sale_order_items` columns. Imported orders are header-only (no items), so they never appear in Design's convert-to-project picker.
- **Import** — `scripts/import-sales-tracker.mjs <xlsx> [--apply]` (dry run by default). Loaded 1,006 orders + 1,905 payments from "SB Order - Payments Tracker 21-26" on 2026-09-19, all tagged `created_by='import:sales-tracker-2026-09-19'` (rollback = delete by tag). Payments sheet is the money source of truth; the ORDER sheet's Received/Pending are formulas. Verified field-by-field against the sheet: 0 mismatches. Removed the 3 test orders (SO-22/23/24); project 61 (SB-1109-01-50) now has `sale_order_id NULL`, `customer_id` 166 kept; all 59 HKM rows link to customer 166.
- **Known data issues, imported as-is (source typos):** 3 orders dated Oct/Nov 2026, 4 payments dated year 2055, 6 orders with no date, duplicate Order IDs (2nd gets " (2)": DN20, DN 16, SAS-322), payments skipped because their order isn't in the ORDER sheet (SB-1008 ₹16,10,950.77; SB-1054 A/B ₹1,00,300) or have no Order ID (16 rows, ₹3,63,254.60) or no amount (29). 34 orders' sheet "Payment Received" differs from the sum of PAYMENT rows (mostly SB-1109-x units and the duplicate IDs). 846 orders' customers aren't in the 334-row `customers` table (kept as text, not linked).
- **Add order / audit / paging:** Orders tab has an "Add order" sheet (`POST /api/sale-orders` now accepts `so_no`, `customer_id`, `total`, `order_date`, `track_status`, `sales_person`, `remarks`; duplicate Order ID → 409). Every order edit and every payment edit/delete writes `usb_audit` (`sale_order_edit`, `sale_payment_edit`, `sale_payment_deleted`, old → new). Sales Person is a dropdown; the legacy Sale Orders tab is paginated + searchable.
- **Open items:** client data questions + Excel row references + the remaining app gaps (Accounts/Marketing access, hard-coded Sales Person list, unchecked light/phone layouts) live in `docs/sales-tracker-data-issues.md`.
