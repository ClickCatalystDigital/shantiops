 ...
 Reserve stock            Stores decides
  ...
  Sales: {
    ...
    features: [
      ...
      feature('payment-tracker', 'Payment Tracker', IndianRupeeIcon, ['Payment Tracker → Orders lists every order with its value, what has been billed and what has been received. Pick the Current Stage from the dropdown (Advance → Dispatched → Site Work Completed → Commissioning → Pending Site Issue → Cleared Issue → Completed); the stage shown is the next step still to do, as in the old Excel tracker.', 'Bill Value is the total of the order\'s issued or paid Sales Invoices. Until an order has one, type the billed amount into Bill Value yourself.', 'A row turns light green when Order Value, Bill Value and Payment Received all agree (within ₹1 for rounding), and light red when they don\'t — for example a part payment, TDS kept back by the customer, or a bill that differs from the order. A row has no colour until it has both an order value and a bill value.', 'Click a remark (or "Add remark") to write it in a larger box. Payment Tracker → Payments is the log of every payment received.']),
