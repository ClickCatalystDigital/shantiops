// components/SalesFlow.jsx
'use client';

// Operations' Sales pipeline glance — same node/spine shapes as DesignFlow.jsx (copied, not
// abstracted, same precedent as that file's own note about ProcurementFlow.jsx). Counts-only,
// straight spine: Sales has no per-project "current stage" view to render here like Design's
// activeStage mode, so that half of DesignFlow's API is dropped.
...
const STAGES = [
  { key: 'leads', label: 'Leads', tone: 'plain', help: 'Open leads not yet converted or lost (new, contacted, qualified).' },
  { key: 'quotations', label: 'Quotations', tone: 'enquiry', help: 'Draft or sent quotations, waiting on a customer decision.' },
  { key: 'sale_orders', label: 'Sale Orders', tone: 'ordered', help: 'Confirmed orders still open (not yet fulfilled or cancelled).' },
  { key: 'projects', label: 'Projects', tone: 'received', help: 'Sale Orders that have been converted to a Project.' },
];
...
function SalesFlowVertical({ counts }) {
  return (
...
export default function SalesFlow({ counts }) {
      ...
      <CardHeader>
        ...
        <CardAction>
          <Button asChild size="sm" variant="outline">
            <Link href="/sales">Open Sales workspace →</Link>
          </Button>
        </CardAction>
      </CardHeader>
      <CardContent>
        ...
        <div className="sm:hidden">
          <SalesFlowVertical counts={counts} />
        </div>
      </CardContent>
    </Card>
  );
