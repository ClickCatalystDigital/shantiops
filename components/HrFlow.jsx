// components/HrFlow.jsx
'use client';

// Operations' HR pipeline glance (SYSTEM.md §3d) — same node/spine shapes as SalesFlow.jsx/
// DesignFlow.jsx (copied, not abstracted, same precedent those files already state). Headcount
// lifecycle only: employee_onboarding + employee_separation give a real Onboarding -> Active ->
// Separation -> Exited spine. Recruitment (job_applicants funnel) and payroll runs
// (draft/processed/submitted) are real pipelines too, but conceptually separate from headcount
// state — left as candidates for their own smaller widgets later, not bolted onto this spine.
...
const STAGES = [
  { key: 'onboarding', label: 'Onboarding', tone: 'warning', help: 'Newly joined, onboarding checklist still in progress.' },
  { key: 'active', label: 'Active', tone: 'plain', help: 'Active employees with no onboarding or separation in progress.' },
  { key: 'separation', label: 'Separation', tone: 'warning', help: 'Separation checklist in progress — still on the active roster until it completes.' },
  { key: 'exited', label: 'Exited', tone: 'received', help: 'No longer active — separation completed.' },
];
...
function HrFlowVertical({ counts }) {
  return (
...
export default function HrFlow({ counts }) {
    ...
    <Card>
      ...
      <CardContent>
        <div className="hidden overflow-x-auto sm:block">
          <div className="mx-auto flex min-w-[30rem] flex-col items-center">
