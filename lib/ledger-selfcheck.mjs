...
// Customer Ledger: an invoice before the window funds the opening balance (not dropped), a receipt
// and a credit note inside the window both reduce the running balance.
{
  const rows = [
    { ref: 'INV-1', date: '2026-03-01', debit: 50000, credit: 0, kind: 'Invoice' },
    { ref: 'INV-2', date: '2026-04-05', debit: 20000, credit: 0, kind: 'Invoice' },
    { ref: 'RCT-1', date: '2026-04-10', debit: 0, credit: 15000, kind: 'Receipt' },
    { ref: 'CN-1', date: '2026-04-12', debit: 0, credit: 2000, kind: 'Credit Note' },
  ];
  const cl = runningLedger(rows, { from: '2026-04-01', to: '2026-04-30' });
  assert.equal(cl.openingBalance, 50000);
  assert.equal(cl.entries.length, 3);
  assert.equal(cl.entries[0].balance, 70000); // opening 50000 + INV-2 20000
  assert.equal(cl.closingBalance, 53000); // 70000 - 15000 - 2000
}

// Aging: one current, one 1-30 overdue, one 90+ overdue, one fully settled (dropped).
{
  const asOf = '2026-08-22';
  const rows = [
    { ref: 'INV-1', date: '2026-08-01', dueDate: '2026-08-15', amount: 10000, settled: 0 }, // 7d overdue -> 1-30
    { ref: 'INV-2', date: '2026-08-20', dueDate: '2026-08-25', amount: 5000, settled: 0 }, // not due -> Current
    { ref: 'INV-3', date: '2026-01-01', dueDate: '2026-01-15', amount: 20000, settled: 0 }, // long overdue -> 90+
    { ref: 'INV-4', date: '2026-07-01', dueDate: '2026-07-10', amount: 8000, settled: 8000 }, // fully paid -> dropped
  ];
  const aging = agingBuckets(rows, asOf);
  assert.equal(aging.items.length, 3);
  assert.equal(aging.totals['1-30'], 10000);
  assert.equal(aging.totals['Current'], 5000);
  assert.equal(aging.totals['90+'], 20000);
  assert.equal(aging.total, 35000);
}
