...
import { groupByParent, nodeDepth, nodePath, expandedIdsForSearch, suggestNodeType, rankItemsByRelevance, effectiveNodeLevel, itemMatchAncestorIds, matchingItemIds, matchingNodeNameIds, templateVersionState } from './bom-tree.mjs';
...
function selfcheck() {
  ...
  const nameMatches = matchingNodeNameIds('smoke box', [
    { id: 1, name: 'Boiler Smoke Box' }, { id: 2, name: 'Feedwater Subsystem' }, { id: 3, name: 'Smoke Box Access Door' },
  ]);
  assert.deepStrictEqual([...nameMatches].sort(), [1, 3], 'matches every node whose own name contains the query, not just the first');
  assert.strictEqual(matchingNodeNameIds('', [{ id: 1, name: 'Boiler Smoke Box' }]).size, 0, 'empty query matches no node names');

  assert.strictEqual(templateVersionState({ stamped: 1, current: 3, found: true }), 'stale', 'older stamp than current -> stale');
  assert.strictEqual(templateVersionState({ stamped: 3, current: 3, found: true }), 'current', 'same version -> current');
  assert.strictEqual(templateVersionState({ stamped: null, current: 3, found: true }), 'unknown', 'applied before versioning -> never claimed stale');
  assert.strictEqual(templateVersionState({ stamped: 1, current: 3, found: false }), 'removed', 'template gone wins over any version compare');
