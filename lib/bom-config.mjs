...
export function configParts(labelRaw, valueRaw) {
  let label = norm(labelRaw).replace(/reliev\w*/i, 'RELIEVING'); // the sheets spell it "RELIEVEING"
...
export function classifyConfigRow(rec, { anyLabel = false } = {}) {
  ...
  // (never "SET PRESSURE GAUGE"), and a MOC still means a real item.
  const qtyIsNotPurchasing = isConfigLabel(rec.material_description) && /^set ?pressure/i.test(norm(rec.material_description));
  if ((norm(rec.qty_text) && !qtyIsNotPurchasing) || norm(rec.moc)) return null;
