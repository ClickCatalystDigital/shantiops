...
// Every stage template that's emerged from usage so far — small table, fetched whole and filtered
// client-side by milestone_key, same pattern as the rest of this file.
export async function getStageTemplates() {
  return queryAll(`SELECT * FROM stage_templates ORDER BY department, milestone_key, sort_order`);
}
