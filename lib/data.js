...
// Operations' Production pipeline glance (components/ProductionFlow.jsx) — same slot/precedent as
// Procurement's. Job Cards are Production's real moving unit (the Work Order itself only has 4
// coarse states and sits upstream as a source, same role Stores' SAS/BOM/stock sources play), so
// the main spine is job_cards.status — Pending/In progress/Done — not work_orders.status.
// project_id is nullable now (§5l, against_stock Work Orders) — a stock-mode card has no project to
// join against, so it's included unconditionally rather than dropped by an inner join.
export async function getProductionFlowCounts() {
  const [activeWorkOrders, cards] = await Promise.all([
  ...
  for (const c of cards) {
    ...
    if (c.rework_of_job_card_id && c.status !== 'done') counts.rework++;
  }
  return counts;
}
