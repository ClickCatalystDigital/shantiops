...
export async function getExecutiveSummary(company = null) {
  ...
  // company: the top-bar selector (null = both). Blank project company = the default letterhead.
  const projects = (await getProjectsWithStatus())
...
export async function getPackingLists() {
  return queryAll(
    `SELECT pl.*, (SELECT COUNT(*) FROM packing_items WHERE packing_list_id = pl.id) AS item_count,
            si.invoice_no AS linked_invoice_no, p.project_no, p.company
...
export async function getDispatchApprovalQueue() {
  return queryAll(
    `SELECT pl.id, pl.packing_no, pl.customer_name, p.project_no, p.company,
...
export async function getProductionFlowCounts() {
  ...
  counts.lifecycle = {
    ...
    workOrderCreated: { value: woCreated.c, href: '/production/shop?tab=workorders&wostatus=draft' },
    workOrderReleased: { value: woReleased.c, href: '/production/shop?tab=workorders&wostatus=released' },
    jobCards: { value: jobCardsRaised.c, href: '/production/shop?tab=jobcards' },
    execution: { value: woExecuting.c, href: '/production/shop?tab=workorders&wostatus=in_progress' },
    qc: { value: qcOpen.c + reworkOpenOnWo, href: '/production/shop?tab=jobcards' },
    completed: { value: woCompleted.c, href: '/production/shop?tab=workorders&wostatus=completed' },
  };
  counts.route = { value: woRoute.c, href: '/production/shop?tab=workorders&wostatus=draft' };
  counts.material = { value: materialIssued.c + piecesCut.c, href: '/production/shop?tab=bom' };
...
export async function getActiveProjectsList({ includeChildren = false } = {}) {
  ...
  return queryAll(
    `SELECT id, project_no, customer_name, series, created_at, bom_release_revision, unit_count, master_project_id FROM projects
  ...
  const [rows, rollupById, unitCounts, pendingInward] = await Promise.all([
    queryAll(
      `SELECT b.id, b.material_description, b.qty_text, b.moc, b.size_spec, b.source,
              ...
              EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id) AS has_children,
              p.id AS project_id, p.project_no, p.customer_name, p.company,
...
export async function getOpportunityPipelineCounts() {
  const [stages, rows] = await Promise.all([
    ...
    queryAll(`SELECT sales_call_status AS stage, expected_value AS value_num FROM leads WHERE COALESCE(owner_dept, 'Sales') = 'Sales'
              UNION ALL
              SELECT stage, value_num FROM opportunities WHERE owner_dept = 'Marketing'`),
  ]);
  const counts = {};
...
export async function getSalesFlowCounts(company = null) {
  // company narrows quotations/orders (the top-bar selector); enquiries are shared across companies.
  const co = company ? ` WHERE COALESCE(NULLIF(company, ''), '${COMPANY_NAMES[0]}') = ?` : '';
  const args = company ? [company] : [];
  const [leads, quotations, saleOrders] = await Promise.all([
    queryAll('SELECT status FROM leads'),
    queryAll('SELECT status FROM quotations' + co, args),
    ...
    queryAll("SELECT so.status, EXISTS(SELECT 1 FROM projects p WHERE p.sale_order_id = so.id) AS project_id FROM sale_orders so" + co.replace('company', 'so.company'), args),
  ]);
  return {
...
export async function getPurchaseOrders(filter = {}) {
  ...
  const sql = `
    SELECT po.*, s.name AS supplier_name,
           ...
           -- Company for the global selector when the PO itself has none (all current POs): the
           -- first real project on its lines. Blank still means the default letterhead company.
           (SELECT p.company FROM po_items pi JOIN projects p ON p.id = pi.project_id
             WHERE pi.po_id = po.id AND p.is_system = 0 AND p.company IS NOT NULL
             ORDER BY pi.sort_order, pi.id LIMIT 1) AS line_project_company
      FROM purchase_orders po JOIN suppliers s ON s.id = po.supplier_id
      ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY po.created_at DESC`;
  const rows = await queryAll(sql, args);
  for (const r of rows) { if (!r.company) r.company = r.line_project_company; delete r.line_project_company; }
