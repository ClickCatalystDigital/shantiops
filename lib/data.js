...
export async function getBomItemsForProject(projectId) {
  return queryAll(
    `SELECT id, material_description, moc, size_spec, make, category, category_fields_json, inventory_item_id
...
export async function getDrawingRegisterLines(company, { from, to } = {}) {
  ...
  const rows = await queryAll(
    `SELECT cd.id, cd.name, cd.drawing_type, cd.status, cd.assigned_to, cd.due_date,
            p.project_no, p.customer_name
