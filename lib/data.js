...
async function getCustomerViewSplitOrder(master, children) {
  ...
  const MASTER_LEVEL_PHASES = new Set(['design', 'procurement']);
  ...
  const phases = CUSTOMER_PHASES.map((ph, i) => {
    ...
    // Physical-unit phase — real aggregate across every child.
...
export async function getActiveProjectsList({ includeChildren = false } = {}) {
  ...
  return queryAll(
    `SELECT id, project_no, customer_name, series, created_at, bom_release_revision, unit_count FROM projects
