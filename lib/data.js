...
export async function getBomStructure(projectId) {
  const [assemblies, items] = await Promise.all([
...
export async function getEngineeringChangeNotes(projectId = null) {
  const where = projectId ? 'WHERE ecn.project_id = ?' : '';
...
export async function getSalesInvoiceDetail(id) {
  const [invoice, items] = await Promise.all([
    queryOne(`SELECT si.*, c.name AS customer_name, c.state_code AS customer_state_code FROM sales_invoices si JOIN customers c ON c.id = si.customer_id WHERE si.id = ?`, [id]),
...
export async function getVendorBillDetail(id) {
  const [bill, items] = await Promise.all([
    queryOne(
      `SELECT vb.*, po.po_no, s.name AS supplier_name, s.state_code AS supplier_state_code FROM vendor_bills vb
