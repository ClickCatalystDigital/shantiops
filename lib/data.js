...
// The two calendar layers for a date range: ad-hoc tasks, and the Production milestones already
// scheduled on projects. from/to are the 42-day grid bounds (lib/date.js monthGridBounds).
export async function getProductionCalendar(from, to) {
  const [tasks, milestones] = await Promise.all([
    queryAll(
      `SELECT id, title, due_date AS date, status, assigned_to
         FROM tasks
        WHERE department = 'Production' AND due_date BETWEEN ? AND ?
        ORDER BY due_date, id`,
      [from, to]
    ),
    queryAll(
      `SELECT m.id, m.milestone_label AS title, m.planned_end AS date, m.status,
              m.project_id, p.project_no, p.customer_name
         FROM milestones m JOIN projects p ON p.id = m.project_id
        WHERE m.department = 'Production' AND m.planned_end BETWEEN ? AND ?
...
export async function getOpenProductionTasks(through) {
  return queryAll(
    `SELECT * FROM tasks
      WHERE department = 'Production' AND status = 'open' AND due_date <= ?
      ORDER BY due_date, id`,
    [through]
  );
}
