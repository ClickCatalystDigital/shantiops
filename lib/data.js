...
export async function getPackingDetail(id) {
  ...
  return { list: { ...list, freightPosted: !!freightPosted, masterBomRevision, bomRevisionDrift }, items };
}
...
export async function getProjectBom(projectId) {
  ...
  for (const b of bom) b.self_routed_to = selfRoutingMap.get(b.id) ?? null;
  ...
  const readyForPacking = pending.filter(b => {
    ...
    return baseReady && b.self_routed_to !== 'production';
  });
...
export async function getDispatchWork() {
  const [projects, bomItems, packingItemStatus, lists, routing] = await Promise.all([
...
export async function getPendingPackingItems() {
  ...
  return rows.map((b) => {
    ...
    return {
      ...b,
      readyForPacking: baseReady && b.self_routed_to !== 'production',
