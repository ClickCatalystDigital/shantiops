...
export async function getProjectBom(projectId) {
  ...
  // Stores/Inventory hardening Phase 5 — self_routed_to now actually gates readiness, not just
  // display. A NULL routing decision (no explicit choice made — every row before this phase, and
  // any row nobody has routed since) falls through to the exact prior behavior, byte-identical.
  // Only an EXPLICIT 'production' routing can make a line LESS ready than the base check already
  // says (never more) — Stores can route a line to Manufacturing even when Engineering never
  // flagged requires_manufacturing, and once they have, it must not become packable until
  // Production also signals done. E2E findings fix (2026-09-14) — the check below used to block
  // forever once routed to 'production', regardless of production_done, contradicting this very
  // comment (found live: a real item stayed permanently unpackable after Production finished it).
  // A 'production' routing now blocks only until production_done flips true, matching the comment.
  const readyForPacking = pending.filter(b => {
    ...
    return baseReady && !(b.self_routed_to === 'production' && !b.production_done) && !pendingInwardIds.has(b.id);
  });
...
export async function getDispatchWork() {
  const [projects, bomItems, packingItemStatus, lists, routing, pendingInward] = await Promise.all([
  ...
  return projects.map((p) => {
    ...
    const readyNotCarried = items.filter((b) => !carriedIds.has(b.id) &&
      ...
      // E2E findings fix (2026-09-14) — same one-line correction as getProjectBom()'s own
      // readyForPacking: a 'production' routing blocks only until production_done flips true.
      !(routingMap.get(`${b.id}:${b.project_id}`) === 'production' && !b.production_done) &&
      !pendingInwardIds.has(b.id)
    ).length;
    return {
      id: p.id, project_no: p.project_no, customer_name: p.customer_name,
...
export async function getPendingPackingItems() {
  ...
  return rows.map((b) => {
    ...
    return {
      ...
      // E2E findings fix (2026-09-14) — same one-line correction as getProjectBom()'s own
      // readyForPacking: a 'production' routing blocks only until production_done flips true.
      readyForPacking: baseReady && !(b.self_routed_to === 'production' && !b.production_done) && !pendingInwardIds.has(b.id),
