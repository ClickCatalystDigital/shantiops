...
import { CUSTOMER_PHASES, MASTER_LEVEL_MILESTONE_KEYS } from './milestones';
...
import { maybeSweepQuotationReminders } from './quotation-reminders';
import { currentStage, waitingOnQc, JOB_SHEET_GROUPS, JOB_SHEET_STAGES } from './job-sheet-stages.mjs';
import { rollupQty, itemRollupQty, qtyBreakdown, partIdentityKey, sumKnownWeight } from './bom-structure.mjs';
import { parseConfig } from './bom-config.mjs';
import { groupByParent } from './bom-tree.mjs';
import { jobWorkVariance, calibrationStatus } from './qc-inspections.mjs';
import { milestoneDepartmentEntry, procurementEntry, storesEntry, childStoresEntry, productionEntry, qcEntry, dispatchEntry } from './department-state.mjs';
...
// Project View redesign, Part 2 — "which department(s) currently have real, live operational work
// on this project," for the unified Project View's Row 2 dynamic card. Deliberately NOT a new
// handoff engine/abstraction (per direct instruction): every signal is a straight read of an
// already-authoritative table/query. The actual decision logic (given these rows, is this
// department active, and why) lives in lib/department-state.mjs as plain, dependency-free
// functions — this function's own job is only the I/O: run the queries, hand the rows to those
// functions, collect whichever entries come back non-null. See that file's own header for why
// Design/Installation read milestone status while every other department reads live tables.
export async function getDepartmentState(project) {
  const departments = [];
  const isChild = !!project.master_project_id;

  const milestoneDepts = isChild ? ['Installation'] : ['Design', 'Installation'];
  const ms = await queryAll(
    `SELECT department, status, milestone_label FROM milestones WHERE project_id = ? AND department IN (${milestoneDepts.map(() => '?').join(',')})`,
    [project.id, ...milestoneDepts]);
  for (const dept of milestoneDepts) {
    const entry = milestoneDepartmentEntry(ms, dept);
    if (entry) departments.push(entry);
  }

  if (isChild) {
    const derived = await getChildDerivedBom(project.id);
    if (derived) {
      const entry = childStoresEntry(derived.items);
      if (entry) departments.push(entry);
    }
  } else {
    const bomItems = await queryAll(
      `SELECT id, purchase_status, selected_quote_id, po_ref,
              (SELECT COUNT(*) FROM supplier_quotes sq WHERE sq.bom_item_id = b.id) AS quote_count
         FROM bom_items b WHERE b.project_id = ?`, [project.id]);
    const procEntry = procurementEntry(bomItems);
    if (procEntry) departments.push(procEntry);

    const receivedIds = bomItems.filter(it => ['Received', 'In-Stock'].includes(it.purchase_status)).map(it => it.id);
    let notPackedCount = 0;
    if (receivedIds.length) {
      const placeholders = receivedIds.map(() => '?').join(',');
      const notYetPacked = await queryAll(
        `SELECT b.id FROM bom_items b WHERE b.id IN (${placeholders})
           AND NOT EXISTS (
             SELECT 1 FROM packing_items pi JOIN packing_lists pl ON pl.id = pi.packing_list_id
              WHERE pi.bom_item_id = b.id AND pl.status != 'draft')`,
        receivedIds);
      notPackedCount = notYetPacked.length;
    }
    const pendingInward = await queryOne(
      "SELECT COUNT(*) AS n FROM inward_approvals WHERE project_id = ? AND status = 'pending'", [project.id]);
    const storesEntryResult = storesEntry({ totalBomCount: bomItems.length, notPackedCount, pendingInwardCount: Number(pendingInward.n || 0) });
    if (storesEntryResult) departments.push(storesEntryResult);
  }

  const [workOrders, jobCards] = await Promise.all([
    queryAll("SELECT status FROM work_orders WHERE project_id = ?", [project.id]),
    queryAll("SELECT status, requires_qc_hold, qc_released_at FROM job_cards WHERE project_id = ?", [project.id]),
  ]);
  const { entry: prodEntry, heldCount } = productionEntry(workOrders, jobCards);
  if (prodEntry) departments.push(prodEntry);

  // inward_approvals is master-scoped for split-order material (bom_item_receipts only exists on
  // the master) — for a child, that signal is already surfaced under Stores above, so it's not
  // re-read here to avoid double-counting.
  const [qcPending, openNcrs, pendingInwardQc] = await Promise.all([
    queryOne("SELECT COUNT(*) AS n FROM qc_records WHERE project_id = ? AND result = 'pending'", [project.id]),
    queryOne("SELECT COUNT(*) AS n FROM ncr_records WHERE project_id = ? AND status != 'closed'", [project.id]),
    isChild ? Promise.resolve({ n: 0 }) : queryOne("SELECT COUNT(*) AS n FROM inward_approvals WHERE project_id = ? AND status = 'pending'", [project.id]),
  ]);
  const qcEntryResult = qcEntry({
    qcPendingCount: Number(qcPending.n || 0), openNcrCount: Number(openNcrs.n || 0),
    pendingInwardCount: Number(pendingInwardQc.n || 0), heldJobCardCount: heldCount,
  });
  if (qcEntryResult) departments.push(qcEntryResult);

  const [openPacking, pendingApproval] = await Promise.all([
    queryAll("SELECT status FROM packing_lists WHERE project_id = ?", [project.id]),
    queryAll(
      `SELECT pda.status FROM pre_dispatch_approvals pda
        WHERE pda.project_id = ? AND pda.id = (SELECT MAX(id) FROM pre_dispatch_approvals WHERE packing_list_id = pda.packing_list_id)`,
      [project.id]),
  ]);
  const dispatchEntryResult = dispatchEntry(openPacking, pendingApproval);
  if (dispatchEntryResult) departments.push(dispatchEntryResult);
...
export async function getProjectsWithStatus() {
  ...
  return projects.map(p => {
    ...
    // Multi-unit split — a split child inherits the master's whole milestone template, including 9
    // Design/Procurement keys that are structurally never actionable at the child level (that work
    // happens once, on the master). Excluding them here from the rollup-only inputs — never from
    // `ms`/`milestones` itself, which stays the real, full list — is what lets a child's roll-up
    // actually reach 'done' once its own real per-unit work finishes; without this, worstStatus()
    // requires every milestone in the set to be done, so these 9 permanently-pending rows would keep
    // it stuck at 'pending' forever. Untouched for a master itself or any ordinary project (no
    // master_project_id), where these 9 keys ARE the project's own real work.
    const rollupMs = p.master_project_id
      ? ms.filter(m => !MASTER_LEVEL_MILESTONE_KEYS.includes(m.milestone_key))
      : ms;
    const done = rollupMs.filter(m => m.actual_end || m.status === 'done').length;
    const { departmentProgress } = activeDepartmentStatus(ms);
    return {
      ...p,
      roll: worstStatus(rollupMs),
      blocker: biggestBlocker(rollupMs),
      progress: rollupMs.length ? Math.round((done / rollupMs.length) * 100) : 0,
      overallDone: done,
      overallTotal: rollupMs.length,
      departmentProgress,
      milestones: ms,
    };
  });
...
// Multi-unit BOM split — the Projects list rollup. getProjectsWithStatus() itself stays a flat list
// (Executive, getStageBottlenecks, etc. all depend on that exact shape) — this is a separate,
// additive grouping step used only by the Projects list page. A master with real children (created
// via POST /api/projects/[id]/split) would otherwise show up as N+1 separate top-level rows, one per
// unit, cluttering the list with what's really one order. Groups child rows (master_project_id set)
// under their master, attaches a live childSummary ({total, done} — done = roll.code === 'done',
// the exact same status the row's own StatusBadge already uses, no new vocabulary), and drops
// children out of the top-level list entirely (the page renders them only when a master is
// expanded). A project with no children is completely unaffected — childSummary is null,
// children is [].
export function groupProjectsByMaster(projects) {
  const childrenByMaster = {};
  const topLevel = [];
  for (const p of projects) {
    if (p.master_project_id) (childrenByMaster[p.master_project_id] ||= []).push(p);
    else topLevel.push(p);
  }
  return topLevel.map(p => {
    const children = (childrenByMaster[p.id] || []).sort((a, b) => (a.unit_no ?? 0) - (b.unit_no ?? 0));
    const childSummary = children.length
      ? { total: children.length, done: children.filter(c => c.roll?.code === 'done').length }
      : null;
    return { ...p, children, childSummary };
  });
}
...
export async function getDependencyHealthSummary(company = null) {
  const milestones = await queryAll(
    `SELECT m.*, p.project_no FROM milestones m JOIN projects p ON p.id = m.project_id WHERE p.status = 'active'`
      + (company ? ` AND COALESCE(NULLIF(p.company, ''), ?) = ?` : ''),
    company ? [COMPANY_NAMES[0], company] : []
  );
  const bomItems = await queryAll(
...
export async function getProjectDetail(id) {
  const project = await queryOne('SELECT p.*, so.so_no AS sale_order_no FROM projects p LEFT JOIN sale_orders so ON so.id = p.sale_order_id WHERE p.id = ?', [id]);
  ...
  // Multi-unit split — same exclusion as getProjectsWithStatus(): a split child's own copies of the
  // 9 Design/Procurement milestone keys are structurally never actionable at the child level, so
  // they're dropped from the rollup-only inputs (health/progress/blocker) but never from the real,
  // full `milestones` array the page renders. Untouched for the master itself or any ordinary
  // project.
  const rollupMs = project.master_project_id
    ? milestones.filter(m => !MASTER_LEVEL_MILESTONE_KEYS.includes(m.milestone_key))
    : milestones;
  const done = rollupMs.filter(m => m.actual_end || m.status === 'done').length;
  ...
  // Multi-unit split, Phase 4 — cheap enough to always compute (one indexed COUNT); lets the page
  // decide whether to render the Stores allocation panel without a second round-trip. hasChildren
  // is false for every ordinary project.
  const childCount = await queryOne('SELECT COUNT(*) AS n FROM projects WHERE master_project_id = ?', [id]);

  return {
    project,
    milestones,
    health: worstStatus(rollupMs),
    blocker: biggestBlocker(rollupMs),
    progress: rollupMs.length ? Math.round((done / rollupMs.length) * 100) : 0,
    currentPhase: current?.milestone_label || null,
    nextPhase: next?.milestone_label || null,
    estDispatch: estDispatch || null,
    hasChildren: Number(childCount.n) > 0,
  };
}
...
export async function getExecutiveSummary(company = null) {
  // Multi-unit split — a split master's real children must never inflate portfolio-level KPIs/
  // forecast/risk counts as N+1 separate entries; cross-unit reporting happens at the master level
  // (MULTI-UNIT-SPLIT-DESIGN.md §1.4, confirmed architecture). getProjectsWithStatus() itself stays
  // the flat, unfiltered list other callers depend on — this is the one filter, applied here.
  // company: the top-bar selector (null = both). A blank project company is the default letterhead.
  const projects = (await getProjectsWithStatus())
    .filter(p => !p.master_project_id && (!company || (p.company || COMPANY_NAMES[0]) === company));
...
// Multi-unit split — the portal's aggregate view for a split order (e.g. SB-1109-01-50: one order,
// 50 physical boilers). Additive-only seam, per direct instruction to touch nothing about the
// ordinary single-project portal: getCustomerView below checks for real children FIRST and, if none
// exist, falls straight through to its own original, completely untouched logic — this function is
// never reached for the vast majority of projects.
//
// Design & Procurement happen once, before the split (real work lives only on the master's own
// milestones — every child also carries a full copy of the milestone template, but those two
// phases' rows on a child are never actually worked, so reading them would be meaningless).
// Manufacturing/Testing/Finishing/Packing/Installation/Commissioning happen per physical unit —
// their status here is a genuine aggregate ("N of 50 units complete"), computed live off every
// child's own milestone rows, same "computed live, never stored" precedent as
// lib/dependency.mjs's blocked_by. Invoicing (client-confirmed, 2026-09-04): one invoice for the
// whole order, so `invoices` reads the MASTER's own sales_invoices — no aggregation needed there.
async function getCustomerViewSplitOrder(master, children) {
  const childIds = children.map(c => c.id);
  const placeholders = childIds.map(() => '?').join(',');

  const [masterMilestones, childMilestones, allDrawings, invoices, packingRows, qcRows] = await Promise.all([
    queryAll('SELECT * FROM milestones WHERE project_id = ?', [master.id]),
    queryAll(`SELECT * FROM milestones WHERE project_id IN (${placeholders})`, childIds),
    getCalcDrawings(master.id),
    queryAll(
      `SELECT id, invoice_no, invoice_date, status, total FROM sales_invoices
        WHERE project_id = ? AND status != 'draft' ORDER BY invoice_date DESC`, [master.id]),
    // Every past-draft packing list across every child, newest dispatch first — same "past draft,
    // not just latest" rule the single-project path already applies (§5al #11), just unioned across
    // units. dispatched_at falls back to created_at for a list stamped before that column existed.
    queryAll(
      `SELECT pl.id, pl.packing_no, pl.dispatched_at, pl.created_at, p.project_no
         FROM packing_lists pl JOIN projects p ON p.id = pl.project_id
        WHERE pl.project_id IN (${placeholders}) AND pl.status != 'draft'
        ORDER BY COALESCE(pl.dispatched_at, pl.created_at) DESC`, childIds),
    queryAll(
      `SELECT qd.id, qd.doc_id, qd.customer_visible_at, p.project_no
         FROM qc_documents qd JOIN projects p ON p.id = qd.project_id
        WHERE qd.project_id IN (${placeholders}) AND qd.customer_visible = 1
        ORDER BY qd.customer_visible_at DESC`, childIds),
  ]);
  ...
  const masterByKey = {};
  masterMilestones.forEach(m => { (masterByKey[m.milestone_key] ||= []).push(m); });
  const byChild = new Map(childIds.map(id => [id, {}]));
  childMilestones.forEach(m => {
    const bucket = byChild.get(m.project_id);
    if (bucket) (bucket[m.milestone_key] ||= []).push(m);
  });

  // The exact same per-set completion rule the single-project path uses below — applied once to the
  // master's own set (design/procurement) and once per child (every other phase), never a different
  // definition of "done"/"in progress" for the split case.
  function phaseStatusFor(ms) {
    ...
    if (doneCount === ms.length) return 'done';
    if (started) return 'in_progress';
    return 'upcoming';
  }

  const MASTER_LEVEL_PHASES = new Set(['design']);

  // Material Procurement is a real per-unit signal, not the master's own 5 whole-BOM procurement
  // milestones (those can only ever mean "every one of N lines cleared this stage" — never "N of 50
  // units"). Driven by the allocation feature's own readiness computation (§5bi) — a child only
  // counts once every BOM line someone actually chose to allocate per-unit (the physical-lot items;
  // bulk commodities bought once for the whole order are never tracked this way, by design) is
  // ready for it. trackedLineIds.size === 0 guards the vacuous-truth case (nothing allocated to
  // ANY child yet must read as "upcoming", never as a trivially-satisfied "done").
  const routingBoard = await getChildRoutingBoard(master.id);
  const cellsByChild = new Map();
  routingBoard.cells.forEach(c => {
    if (!cellsByChild.has(c.child_project_id)) cellsByChild.set(c.child_project_id, []);
    cellsByChild.get(c.child_project_id).push(c);
  });
  const trackedLineIds = new Set(routingBoard.cells.map(c => c.bom_item_id));

  const phases = CUSTOMER_PHASES.map((ph, i) => {
    if (i === 0) return { key: ph.key, label: ph.label, status: 'done' }; // Order Received

    if (MASTER_LEVEL_PHASES.has(ph.key)) {
      const ms = ph.keys.flatMap(k => masterByKey[k] || []);
      let status = phaseStatusFor(ms);
      if (ph.key === 'design' && status !== 'done' && awaitingCustomer) status = 'awaiting_customer';
      return { key: ph.key, label: ph.label, status };
    }

    if (ph.key === 'procurement') {
      let doneUnits = 0, startedUnits = 0;
      for (const id of childIds) {
        const childCells = cellsByChild.get(id) || [];
        const allReady = trackedLineIds.size > 0 &&
          [...trackedLineIds].every(lineId => childCells.find(c => c.bom_item_id === lineId)?.ready);
        if (allReady) doneUnits++;
        else if (childCells.length > 0) startedUnits++;
      }
      const status = trackedLineIds.size === 0 ? 'upcoming'
        : doneUnits === childIds.length ? 'done'
        : (doneUnits > 0 || startedUnits > 0) ? 'in_progress' : 'upcoming';
      return { key: ph.key, label: ph.label, status, unitProgress: { done: doneUnits, total: childIds.length } };
    }

    // Physical-unit phase — real aggregate across every child, off its own milestone keys.
    let doneUnits = 0, startedUnits = 0;
    for (const id of childIds) {
      const s = phaseStatusFor(ph.keys.flatMap(k => byChild.get(id)[k] || []));
      if (s === 'done') doneUnits++;
      else if (s === 'in_progress') startedUnits++;
    }
    const status = doneUnits === childIds.length ? 'done' : (doneUnits > 0 || startedUnits > 0) ? 'in_progress' : 'upcoming';
    return { key: ph.key, label: ph.label, status, unitProgress: { done: doneUnits, total: childIds.length } };
  });

  const estDispatch = [...masterMilestones, ...childMilestones]
    .reduce((a, m) => (m.planned_end && m.planned_end > a ? m.planned_end : a), '');

  return {
    project: master, phases, estDispatch: estDispatch || null,
    packingLists: packingRows.map(p => ({ id: p.id, packingNo: p.packing_no, unitProjectNo: p.project_no, dispatchedAt: p.dispatched_at || p.created_at })),
    drawings, invoices,
    qcCertificates: qcRows.map(q => ({ id: q.id, doc_id: q.doc_id, unitProjectNo: q.project_no })),
    isSplitOrder: true, unitCount: childIds.length,
  };
}
...
export async function getCustomerView(projectId) {
  ...
  if (!project) return null;
  // Multi-unit split — the one seam. A project with real children delegates to the aggregate view
  // above; a project with none (the vast majority) falls straight through to the original,
  // completely unmodified logic below it.
  const children = await queryAll(
    'SELECT id, project_no, unit_no FROM projects WHERE master_project_id = ? ORDER BY unit_no', [projectId]);
  if (children.length > 0) return getCustomerViewSplitOrder(project, children);
...
export async function getPackingLists() {
  return queryAll(
    `SELECT pl.*, (SELECT COUNT(*) FROM packing_items WHERE packing_list_id = pl.id) AS item_count,
            si.invoice_no AS linked_invoice_no, p.project_no
...
// Project View redesign, Wave 1 — /qc's Test Records tab needs the project's hydro_test milestone
// id (for QcPanel's reworkMilestoneId, same as DepartmentPanel.jsx already resolves inline from a
// full milestones array it already has). /qc has no reason to fetch the whole project detail just
// for this one id, so a tiny dedicated lookup instead of pulling in getProjectDetail's health/
// blocker/dependency computation.
export async function getMilestoneIdByKey(projectId, key) {
  const row = await queryOne('SELECT id FROM milestones WHERE project_id = ? AND milestone_key = ?', [projectId, key]);
  return row?.id ?? null;
}
...
// Inward QC/Production Approval Workflow — pending inward reviews (Material Review workspace's
// Inward tab). Only 'pending' rows: a decided-then-resubmitted cycle's OLD row stays permanently
// decided (never re-queried here), and the NEW row it spawned is what's actually pending.
export async function getPendingInwardApprovals() {
  return queryAll(
    `SELECT ia.*, b.material_description, b.moc, b.qty_text, p.project_no, p.customer_name,
            r.qty_received, r.received_by, r.received_at
       FROM inward_approvals ia
       JOIN bom_items b ON b.id = ia.bom_item_id
       LEFT JOIN projects p ON p.id = ia.project_id
       JOIN bom_item_receipts r ON r.id = ia.bom_item_receipt_id
      WHERE ia.status = 'pending'
      ORDER BY ia.id DESC`
  );
}

// Dispatch's own queue (Dispatch's own Approvals tab) — every packing list still 'packed' (not yet
// dispatched), each with its latest pre_dispatch_approvals cycle if one exists. Covers "ready to
// submit" (no cycle), "awaiting review" (pending), "rejected — resubmit", and "approved, ready to
// dispatch" in one query. Drops a list once it moves to 'dispatched' — a live worklist, not history
// (the packing list's own detail page + the Dispatch Register report already cover shipped lists).
export async function getDispatchApprovalQueue() {
  return queryAll(
    `SELECT pl.id, pl.packing_no, pl.customer_name, p.project_no,
            a.id AS approval_id, a.status AS approval_status,
            a.qc_decision, a.production_decision, a.qc_reason, a.production_reason, a.submitted_at
       FROM packing_lists pl
       LEFT JOIN projects p ON p.id = pl.project_id
       LEFT JOIN pre_dispatch_approvals a ON a.id = (
         SELECT id FROM pre_dispatch_approvals WHERE packing_list_id = pl.id ORDER BY id DESC LIMIT 1
       )
...
// Full detail for one inward approval — the receipt event, the bom_item/project it's against, any
// pieces held under it (dimensional case), and the complete resubmission chain (oldest first),
// each prior cycle's own decision/reason/decider/timestamp preserved and never overwritten.
export async function getInwardApprovalDetail(id) {
  const approval = await queryOne(
    `SELECT ia.*, b.material_description, b.moc, b.qty_text, b.requires_heat_no, b.requires_mtc,
            p.project_no, p.customer_name, r.qty_received, r.received_by, r.received_at, r.lot_label
       FROM inward_approvals ia
       JOIN bom_items b ON b.id = ia.bom_item_id
       LEFT JOIN projects p ON p.id = ia.project_id
       JOIN bom_item_receipts r ON r.id = ia.bom_item_receipt_id
  ...
  const pieces = await queryAll(
    "SELECT id, code, status, weight_kg, kind FROM stock_pieces WHERE bom_item_receipt_id = ?", [approval.bom_item_receipt_id]);
  const history = [];
  let cursor = approval;
  while (cursor) {
    history.unshift(cursor);
    if (!cursor.resubmission_of_id) break;
    cursor = await queryOne('SELECT * FROM inward_approvals WHERE id = ?', [cursor.resubmission_of_id]);
  }
  return { ...approval, pieces, history };
}

// Inward QC/Production Approval Workflow — pending pre-dispatch reviews (Material Review
// workspace's Pre-Dispatch tab). Same "only 'pending' rows" reasoning as getPendingInwardApprovals.
export async function getPendingPreDispatchApprovals() {
  return queryAll(
    `SELECT a.*, pl.packing_no, pl.customer_name AS packing_customer_name, p.project_no
       FROM pre_dispatch_approvals a
       JOIN packing_lists pl ON pl.id = a.packing_list_id
...
// Full reviewer-screen detail for one pre-dispatch approval cycle — the packing list itself (items,
// freight, e-way bill), the project's QC context (dispatch_eligible is informational only here,
// never the server-side gate itself — see app/api/packing/[id]/route.js), and the resubmission
// history, oldest first.
export async function getPreDispatchApprovalDetail(id) {
  const approval = await queryOne('SELECT * FROM pre_dispatch_approvals WHERE id = ?', [id]);
  if (!approval) return null;
  const packing = await getPackingDetail(approval.packing_list_id);
  const [qcRecords, qcDocuments] = approval.project_id ? await Promise.all([
    queryAll('SELECT id, test_type, reference_no, result, dispatch_eligible FROM qc_records WHERE project_id = ? ORDER BY id DESC', [approval.project_id]),
    queryAll('SELECT id, doc_id, series, customer_visible FROM qc_documents WHERE project_id = ? ORDER BY id DESC', [approval.project_id]),
  ]) : [[], []];
  const history = [];
  let cursor = approval;
  while (cursor) {
    history.unshift(cursor);
    if (!cursor.resubmission_of_id) break;
    cursor = await queryOne('SELECT * FROM pre_dispatch_approvals WHERE id = ?', [cursor.resubmission_of_id]);
  }
  return { approval, packing, qcRecords, qcDocuments, history };
}
...
export async function getQcProjectSummary(projectId) {
  ...
  // Project View redesign, Decision M — the Certificates & Documents card's third row.
  const ncrs = await queryOne(
    `SELECT COUNT(*) AS total, SUM(CASE WHEN status != 'closed' THEN 1 ELSE 0 END) AS open
       FROM ncr_records WHERE project_id = ?`, [projectId]);
  return {
    certs_total: certs?.total || 0,
    certs_with_pdf: certs?.with_pdf || 0,
    docs_total: docs?.total || 0,
    docs_finalized: docs?.finalized || 0,
    ncrs_total: ncrs?.total || 0,
    ncrs_open: ncrs?.open || 0,
  };
}
...
export async function getQcDocumentDetail(id) {
  ...
  // Ordered by part_no, not sort_order/insertion order — Form IV A's parts are always meant to read
  // 1, 2, 3... regardless of when each one was added or synced, and this is the one query both the
  // editor and the PDF (app/api/qc-documents/[id]/pdf/route.js) read from, so fixing it here fixes
  // both, and stays correct for every future add with no renumbering step. Real part numbers are
  // almost always plain integers-as-text; SQLite's CAST(text AS INTEGER) reads the leading digits and
  // returns 0 for anything non-numeric or blank, so a blank/non-numeric part_no is pushed to the end
  // via the CASE guard first, then numeric order, then the raw string as a tiebreak (so "1A" sorts
  // right after "1"), then id as the final stable tiebreak.
  const parts = await queryAll(
    `SELECT p.*, tc.certificate_no, tc.cast_no AS tc_cast_no, tc.heat_no AS tc_heat_no, tc.plate_no AS tc_plate_no,
            ...
            bi.assembly_id, bi.project_id AS bom_project_id,
      ...
      WHERE p.document_id = ?
      ORDER BY (CASE WHEN p.part_no IS NULL OR TRIM(p.part_no) = '' THEN 1 ELSE 0 END),
               CAST(p.part_no AS INTEGER), p.part_no, p.id`, [id]);
  const mountings = await queryAll(
    `SELECT m.*, tc.certificate_no, tc.cast_no AS tc_cast_no, tc.heat_no AS tc_heat_no, tc.plate_no AS tc_plate_no,
            tc.material_spec, tc.steel_maker
       FROM qc_mountings m
       LEFT JOIN test_certificates tc ON tc.id = m.test_certificate_id
      WHERE m.document_id = ? ORDER BY m.sort_order, m.id`, [id]);
  ...
  // Form IV A's own manual lettered-section groups (distinct from qc_iiia_groups above) — see
  // lib/qc-form4a-sections.mjs.
  const form4aGroups = await queryAll(
    'SELECT * FROM qc_form4a_groups WHERE document_id = ? ORDER BY sort_order, id', [id]);
  // QC statutory-forms plan, Phase 5 — Form III §4's dynamic belt/furnace-ring rows.
  const seams = await queryAll(
    'SELECT * FROM qc_document_longitudinal_seams WHERE document_id = ? ORDER BY location, sequence, id', [id]);
  ...
  // Master/child resolution (§ multi-unit split) — a split child has no calc_drawings rows of its
  // own (drawings, like the BOM, live only on the master); resolving through master_project_id is
  // the same pattern every other qc-documents route already applies (sync-bom, sync-mountings,
  // link-parts). Previously missing here, which is why a split child's own document read zero
  // approved drawings even when the master had real ones.
  const drawingsProject = await queryOne('SELECT master_project_id FROM projects WHERE id = ?', [document.project_id]);
  const drawingsProjectId = drawingsProject?.master_project_id || document.project_id;
  const approvedDrawings = await queryAll(
    `SELECT cd.id, cd.dg_no, cd.name,
            (SELECT f.id FROM calc_drawing_files f WHERE f.drawing_id = cd.id ORDER BY f.id DESC LIMIT 1) AS file_id
       FROM calc_drawings cd
      WHERE cd.project_id = ? AND cd.status = 'approved' AND cd.dg_no IS NOT NULL
      ORDER BY cd.id`,
    [drawingsProjectId]);
  document.approved_drawing_codes = approvedDrawings.map(d => d.dg_no);
  // Richer shape for the Boiler Details sheet's drawing dropdown + inline preview — kept alongside
  // approved_drawing_codes (a bare dg_no array, still what lib/qc-folder-pdf.js's covering
  // letter/§9 lines consume) rather than replacing it, so the PDF generator needed zero changes.
  document.approved_drawings = approvedDrawings;
  // Multi-unit split Phase F — informational only, same idiom as getPackingDetail's own drift flag.
  if (document.bom_release_revision_at_creation != null) {
    const master = await queryOne(
      `SELECT m.bom_release_revision FROM projects c JOIN projects m ON m.id = c.master_project_id WHERE c.id = ?`,
      [document.project_id]);
    document.masterBomRevision = master?.bom_release_revision ?? null;
    document.bomRevisionDrift = document.masterBomRevision != null && document.masterBomRevision > document.bom_release_revision_at_creation;
  }
  return { document, parts, mountings, groups, form4aGroups, seams };
}
...
export async function getPackingDetail(id) {
  ...
  // Multi-unit quantity multiplier — only annotate while still a draft (genuinely editable, meant
  // to be reviewed before approving). Once a list is Ready/Dispatched, packing_items.qty is a
  // frozen, already-committed number; a node's Local Quantity can still change afterward, and a
  // live-recomputed label at that point would risk disagreeing with the real stored qty right next
  // to it — worse than showing nothing. The frozen qty speaks for itself past draft.
  if (list.status === 'draft') {
    const bomRows = await queryAll(
      `SELECT id, qty_text, assembly_id, qty_resolved FROM bom_items WHERE id IN (
    ...
    const [rollupById, unitCounts] = await Promise.all([
      getAssemblyRollupMap(list.project_id), getProjectUnitCounts(list.project_id),
    ]);
    const unitCount = unitCounts.get(list.project_id) ?? 1;
    items = items.map(it => {
      const b = it.bom_item_id ? bomById.get(it.bom_item_id) : null;
      return { ...it, qty_breakdown: b ? qtyBreakdown(b.qty_text, b.assembly_id, rollupById, unitCount, !!b.qty_resolved) : null };
    });
  }
  ...
  // Multi-unit split Phase F — informational only: was this list created from a since-revised
  // master BOM? bom_release_revision_at_creation is null for every ordinary (non-child) project's
  // own packing list, so masterBomRevision/bomRevisionDrift are simply absent there.
  let masterBomRevision = null;
  if (list.bom_release_revision_at_creation != null) {
    const master = await queryOne(
      `SELECT m.bom_release_revision FROM projects c JOIN projects m ON m.id = c.master_project_id WHERE c.id = ?`,
      [list.project_id]);
    masterBomRevision = master?.bom_release_revision ?? null;
  }
  const bomRevisionDrift = masterBomRevision != null && masterBomRevision > list.bom_release_revision_at_creation;
  // Inward QC/Production Approval Workflow — the latest review cycle for this list, if any, so the
  // status badge/Submit-for-approval button on the packing detail page needs no separate fetch.
  const preDispatchApproval = await queryOne(
    'SELECT * FROM pre_dispatch_approvals WHERE packing_list_id = ? ORDER BY id DESC LIMIT 1', [id]);
  return { list: { ...list, freightPosted: !!freightPosted, masterBomRevision, bomRevisionDrift, preDispatchApproval }, items };
}
...
export async function getProjectBom(projectId) {
  ...
  const bom = await queryAll(
    `SELECT b.*, s.name AS selected_supplier_name, sq.unit_price AS selected_unit_price,
            ...
            -- Unified delivery/lot-centric receiving, Phase 2 — lets ReceiveBomItemDialog.jsx know
            -- whether a self-routing decision applies at receive time (no children) or the item
            -- routes per-child later instead (has children), same signal getPurchaseOrderDetail()
            -- already surfaces for Procurement's own lot-reference UI.
            EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id) AS has_children,
            ...
            -- round 3 Phase B — the real Purchase Request a line came from, when it did (same join
            -- getSourcingItems() already uses). b.pr_item_id/b.pr_ref are already exposed via b.*;
            -- pr_no/pr_created_at is the human-readable number the legacy free-text pr_ref never had.
      ...
      WHERE b.project_id = ? ORDER BY b.sort_order, b.id`, [projectId]);
  // Gap #3 (2026-09-04) — the single most-used BOM view (BomTable.jsx, every department's
  // project-page panel, Release BOM, Structure Templates' item tab, and WorkersPanel.jsx's own
  // Cutting & Remnant list, which reads this function directly) never carried the whole-BOM/per-node
  // rollup breakdown §5be wired everywhere else. Same attach pattern getBomStructure()/
  // getSourcingItems()/getOpenBomItems() already use.
  const [rollupById, unitCountRow] = await Promise.all([
    getAssemblyRollupMap(projectId), queryOne('SELECT unit_count FROM projects WHERE id = ?', [projectId]),
  ]);
  const unitCount = unitCountRow?.unit_count ?? 1;
  for (const b of bom) {
    b.qty_breakdown = qtyBreakdown(b.qty_text, b.assembly_id, rollupById, unitCount, !!b.qty_resolved);
    b.rolled_qty = itemRollupQty(b.qty_text, b.assembly_id, rollupById, unitCount, !!b.qty_resolved);
  }
  // Unified delivery/lot-centric receiving, Phase 2 — the self-routing decision (route-self/route.js),
  // the equivalent of getChildDerivedBom()'s own routed_to for a normal/sibling project (one that has
  // no child units of its own, so child_project_id === this project's own id). Now gates
  // readyForPacking too (Stores/Inventory hardening Phase 5) — see that filter's own comment for the
  // fallback discipline that keeps every pre-existing row's behavior unchanged.
  const selfRouting = await queryAll(
    'SELECT bom_item_id, routed_to FROM bom_item_child_routing WHERE child_project_id = ?', [projectId]);
  const selfRoutingMap = new Map(selfRouting.map(r => [r.bom_item_id, r.routed_to]));
  // Inward QC/Production Approval Workflow gap-fix (final Phase 0-7 audit, 2026-09-13) — a real,
  // confirmed hole: purchase_status flips to 'Received' at the moment of physical receipt, inside
  // the SAME transaction that holds the actual stock_pieces/scalar quantity pending QC review
  // (lib/bom-receiving.js). readyForPacking previously read purchase_status alone, so a bought-out
  // line whose material is still sitting in QC-inward limbo would already show as packable — the
  // Inward Approval Workflow's own stated purpose ("determines whether received material becomes
  // usable Stores inventory at all") was silently bypassed on the dispatch side. Also applied to the
  // requires_manufacturing branch, not just the bought-out one — production_done is a manual
  // Production toggle (BomTable.jsx's "Prod. Done" column), not strictly tied to a specific piece
  // ever having been cut, so it carries the identical risk of being set on held material by mistake.
  const pendingInward = await queryAll(
    "SELECT DISTINCT bom_item_id FROM inward_approvals WHERE project_id = ? AND status = 'pending'", [projectId]);
  const pendingInwardIds = new Set(pendingInward.map(r => r.bom_item_id));
  for (const b of bom) {
    b.self_routed_to = selfRoutingMap.get(b.id) ?? null;
    // Final Phase 0-7 audit, item C — surfaces the same hold state readyForPacking now correctly
    // gates on, but on every row (not just the filtered subset), so BomTable.jsx can show a real
    // "pending QC review" badge instead of a line silently looking done with no explanation.
    b.pending_inward_review = pendingInwardIds.has(b.id);
  }
  ...
  // Stores/Inventory hardening Phase 5 — self_routed_to gates readiness, not just display.
  // Routing decoupled from receiving (gentle-snuggling-wozniak.md §7/§10) — a routing-eligible
  // line (source='bom', no split-order children of its own) now REQUIRES an explicit routing
  // decision to be ready: 'dispatch', or 'production' with production_done. A NULL self_routed_to
  // used to fall through as ready (treating "never routed" as equivalent to "routed to dispatch")
  // — that was only safe while routing was mandatory at receive time; now that it's deferred to
  // Stores' Allocate screen, an unrouted line must not be packable until Stores actually decides.
  // Any line outside this population (source != 'bom', or a split master's own has_children=true
  // row, which never goes through self-routing to begin with) keeps its exact prior behavior —
  // baseReady alone, self_routed_to never consulted, same as before this change.
  const readyForPacking = pending.filter(b => {
    ...
    if (!baseReady || pendingInwardIds.has(b.id)) return false;
    if (b.source !== 'bom' || b.has_children) return true;
    return b.self_routed_to === 'dispatch' || (b.self_routed_to === 'production' && b.production_done);
  });
  return { bom, pending, readyForPacking, imports };
}

// Multi-unit BOM split, Phase 3 (MULTI-UNIT-SPLIT-DESIGN.md §5.1, resolved) — a read-only, derived
// per-unit BOM view for a child project. Never stored, never cloned onto the child (confirmed
// architecture: children never get their own bom_items rows). Recomputes the master's own rollup
// chain with a unit multiplier of 1 instead of the master's real unit_count — the only difference
// between the master's aggregate total and one child's own requirement IS that factor, so this is
// exactly "what one physical unit needs," reusing itemRollupQty() unchanged rather than inventing a
// second calculation. Returns null for a project that isn't a child (the common, unaffected case).
export async function getChildDerivedBom(childProjectId) {
  const child = await queryOne('SELECT id, master_project_id FROM projects WHERE id = ?', [childProjectId]);
  if (!child?.master_project_id) return null;
  const master = await queryOne('SELECT id, project_no FROM projects WHERE id = ?', [child.master_project_id]);
  if (!master) return null;

  const [items, rollupById, allocated, routed, certs] = await Promise.all([
    queryAll(
      `SELECT b.id, b.material_description, b.moc, b.size_spec, b.make, b.qty_text, b.assembly_id,
              b.qty_resolved, b.category, b.purchase_status, it.item_code AS catalog_item_code
         FROM bom_items b LEFT JOIN items it ON it.id = b.item_id
        WHERE b.project_id = ? ORDER BY b.sort_order, b.id`, [master.id]),
    getAssemblyRollupMap(master.id),
    // This one child's own allocated-so-far per line — the Status column's readiness half.
    queryAll(
      `SELECT bom_item_id, COALESCE(SUM(qty_allocated), 0) AS total FROM bom_item_child_allocations
        WHERE child_project_id = ? GROUP BY bom_item_id`, [childProjectId]),
    queryAll('SELECT bom_item_id, routed_to FROM bom_item_child_routing WHERE child_project_id = ?', [childProjectId]),
    // Architecture review addition — this child's own linked certificate(s) per line, read-only here
    // (ChildUnitBomCard's own display); the write action lives in /qc, not on this project page.
    queryAll(
      `SELECT bcc.bom_item_id, bcc.certificate_id, tc.certificate_no FROM bom_item_child_certificates bcc
         JOIN test_certificates tc ON tc.id = bcc.certificate_id
        WHERE bcc.child_project_id = ?`, [childProjectId]),
  ]);
  const allocatedMap = new Map(allocated.map(r => [r.bom_item_id, Number(r.total)]));
  const routedMap = new Map(routed.map(r => [r.bom_item_id, r.routed_to]));
  const certsMap = new Map();
  certs.forEach(r => {
    if (!certsMap.has(r.bom_item_id)) certsMap.set(r.bom_item_id, []);
    certsMap.get(r.bom_item_id).push({ id: r.certificate_id, certificate_no: r.certificate_no });
  });

  const items_out = items.map(it => {
    const perUnitQty = itemRollupQty(it.qty_text, it.assembly_id, rollupById, 1, !!it.qty_resolved);
    const allocatedQty = allocatedMap.get(it.id) ?? 0;
    return {
      ...it,
      per_unit_qty: perUnitQty,
      allocated_qty: allocatedQty,
      ready: perUnitQty != null && allocatedQty >= perUnitQty,
      routed_to: routedMap.get(it.id) ?? null,
      certificates: certsMap.get(it.id) ?? [],
    };
  });

  return { masterProjectId: master.id, masterProjectNo: master.project_no, items: items_out };
}

// Multi-unit BOM split, Phase 4 (MULTI-UNIT-SPLIT-DESIGN.md §5.2) — the Stores pipeline view: for
// each BOM line on a (master) project, how much has actually been received (bom_item_receipts, the
// real ledger — correct for both a genuine partial delivery and a completed single-shot receipt,
// since Phase 4a logs a row either way) versus how much has been allocated to specific child units
// so far (bom_item_child_allocations). available = received - allocated is what a Stores head can
// still allocate; it is never compared against the line's required quantity here — allocation is a
// bookkeeping step over stock that has already arrived, never a procurement signal.
export async function getProjectAllocationSummary(projectId) {
  const [items, received, allocated, rollupById] = await Promise.all([
    queryAll(
      `SELECT id, material_description, moc, size_spec, qty_text, assembly_id, qty_resolved
         FROM bom_items WHERE project_id = ? ORDER BY sort_order, id`, [projectId]),
    queryAll(
      `SELECT bom_item_id, COALESCE(SUM(qty_received), 0) AS total FROM bom_item_receipts
        WHERE bom_item_id IN (SELECT id FROM bom_items WHERE project_id = ?) GROUP BY bom_item_id`, [projectId]),
    queryAll(
      `SELECT bom_item_id, COALESCE(SUM(qty_allocated), 0) AS total FROM bom_item_child_allocations
        WHERE bom_item_id IN (SELECT id FROM bom_items WHERE project_id = ?) GROUP BY bom_item_id`, [projectId]),
    getAssemblyRollupMap(projectId),
  ]);
  const receivedMap = new Map(received.map(r => [r.bom_item_id, r.total]));
  const allocatedMap = new Map(allocated.map(r => [r.bom_item_id, r.total]));
  return items.map(it => {
    const receivedTotal = Number(receivedMap.get(it.id) ?? 0);
    const allocatedTotal = Number(allocatedMap.get(it.id) ?? 0);
    // Per-unit quantity for the bundle-allocate button — same rollup math as getChildDerivedBom,
    // so a bundle's auto-computed qty can never disagree with what a child's own BOM view shows.
    const perUnitQty = itemRollupQty(it.qty_text, it.assembly_id, rollupById, 1, !!it.qty_resolved) ?? 1;
    return { ...it, received: receivedTotal, allocated: allocatedTotal, available: receivedTotal - allocatedTotal, per_unit_qty: perUnitQty };
  });
}

// Multi-unit split — Stores routing board: single source of truth for "is this (line, child) ready"
// and "where has Stores routed it," shared by ChildRoutingPanel (the write UI) and the Dispatch batch
// route (the one real consumer that gates on it) so the two can never disagree. Cells are sparse —
// emitted only for (line, child) pairs with allocated > 0; a never-allocated pair is neither ready
// nor routable and simply doesn't appear. Returns empty arrays for a project with no children (an
// ordinary project never reaches this function — every caller is hasChildren-gated).
export async function getChildRoutingBoard(masterProjectId) {
  const [master, children, lines, rollupById, allocRows, routingRows, certRows, pendingInwardRows] = await Promise.all([
    // UI-clarity addition — callers picking a unit to resolve "which order" (e.g.
    // MaterialCertificatePanel) need to say which order they landed on, not just show its
    // children; cheap enough to always fetch alongside everything else here.
    queryOne('SELECT id, project_no, customer_name FROM projects WHERE id = ?', [masterProjectId]),
    queryAll('SELECT id, project_no, unit_no FROM projects WHERE master_project_id = ? ORDER BY unit_no', [masterProjectId]),
    queryAll(
      `SELECT id, material_description, moc, size_spec, make, qty_text, assembly_id, qty_resolved
         FROM bom_items WHERE project_id = ? ORDER BY sort_order, id`, [masterProjectId]),
    getAssemblyRollupMap(masterProjectId),
    queryAll(
      `SELECT bom_item_id, child_project_id, COALESCE(SUM(qty_allocated), 0) AS total
         FROM bom_item_child_allocations
        WHERE child_project_id IN (SELECT id FROM projects WHERE master_project_id = ?)
        GROUP BY bom_item_id, child_project_id`, [masterProjectId]),
    queryAll(
      `SELECT bom_item_id, child_project_id, routed_to FROM bom_item_child_routing
        WHERE child_project_id IN (SELECT id FROM projects WHERE master_project_id = ?)`, [masterProjectId]),
    // Architecture review addition — which certificate(s) QC has linked per (line, child) cell.
    // Many-to-many, so a cell maps to an array, not a single value like routing.
    queryAll(
      `SELECT bcc.bom_item_id, bcc.child_project_id, bcc.certificate_id, tc.certificate_no
         FROM bom_item_child_certificates bcc JOIN test_certificates tc ON tc.id = bcc.certificate_id
        WHERE bcc.child_project_id IN (SELECT id FROM projects WHERE master_project_id = ?)`, [masterProjectId]),
    // STORES_DEPARTMENT.md Gap #1 fix — same guard getProjectBom()/getPendingPackingItems()/
    // getDispatchWork()/getUnroutedReceivedItems()/getPendingProductionMaterialLines() already
    // apply: material QC hasn't cleared yet isn't real, usable stock, so a cell shouldn't read
    // 'ready' either. inward_approvals.bom_item_id is keyed on the MASTER's own bom_items row
    // (bom_items only ever lives on the master, never per-child), so this is scoped to the
    // master's project_id, not per-child.
    queryAll(
      `SELECT DISTINCT ia.bom_item_id FROM inward_approvals ia
         JOIN bom_items b ON b.id = ia.bom_item_id
        WHERE b.project_id = ? AND ia.status = 'pending'`, [masterProjectId]),
  ]);

  const perUnitById = new Map(lines.map(l => [l.id, itemRollupQty(l.qty_text, l.assembly_id, rollupById, 1, !!l.qty_resolved) ?? 1]));
  const cellKey = (bomItemId, childId) => `${bomItemId}:${childId}`;
  const routingMap = new Map(routingRows.map(r => [cellKey(r.bom_item_id, r.child_project_id), r.routed_to]));
  const pendingInwardIds = new Set(pendingInwardRows.map(r => r.bom_item_id));
  const certsMap = new Map();
  certRows.forEach(r => {
    const key = cellKey(r.bom_item_id, r.child_project_id);
    if (!certsMap.has(key)) certsMap.set(key, []);
    certsMap.get(key).push({ id: r.certificate_id, certificate_no: r.certificate_no });
  });

  const cells = allocRows.map(r => {
    const perUnitRequired = perUnitById.get(r.bom_item_id) ?? 1;
    const allocated = Number(r.total);
    return {
      bom_item_id: r.bom_item_id,
      child_project_id: r.child_project_id,
      allocated,
      per_unit_required: perUnitRequired,
      ready: allocated >= perUnitRequired && !pendingInwardIds.has(r.bom_item_id),
      routed_to: routingMap.get(cellKey(r.bom_item_id, r.child_project_id)) ?? null,
      certificates: certsMap.get(cellKey(r.bom_item_id, r.child_project_id)) ?? [],
    };
  });

  return { master, children, lines, cells };
}

// Stores' own cross-project queue: every split (multi-unit) master with at least one BOM line
// that's received-but-not-fully-allocated, or a cell that's allocated-ready-but-not-yet-routed.
// Deliberately reuses getProjectAllocationSummary()'s own `available > 0` rule and
// getChildRoutingBoard()'s own `ready && !routed_to` rule rather than a third predicate for "needs
// action" — the risk being two independently-maintained readiness checks silently drifting apart
// from the project page's own view over time. A live scan, not a stored flag, same as every other
// Stores queue (getReorderSuggestions, getOpenBomItems).
// ponytail: one getProjectAllocationSummary + getChildRoutingBoard pair per split master, in
// parallel — fine while split orders are rare (one exists today); if that ever grows past a
// handful, this becomes a real N+1 fan-out on every /stores page load and is worth a single
// combined query instead.
export async function getSplitOrdersNeedingStoresAction() {
  const masters = await queryAll(
    `SELECT DISTINCT p.id, p.project_no, p.customer_name FROM projects p
      JOIN projects c ON c.master_project_id = p.id
     ORDER BY p.project_no`);
  if (!masters.length) return [];

  const rows = await Promise.all(masters.map(async m => {
    const [allocItems, board] = await Promise.all([
      getProjectAllocationSummary(m.id),
      getChildRoutingBoard(m.id),
    ]);
    return {
      id: m.id,
      project_no: m.project_no,
      customer_name: m.customer_name,
      unit_count: board.children.length,
      unallocated_lines: allocItems.filter(it => it.available > 0).length,
      unrouted_ready_cells: board.cells.filter(c => c.ready && !c.routed_to).length,
    };
  }));

  return rows.filter(r => r.unallocated_lines > 0 || r.unrouted_ready_cells > 0);
}
...
// Projects Stores has started receiving materials for (>=1 Received/In-Stock BOM item, OR a real
// partial-receipt ledger row logged against one) — the point a project becomes QC's business
// (incoming inspection begins per-item on receipt, §5p). The purchase_status check alone misses a
// genuine partial multi-unit delivery (e.g. 20 of 50 units' worth arrived): purchase_status
// correctly stays 'Enquiry' until the FULL required quantity is in (§5be/§5bi), so a real,
// already-logged receiving event (bom_item_receipts) is the earlier, more accurate signal. Used to
...
export async function getReceivedProjectIds() {
  const rows = await queryAll(
    `SELECT DISTINCT project_id FROM bom_items
     WHERE purchase_status IN ('Received','In-Stock')
        OR id IN (SELECT DISTINCT bom_item_id FROM bom_item_receipts)`);
  return rows.map(r => r.project_id);
}

// ponytail: TEMPORARY widening of the QC project picker — also show any project whose Design
// 'release_bom' milestone is done, even before Stores has logged a receipt. Requested directly to
// let QC pre-stage statutory documents right after release. Purely additive (unions into the same
// `relevant` set in app/qc/page.js), so nothing existing narrows or breaks. Revert by deleting this
// function and its one call site if/when the real receipt-gated behavior should be the only rule.
export async function getReleasedBomProjectIds() {
  const rows = await queryAll(
    `SELECT DISTINCT project_id FROM milestones
     WHERE milestone_key = 'release_bom' AND status = 'done'`);
  return rows.map(r => r.project_id);
}

// Split children Stores has actually allocated real received material to
// (bom_item_child_allocations) — the point ONE SPECIFIC unit, not just its master, becomes QC's
// business. A master receiving material for units 1-20 must not make units 21-50 relevant too —
// only the units something was actually set aside for.
export async function getAllocatedChildProjectIds() {
  const rows = await queryAll('SELECT DISTINCT child_project_id FROM bom_item_child_allocations');
  return rows.map(r => r.child_project_id);
}
...
export async function getSourcingItems() {
  const rows = await queryAll(
    `SELECT b.*, (SELECT procurement_class FROM items WHERE id = b.item_id) AS procurement_class, p.project_no, p.customer_name, p.is_system AS project_is_system,
            s.name AS selected_supplier_name, sq.unit_price AS selected_unit_price,
            pr.pr_no, pr.created_at AS pr_created_at, pr.raised_by_dept AS pr_raised_by_dept,
            -- The PR line's own header record (§5c PR aggregation) — the canonical "what material
            -- are we buying" spec (MOC + the fixed shape fields, never a per-project Length/Width),
            -- written once at raise time. aggregatePrGroups() reads this as the real common spec
            -- for a group instead of re-deriving it by picking one project's own row and stripping
            -- fields back out — a per-project bom_items row can be edited independently afterward,
            -- so it's not a reliable stand-in for "what every project in this group is supposed to
            -- share." NULL for anything not raised through the unified PR flow.
            pi.moc AS pr_item_moc, pi.size_spec AS pr_item_size_spec, pi.category_fields_json AS pr_item_category_fields_json,
            ...
            -- Unified delivery/lot-centric receiving, Phase 0 — Procurement previously had zero
            -- visibility into "this line's project is a split order" anywhere (Sourcing or the PO
            -- editor); needed regardless of where the lot-reference UI itself lands.
            EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id) AS has_children
       FROM bom_items b
      ...
      -- from stock, so it never needs to appear here at all). A PR-raised line (pr_item_id set) is
      -- never gated behind this at all — app/api/purchase-requisitions/route.js stopped setting
      -- pending_review=1 on it (direct product decision: the unified PR flow's own original design
      -- was "no acceptance gate," and this Manual-mode self-review step never should have re-applied
      -- to it in the first place — it belongs to PMB-imported/manual/template lines only).
      ...
      -- at the sentinel system project, which has no milestones at all, so they're exempt. A
      -- PR-raised line is exempt too, same direct product decision as above — a PR is a discrete,
      -- already-approved-by-raising-it request, not part of "has Design finished this project's
      -- whole BOM yet." PMB-imported/manual/template lines still need it exactly as before.
      WHERE (p.status = 'active' OR p.is_system = 1) AND b.pending_review = 0
        AND (b.source != 'bom' OR b.pr_item_id IS NOT NULL OR EXISTS (
  ...
  // Multi-unit quantity multiplier — rows can span several projects, so one unscoped rollup map
  // covers all of them (bom_assemblies is tiny table-wide; see getAssemblyRollupMap's own comment).
  // Whole-BOM Unit Count is the same story one level up — one unscoped project-unit-count map,
  // looked up per row by its own project_id.
  const [rollupById, unitCounts] = await Promise.all([getAssemblyRollupMap(), getProjectUnitCounts()]);
  return rows.map(r => ({
    ...r,
    qty_breakdown: qtyBreakdown(r.qty_text, r.assembly_id, rollupById, unitCounts.get(r.project_id) ?? 1, !!r.qty_resolved),
  }));
}

// Unified delivery/lot-centric receiving — lot-aware expected-delivery data for Stores' "Receive a
// Delivery" tab. Deliberately NOT folded into getSourcingItems() itself: that function also feeds
// /procurement, which has no use for this data, so this is a separate merge step called only where
// needed (the Stores page). Mirrors getPoDeliveryLots()'s own allocated/unallocated + RFQ-fallback
// logic ("keep the RFQ date" for whatever's still unallocated, lot dates otherwise), just batched
// across many bom_items instead of scoped to one PO — same 1e-6 epsilon, same "no po_items yet
// means fully unallocated" rule.
//
// Per row this attaches: `delivery_lots` (one entry per real lot covering this item, each carrying
// its own PO so the UI can group per PO — a bom_item can in principle span more than one po_item/PO,
// §5bx's own `getPlannedRecipients` ponytail note), `unscheduled_qty`/`unscheduled_expected_delivery`
// (whatever's not yet in any lot, falling back to the RFQ date — only set when real), and
// `nearest_expected_delivery` (min of every date above, for sorting/date-filtering) and
// `all_expected_dates` (the same set, deduped and sorted, for a "+N more" UI hint). No lots exist
// anywhere for an item → `delivery_lots: []`, `unscheduled_expected_delivery` is just the plain RFQ
// date, i.e. the original pre-lots behavior. Mutates and returns `items` in place, same style
// getPoDeliveryLots already uses for its own it.unallocated_qty.
export async function attachDeliveryLotDates(items) {
  if (!items.length) return items;
  const quoteIds = [...new Set(items.map(r => r.selected_quote_id).filter(Boolean))];
  const bomIds = items.map(r => r.id);
  const [quotes, poItems] = await Promise.all([
    quoteIds.length
      ? queryAll(`SELECT id, expected_delivery_date FROM supplier_quotes WHERE id IN (${quoteIds.map(() => '?').join(',')})`, quoteIds)
      : [],
    queryAll(
      `SELECT poi.id, poi.bom_item_id, poi.qty, poi.po_id, po.po_no
         FROM po_items poi JOIN purchase_orders po ON po.id = poi.po_id
        WHERE poi.bom_item_id IN (${bomIds.map(() => '?').join(',')})`, bomIds),
  ]);
  const rfqDateByQuote = new Map(quotes.map(q => [q.id, q.expected_delivery_date]));
  const poItemIds = poItems.map(p => p.id);
  const lotRows = poItemIds.length
    ? await queryAll(
        `SELECT li.po_item_id, li.qty, pdl.lot_label, pdl.expected_delivery_date
           FROM po_delivery_lot_items li JOIN po_delivery_lots pdl ON pdl.id = li.lot_id
          WHERE li.po_item_id IN (${poItemIds.map(() => '?').join(',')})
          ORDER BY pdl.expected_delivery_date, pdl.id`, poItemIds)
    : [];
  const allocatedByPoItem = new Map();
  const lotRowsByPoItem = new Map();
  for (const lr of lotRows) {
    allocatedByPoItem.set(lr.po_item_id, (allocatedByPoItem.get(lr.po_item_id) || 0) + lr.qty);
    if (!lotRowsByPoItem.has(lr.po_item_id)) lotRowsByPoItem.set(lr.po_item_id, []);
    lotRowsByPoItem.get(lr.po_item_id).push(lr);
  }
  const poItemsByBomItem = new Map();
  for (const poi of poItems) {
    if (!poItemsByBomItem.has(poi.bom_item_id)) poItemsByBomItem.set(poi.bom_item_id, []);
    poItemsByBomItem.get(poi.bom_item_id).push(poi);
  }
  for (const r of items) {
    const poi = poItemsByBomItem.get(r.id) || [];
    const lots = [];
    let unscheduledQty = 0;
    let anyUnscheduled = poi.length === 0; // no PO line at all -> nothing scheduled, plain RFQ fallback
    for (const p of poi) {
      for (const lr of (lotRowsByPoItem.get(p.id) || [])) {
        lots.push({ po_id: p.po_id, po_no: p.po_no, lot_label: lr.lot_label, qty: lr.qty, expected_delivery_date: lr.expected_delivery_date });
      }
      const remainder = p.qty - (allocatedByPoItem.get(p.id) || 0);
      if (remainder > 1e-6) { anyUnscheduled = true; unscheduledQty += remainder; }
    }
    lots.sort((a, b) => (a.expected_delivery_date || '').localeCompare(b.expected_delivery_date || ''));
    r.delivery_lots = lots;
    r.unscheduled_qty = Math.round(unscheduledQty * 1e6) / 1e6;
    r.unscheduled_expected_delivery = anyUnscheduled ? (rfqDateByQuote.get(r.selected_quote_id) || null) : null;
    const allDates = [...new Set([...lots.map(l => l.expected_delivery_date), r.unscheduled_expected_delivery].filter(Boolean))].sort();
    r.nearest_expected_delivery = allDates[0] || null;
    r.all_expected_dates = allDates;
  }
  return items;
}

// Procurement's own "Overdues" tab — every already-committed (Ordered/Transit) item whose nearest
// expected date has passed and hasn't arrived. Deliberately reuses attachDeliveryLotDates() above
// rather than re-deriving the date logic against po_delivery_lots directly — that function already
// covers both a real scheduled Delivery Lot *and* the RFQ-quoted fallback for an item never put
// into one, which is exactly the same "as we show in Stores" comparison this tab is meant to
// mirror; a lot-only query would silently miss the unscheduled case.
export async function getOverdueDeliveries() {
  const rows = await queryAll(
    `SELECT b.id, b.material_description, b.project_id, b.selected_quote_id, b.purchase_status,
            p.project_no, p.is_system AS project_is_system, b.source, b.sale_order_no,
            s.name AS supplier_name, s.phone, s.email, s.contact_person
       FROM bom_items b
       JOIN projects p ON p.id = b.project_id
       LEFT JOIN supplier_quotes sq ON sq.id = b.selected_quote_id
       LEFT JOIN suppliers s ON s.id = sq.supplier_id
      WHERE b.purchase_status IN ('Ordered', 'Transit')`);
  const withDates = await attachDeliveryLotDates(rows);
  const today = todayISO();
  return withDates.filter(r => r.nearest_expected_delivery && r.nearest_expected_delivery < today);
}

// PR History — Requests/Engineering's own record of every PR ever raised, header-first. Unlike
// getSourcingItems() above (deliberately scoped to "still needs sourcing" — excludes
// pending_review=1, requires release_bom done), this shows every PR regardless of where its lines
// currently sit, including Received/Cancelled/In-Stock terminal ones and lines still awaiting
// Stores Review. One row per PR item, each carrying its own per-project split(s) as `lines` — a
// project split's dimensions are read from bom_items.size_spec, already the fully-rendered display
// string (categoryDisplaySpec()) written once at raise time — no need to re-parse
// category_fields_json here.
export async function getPurchaseRequisitions() {
  const rows = await queryAll(
    `SELECT pr.id AS pr_id, pr.pr_no, pr.raised_by_dept, pr.created_by, pr.created_at AS pr_created_at,
            pi.id AS pr_item_id, pi.material_description, pi.moc AS pr_item_moc, pi.sort_order,
            b.id AS bom_item_id, b.project_id, b.purchase_status, b.qty_text, b.size_spec,
            b.source, b.sale_order_no,
            p.project_no, p.customer_name, p.is_system AS project_is_system
       FROM purchase_requisitions pr
       JOIN pr_items pi ON pi.pr_id = pr.id
       LEFT JOIN bom_items b ON b.pr_item_id = pi.id
       LEFT JOIN projects p ON p.id = b.project_id
      ORDER BY pr.created_at DESC, pi.sort_order, b.id`
  );
  const byPr = new Map();
  for (const r of rows) {
    if (!byPr.has(r.pr_id)) {
      byPr.set(r.pr_id, {
        pr_id: r.pr_id, pr_no: r.pr_no, raised_by_dept: r.raised_by_dept,
        created_by: r.created_by, created_at: r.pr_created_at, items: new Map(),
      });
    }
    const pr = byPr.get(r.pr_id);
    if (!pr.items.has(r.pr_item_id)) {
      pr.items.set(r.pr_item_id, {
        pr_item_id: r.pr_item_id, material_description: r.material_description,
        moc: r.pr_item_moc, lines: [],
      });
    }
    if (r.bom_item_id != null) {
      pr.items.get(r.pr_item_id).lines.push({
        bom_item_id: r.bom_item_id, project_id: r.project_id, project_no: r.project_no,
        customer_name: r.customer_name, project_is_system: r.project_is_system,
        source: r.source, sale_order_no: r.sale_order_no,
        purchase_status: r.purchase_status, qty_text: r.qty_text, size_spec: r.size_spec,
      });
    }
  }
  return [...byPr.values()].map(pr => ({ ...pr, items: [...pr.items.values()] }));
}
...
export async function getProductionFlowCounts() {
  // 2026-09-26: the lifecycle is now the 33-stage Job Card sheet, grouped. A job counts under the
  // group/stage it is currently AT (first stage without an end date); finished jobs count as done.
  const [sheets, stages] = await Promise.all([
    queryAll(`SELECT js.id FROM job_sheets js LEFT JOIN projects p ON p.id = js.project_id
               WHERE js.project_id IS NULL OR p.status = 'active'`),
    queryAll(`SELECT sheet_id, name, group_key, end_date, qc_sign_by FROM job_sheet_stages ORDER BY sheet_id, sort_order`),
  ]);
  const ids = new Set(sheets.map(r => r.id));
  const by = new Map();
  for (const r of stages) { if (ids.has(r.sheet_id)) { if (!by.has(r.sheet_id)) by.set(r.sheet_id, []); by.get(r.sheet_id).push(r); } }
  const atStage = new Map();
  let finished = 0, waiting = 0;
  for (const rows of by.values()) {
    const cur = currentStage(rows);
    if (cur) atStage.set(cur.name, (atStage.get(cur.name) || 0) + 1); else finished++;
    if (waitingOnQc(rows)) waiting++;
  }
  const groups = JOB_SHEET_GROUPS.map(g => {
    const list = JOB_SHEET_STAGES.map((st, i) => ({ ...st, no: i + 1 })).filter(st => st.group === g.key)
      .map(st => ({ no: st.no, name: st.name, count: atStage.get(st.name) || 0 }));
    return { key: g.key, label: g.label, count: list.reduce((n, st) => n + st.count, 0), stages: list };
  });
  return { total: ids.size, finished, waitingOnQc: waiting, groups, href: '/production/shop?tab=jobcards' };
}

// Project picker for the PR composer (Group 5 Bundle A) — id/label only, no rollups needed.
export async function getActiveProjectsList({ includeChildren = false } = {}) {
  // series + created_at are additive for the QC workspace (series filter, latest-first ordering);
  // bom_release_revision is additive for the BOM workspace's project picker (§4, unreleased-by-
  // default filter); unit_count is additive for the Whole-BOM Unit Count control on that same
  // workspace's ReleaseReadinessPanel; existing callers ignore fields they don't ask for. Kept
  // ordered by project_no here for those callers — the QC workspace re-sorts latest-first
  // client-side. Filtering by release status happens only in the one caller that needs it, never
  // here — this function must keep returning the full active list for every other caller.
  //
  // Real bug found live (2026-09-04): a split master's own N child projects are genuinely
  // status='active', so every one of this function's 7 callers started showing all N of them
  // alongside the master the moment SB-1109-01-50 was split — Design/Engineering/Requests/Stores/
  // Installation/project-creation never want to see children at all (no per-child BOM/design work
  // exists), so master-only is the correct default. QC is the one deliberate exception — it
  // operates per-child (real QC documents are created per unit) and explicitly opts back in via
  // includeChildren, per app/qc/page.js.
  return queryAll(
    `SELECT id, project_no, customer_name, series, created_at, bom_release_revision, unit_count, master_project_id, company FROM projects
     WHERE status = 'active'${includeChildren ? '' : ' AND master_project_id IS NULL'} ORDER BY project_no`);
}
...
export async function getDesignWork() {
  const [projects, milestones, sheets, sheetSnapshots, drawings] = await Promise.all([
    // master_project_id IS NULL: same real bug as getActiveProjectsList() above — Design never does
    // per-child work, so a split project's N children must not each get their own row here.
    queryAll("SELECT id, project_no, customer_name FROM projects WHERE status = 'active' AND master_project_id IS NULL"),
...
export async function getDispatchWork() {
  const [projects, bomItems, packingItemStatus, lists, routing, pendingInward, mastersRows] = await Promise.all([
    queryAll("SELECT id, project_no, customer_name FROM projects WHERE status = 'active'"),
    queryAll("SELECT id, project_id, source, purchase_status, requires_manufacturing, production_done FROM bom_items WHERE purchase_status != 'Cancelled'"),
    ...
    // Stores/Inventory hardening Phase 5 — same routing gate getProjectBom() now applies, so this
    // cross-project bottleneck count never drifts from the per-project view's own definition of
    // "ready." Joined by (bom_item_id, child_project_id) — a normal/sibling project's own routing
    // decision keys on its own id as both, same as getProjectBom()'s query already relies on.
    queryAll('SELECT bom_item_id, child_project_id, routed_to FROM bom_item_child_routing'),
    // Same gap-fix as getProjectBom() — see that function's own comment for the full trace.
    queryAll("SELECT DISTINCT bom_item_id FROM inward_approvals WHERE status = 'pending'"),
    // Routing decoupled from receiving (gentle-snuggling-wozniak.md §7/§10) — a split master's own
    // project has children, so its own BOM items must stay outside the routing-eligible population
    // exactly like getProjectBom()'s per-row has_children. Dispatch legitimately keeps both master
    // and child rows here (one row per physical unit), unlike getActiveProjectsList()'s own
    // master-only default — this set is only for the routing-eligibility check below.
    queryAll('SELECT DISTINCT master_project_id AS id FROM projects WHERE master_project_id IS NOT NULL'),
  ]);
  const routingMap = new Map(routing.map(r => [`${r.bom_item_id}:${r.child_project_id}`, r.routed_to]));
  const pendingInwardIds = new Set(pendingInward.map(r => r.bom_item_id));
  const mastersWithChildren = new Set(mastersRows.map(r => r.id));
  ...
  return projects.map((p) => {
    ...
    const readyNotCarried = items.filter((b) => {
      if (carriedIds.has(b.id) || pendingInwardIds.has(b.id)) return false;
      const baseReady = b.requires_manufacturing ? b.production_done : (b.purchase_status === 'Received' || b.purchase_status === 'In-Stock');
      if (!baseReady) return false;
      // Routing decoupled from receiving — see getProjectBom()'s own comment for the full trace.
      const routingEligible = b.source === 'bom' && !mastersWithChildren.has(b.project_id);
      if (!routingEligible) return true;
      const routedTo = routingMap.get(`${b.id}:${b.project_id}`);
...
export async function getPendingPackingItems() {
  const [rows, rollupById, unitCounts, pendingInward] = await Promise.all([
    queryAll(
      `SELECT b.id, b.material_description, b.qty_text, b.moc, b.size_spec, b.source,
              b.purchase_status, b.requires_manufacturing, b.production_done,
              b.assembly_id, b.qty_resolved,
              EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id) AS has_children,
              p.id AS project_id, p.project_no, p.customer_name,
              bcr.routed_to AS self_routed_to
         FROM bom_items b
         JOIN projects p ON p.id = b.project_id
         LEFT JOIN bom_item_child_routing bcr ON bcr.bom_item_id = b.id AND bcr.child_project_id = b.project_id
        WHERE p.status = 'active'
          AND b.purchase_status != 'Cancelled'
          AND b.id NOT IN (SELECT bom_item_id FROM packing_items WHERE bom_item_id IS NOT NULL)
        ORDER BY p.project_no, b.sort_order, b.id`),
    getAssemblyRollupMap(), getProjectUnitCounts(),
    // Same gap-fix as getProjectBom() — see that function's own comment for the full trace.
    queryAll("SELECT DISTINCT bom_item_id FROM inward_approvals WHERE status = 'pending'"),
  ]);
  const pendingInwardIds = new Set(pendingInward.map(r => r.bom_item_id));
  // readyForPacking mirrors getProjectBom()'s exact predicate — never a second, independently-
  // drifting definition of "ready." qty_breakdown — gap #4/#6 (2026-09-04): the SELECT was missing
  // assembly_id/qty_resolved entirely, so this cross-project pre-generation preview (Dispatch's own
  // "Pending" tab, distinct from the draft-packing-list view getPackingDetail already covers)
  // couldn't show the whole-BOM/per-node rollup wired everywhere else in §5be. Stores/Inventory
  // hardening Phase 5 — self_routed_to now gates this readyForPacking too, same fallback discipline
  // as getProjectBom()'s own. Routing decoupled from receiving (gentle-snuggling-wozniak.md §7/§10)
  // — see getProjectBom()'s own comment for the full explanation of why a NULL self_routed_to on a
  // routing-eligible line (source='bom', no children) can no longer fall through as ready.
  return rows.map((b) => {
    const unitCount = unitCounts.get(b.project_id) ?? 1;
    const baseReady = b.requires_manufacturing
      ? !!b.production_done
      : (b.purchase_status === 'Received' || b.purchase_status === 'In-Stock');
    const routingEligible = b.source === 'bom' && !b.has_children;
    const readyForPacking = baseReady && !pendingInwardIds.has(b.id) && (
      !routingEligible || b.self_routed_to === 'dispatch' || (b.self_routed_to === 'production' && b.production_done)
    );
    return {
      ...b,
      readyForPacking,
      qty_breakdown: qtyBreakdown(b.qty_text, b.assembly_id, rollupById, unitCount, !!b.qty_resolved),
    };
  });
}

// Stores' Allocate screen (gentle-snuggling-wozniak.md §5/§10) — cross-project, same shape as
// getPendingPackingItems() above: every fully-received, routing-eligible line (source='bom', no
// split-order children) that hasn't been routed yet. Routing is now fully decoupled from
// receiving (§7/§9) — this is the queue that closes the loop. Attaches both this line's own
// frozen `requires_manufacturing` (drives the Production/Dispatch pre-fill — untouched by this
// whole feature) and, when catalog-linked, the live `items.default_requires_manufacturing` (the
// "Default" column — a genuinely different, forward-looking value, never conflated with the
// per-line one).
export async function getUnroutedReceivedItems() {
  const [rows, rollupById, unitCounts] = await Promise.all([
    queryAll(
      `SELECT b.id, b.material_description, b.qty_text, b.assembly_id, b.qty_resolved,
              b.requires_manufacturing, b.item_id,
              p.id AS project_id, p.project_no, p.customer_name,
              i.default_requires_manufacturing
         FROM bom_items b
         JOIN projects p ON p.id = b.project_id
         LEFT JOIN items i ON i.id = b.item_id
        WHERE p.status = 'active'
          AND b.source = 'bom'
          AND b.purchase_status IN ('Received', 'In-Stock')
          AND NOT EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id)
          AND NOT EXISTS (
            SELECT 1 FROM bom_item_child_routing bcr
             WHERE bcr.bom_item_id = b.id AND bcr.child_project_id = b.project_id)
          -- Same guard getProjectBom()/getPendingPackingItems()/getDispatchWork() already apply:
          -- a line QC hasn't cleared yet isn't real usable stock, so it shouldn't be routable yet
          -- either — routing it now would be a decision made on material that might still be
          -- rejected. Real, confirmed gap this query previously missed.
          AND NOT EXISTS (
            SELECT 1 FROM inward_approvals ia WHERE ia.bom_item_id = b.id AND ia.status = 'pending')
        ORDER BY p.project_no, b.sort_order, b.id`),
    getAssemblyRollupMap(), getProjectUnitCounts(),
  ]);
  return rows.map(b => {
...
export async function getRfqSummaryByItem() {
  const rows = await queryAll(
    `SELECT ri.bom_item_id, ri.rfq_id, r.rfq_no,
      ...
      WHERE ri.bom_item_id IS NOT NULL AND r.status != 'closed'
...
export async function getRfqByToken(token) {
  ...
  if (!rs) return null;
  const items = await queryAll(
    `SELECT ri.id AS rfq_item_id, b.id AS bom_item_id, b.material_description, b.moc, b.size_spec,
            b.qty_text, b.assembly_id, b.qty_resolved, b.project_id
       FROM rfq_items ri JOIN bom_items b ON b.id = ri.bom_item_id
      WHERE ri.rfq_id = ?`, [rs.rfq_id]);
  // Gap #1 (2026-09-04) — a supplier quoting through their private portal link only ever saw the
  // raw per-unit qty_text ("Qty 2 Mtrs"), not the real multiplied order size. Their submitted quote
  // (unit price/UoM/terms only, never a quantity) stays correct either way — this is a transparency
  // fix, not a pricing-correctness one. Computed inside the plain-object-copy map below (not by
  // mutating the raw Row instances first) — see that map's own comment on why a Row needs a real
  // plain-object copy here specifically.
  const [rollupById, unitCounts] = await Promise.all([getAssemblyRollupMap(), getProjectUnitCounts()]);
  ...
  return {
    ...
    items: items.map(it => ({
      ...it,
      qty_breakdown: qtyBreakdown(it.qty_text, it.assembly_id, rollupById, unitCounts.get(it.project_id) ?? 1, !!it.qty_resolved),
    })),
  };
}
...
export async function getSaleOrders() {
  ...
  // GROUP BY so.id + MIN(p.id): defends against fan-out if two multi-unit master projects ever
  // intentionally share one Sale Order (MULTI-UNIT-SPLIT-DESIGN.md §7's variant-order convention) —
  // a bare LEFT JOIN would otherwise duplicate this Sale Order's row once per linked project. The
  // UI only ever needs "does a project already exist" (to hide Convert to Project), not which
  // specific one, so picking the lowest id deterministically is correct for that purpose.
  return queryAll(
    `SELECT so.*, MIN(p.id) AS project_id,
            COALESCE(so.sales_person_override, u.display_name, so.created_by) AS sales_person,
       ...
       LEFT JOIN users u ON u.username = so.created_by
      GROUP BY so.id
...
// "open pipeline" total since they're resolved, not active. Stage set and the won/lost
// classification are both derived live from `sales_stages` (DB-configurable, currently the
// 9-value Sales Call funnel — Sales CRM expansion Phase 0c) rather than a hardcoded literal-string
// list, so a future stage rename/add can never silently drop opportunities from these counts the
// way a fixed 5-key object once did.
// Sales CRM plan 1b: Sales deals are enquiries now (the enquiry is the deal); Marketing's own
// opportunities are still counted as they are. Same row shape as before, so the tile is unchanged.
export async function getOpportunityPipelineCounts() {
  const [stages, rows] = await Promise.all([
    queryAll('SELECT name, is_won, is_lost FROM sales_stages WHERE active = 1 ORDER BY sort_order'),
    // Sales enquiries only: Marketing's opportunities are placeholder demo rows until Marketing is designed.
    queryAll(`SELECT sales_call_status AS stage, expected_value AS value_num, sales_call_closed_at FROM leads WHERE COALESCE(owner_dept, 'Sales') = 'Sales'`),
  ]);
  const counts = {};
  const closedByName = new Set();
  for (const s of stages) { counts[s.name] = 0; if (s.is_won || s.is_lost) closedByName.add(s.name); }
  let openValue = 0;
  for (const r of rows) {
    if (counts[r.stage] === undefined) continue; // a stage since renamed/deactivated — not counted, not crashed
    if (r.sales_call_closed_at && !closedByName.has(r.stage)) continue; // closed sales call at an open stage (isClosedCall)
    counts[r.stage]++;
    if (!closedByName.has(r.stage)) openValue += r.value_num || 0;
  }
  return { counts, openValue, total: Object.values(counts).reduce((a, b) => a + b, 0) };
}
...
export async function getSalesFlowCounts() {
  const [leads, quotations, saleOrders] = await Promise.all([
    ...
    // A scalar EXISTS, not a JOIN — a bare LEFT JOIN would double-count a Sale Order shared by two
    // variant-master projects (MULTI-UNIT-SPLIT-DESIGN.md §7) toward the "projects" tile below.
    queryAll("SELECT so.status, EXISTS(SELECT 1 FROM projects p WHERE p.sale_order_id = so.id) AS project_id FROM sale_orders so"),
  ]);
  return {
    leads: leads.filter(l => l.status === 'open').length, // open funnel stage (lib/lead-stage.mjs)
...
export async function getInventoryItems() {
  return queryAll(
    // Reservation/Issue reconciliation fix — qty - qty_issued: an already-issued portion of a
    // partially-issued reservation already left on_hand, so it must not also still be subtracted
    // here or `available` reads artificially low.
    `SELECT i.*,
            i.on_hand - COALESCE((SELECT SUM(r.qty - r.qty_issued) FROM inventory_reservations r
...
export async function getOpenBomItems() {
  const rows = await queryAll(
    `SELECT b.*, p.project_no, p.customer_name, p.is_system AS project_is_system,
            ...
            sq.expected_delivery_date AS expected_delivery_date
       FROM bom_items b
       JOIN projects p ON p.id = b.project_id
       LEFT JOIN items it ON it.id = b.item_id
       LEFT JOIN supplier_quotes sq ON sq.id = b.selected_quote_id
  ...
  const [rollupById, unitCounts] = await Promise.all([getAssemblyRollupMap(), getProjectUnitCounts()]);
  return rows.map(r => {
    const unitCount = unitCounts.get(r.project_id) ?? 1;
    return {
      ...r,
      qty_breakdown: qtyBreakdown(r.qty_text, r.assembly_id, rollupById, unitCount, !!r.qty_resolved),
      rolled_qty: itemRollupQty(r.qty_text, r.assembly_id, rollupById, unitCount, !!r.qty_resolved),
    };
  });
}

// Material Indent bridge — Production's own cross-project worklist of "material Stores has routed
// to me, ready to indent." Keys off bom_item_child_routing.routed_to='production' directly (never
// bom_items.requires_manufacturing) — see the plan file's §7: this is the real "material has become
// available and needs Production" signal, not the static BOM-time flag. A non-manufacturing line is
// never routed to Production in the first place, so it structurally can't appear here — no separate
// exclusion filter needed.
//
// Two joins, UNION ALL, matching the two real routing write paths (both write into the same table):
// (a) an ordinary/self-routed line — bcr.child_project_id = b.project_id, same join shape
//     getPendingPackingItems()/getChildDerivedBom() already use for self_routed_to; (b) a split
//     master's own line, routed per-child (bcr.child_project_id is a genuine CHILD, never the
//     master's own id — route-self's own guard already rejects a split master, so (a)'s join can
//     never accidentally match a split master's row). A line routed for several child units
//     produces one row per unit here (each was independently allocated/routed), all sharing the
//     same underlying bom_item_id and the same indent_project_id (the master).
//
// indent_project_id is always bom_items.project_id — for a split-child row this is the MASTER's id,
// not the specific child's. That's deliberate, not an oversight: material_indents.project_id is
// what the release route's own cross-check (app/api/material-indents/[id]/items/[itemId]/release/
// route.js) compares against bom_items.project_id, and bom_items only ever lives on the master —
// so an indent for split-routed material must be scoped to the master to release correctly.
// unit_project_no/unit_no are display-only context for Production (which physical unit this
// specific routing was for); material_indent_items has no column to persist that per-line today —
// a known, documented limitation, not silently dropped (the create UI should fold it into the
// indent's free-text notes when relevant, see components implementing this).
export async function getPendingProductionMaterialLines() {
  const rows = await queryAll(
    `SELECT b.id AS bom_item_id, b.material_description, b.moc, b.size_spec, b.qty_text,
            b.assembly_id, b.qty_resolved, b.project_id AS indent_project_id,
            p.project_no AS indent_project_no, p.customer_name,
            NULL AS unit_project_no, NULL AS unit_no, bcr.decided_at AS routed_at,
            it.item_code AS catalog_item_code, i.tracking_mode AS tracking_mode
       FROM bom_items b
       JOIN projects p ON p.id = b.project_id
       JOIN bom_item_child_routing bcr
            ON bcr.bom_item_id = b.id AND bcr.child_project_id = b.project_id AND bcr.routed_to = 'production'
       LEFT JOIN items it ON it.id = b.item_id
       LEFT JOIN inventory_items i ON i.id = b.inventory_item_id
      WHERE p.status = 'active' AND b.purchase_status != 'Cancelled'
        -- Same guard getProjectBom()/getPendingPackingItems()/getDispatchWork() already apply —
        -- material QC hasn't cleared yet isn't real, usable stock, so it shouldn't be indentable
        -- either. inward_approvals is keyed on bom_item_id alone (bom_items only ever lives on the
        -- master), so this applies identically regardless of which child this branch routes to.
        AND NOT EXISTS (SELECT 1 FROM inward_approvals ia WHERE ia.bom_item_id = b.id AND ia.status = 'pending')
      UNION ALL
     SELECT b.id AS bom_item_id, b.material_description, b.moc, b.size_spec, b.qty_text,
            b.assembly_id, b.qty_resolved, b.project_id AS indent_project_id,
            p.project_no AS indent_project_no, p.customer_name,
            cp.project_no AS unit_project_no, cp.unit_no AS unit_no, bcr.decided_at AS routed_at,
            it.item_code AS catalog_item_code, i.tracking_mode AS tracking_mode
       FROM bom_items b
       JOIN projects p ON p.id = b.project_id
       JOIN bom_item_child_routing bcr
            ON bcr.bom_item_id = b.id AND bcr.child_project_id != b.project_id AND bcr.routed_to = 'production'
       JOIN projects cp ON cp.id = bcr.child_project_id
       LEFT JOIN items it ON it.id = b.item_id
       LEFT JOIN inventory_items i ON i.id = b.inventory_item_id
      WHERE p.status = 'active' AND b.purchase_status != 'Cancelled'
        AND NOT EXISTS (SELECT 1 FROM inward_approvals ia WHERE ia.bom_item_id = b.id AND ia.status = 'pending')
      ORDER BY indent_project_no, bom_item_id`
  );
  if (!rows.length) return [];

  const bomItemIds = [...new Set(rows.map(r => r.bom_item_id))];
  const placeholders = bomItemIds.map(() => '?').join(',');
  const [rollupById, unitCounts, indented, pieces] = await Promise.all([
    getAssemblyRollupMap(), getProjectUnitCounts(),
    // Already-indented coverage — bom_item-level only (material_indent_items has no per-unit
    // column), so this can't distinguish which specific child unit was already indented on a
    // split line; it's the best available signal without a schema change.
    queryAll(
      `SELECT bom_item_id, status, SUM(qty_requested) AS qty
         FROM material_indent_items WHERE bom_item_id IN (${placeholders}) AND status != 'cancelled'
        GROUP BY bom_item_id, status`, bomItemIds),
    // A representative piece per line, for piece-tracked material — code/heat/status, same fields
    // every other piece-aware screen in this app already surfaces.
    queryAll(
      `SELECT bom_item_id, code, heat_no, status FROM stock_pieces
        WHERE bom_item_id IN (${placeholders}) AND status IN ('available','reserved')
        ORDER BY id`, bomItemIds),
  ]);
  const indentedByItem = new Map();
  indented.forEach(r => {
    const cur = indentedByItem.get(r.bom_item_id) || { open: 0, released: 0 };
    if (r.status === 'released') cur.released += Number(r.qty);
    else cur.open += Number(r.qty);
    indentedByItem.set(r.bom_item_id, cur);
  });
  const pieceByItem = new Map();
  pieces.forEach(p => { if (!pieceByItem.has(p.bom_item_id)) pieceByItem.set(p.bom_item_id, p); });

  return rows.map(r => {
    const unitCount = r.unit_project_no ? 1 : (unitCounts.get(r.indent_project_id) ?? 1);
    const requiredQty = itemRollupQty(r.qty_text, r.assembly_id, rollupById, unitCount, !!r.qty_resolved);
    const indentStatus = indentedByItem.get(r.bom_item_id) || null;
    return {
      ...r,
      required_qty: requiredQty,
      already_indented_open: indentStatus?.open || 0,
      already_indented_released: indentStatus?.released || 0,
      piece: pieceByItem.get(r.bom_item_id) || null,
    };
  });
}

// One indent + its items, with the item context (spec/MOC/qty_text) both the JSON detail route and
// the PDF route need — shared so the two never drift apart, same precedent as getPurchaseOrderDetail
// being used by both the PO's own API route and its PDF route.
export async function getMaterialIndentDetail(id) {
  ...
  const items = await queryAll(
    `SELECT mii.*, b.material_description AS bom_description, b.size_spec, b.moc, b.qty_text,
            i.description AS inventory_description, i.tracking_mode AS tracking_mode
...
// Project View redesign, Wave 2 — the Production card's Material Indents summary. Note the same
// caveat getPendingProductionMaterialLines() already documents: indent_project_id is always the
// BOM item's own project_id, which for a split-order line is the MASTER's id, not the specific
// child's — deliberate, since bom_items only lives on the master.
// Project View redesign, Decision K — the Stores card's "Total project inventory": what Stores
// physically has in hand for this project right now (Received/In-Stock only — not the whole BOM,
// which is the separate common-BOM row's job).
export async function getProjectInventoryItems(projectId) {
  return queryAll(
    `SELECT id, material_description, moc, size_spec, qty_text, purchase_status, grn_ref
       FROM bom_items WHERE project_id = ? AND purchase_status IN ('Received', 'In-Stock')
      ORDER BY material_description`, [projectId]);
}

export async function getMaterialIndentsByProject(projectId) {
  return queryAll(
    `SELECT mi.id, mi.indent_no, mi.status, mi.created_at,
            COUNT(mii.id) AS item_count
       FROM material_indents mi LEFT JOIN material_indent_items mii ON mii.indent_id = mi.id
      WHERE mi.project_id = ?
      GROUP BY mi.id ORDER BY mi.created_at DESC`, [projectId]);
}
...
export async function getReorderSuggestions() {
  return queryAll(
    // Reservation/Issue reconciliation fix — qty - qty_issued, same reasoning as getInventoryItems().
    `SELECT i.*,
            i.on_hand - COALESCE((SELECT SUM(r.qty - r.qty_issued) FROM inventory_reservations r
                                    WHERE r.inventory_item_id = i.id AND r.status = 'active'), 0) AS available
       FROM inventory_items i
      WHERE i.reorder_point IS NOT NULL
        AND (i.on_hand - COALESCE((SELECT SUM(r.qty - r.qty_issued) FROM inventory_reservations r
...
// STERP item 14, Formal GIR — Gate Inward Receipt log, newest first. Also attaches any
// stock_receipts row that already cross-references this GIR (S6 link, lib/db.js) — makes the
// existing link visible on the GIR side instead of only discoverable from ReceiptPicker.jsx, and
// lets the UI default a Close action's GRN ref to a real receipt's own value instead of always
// requiring separate re-entry.
export async function getGateInwardReceipts() {
  const girs = await queryAll(`SELECT * FROM gate_inward_receipts ORDER BY gir_no DESC`);
  if (!girs.length) return girs;
  const receipts = await queryAll(
    `SELECT sr.id, sr.inward_batch_no, sr.grn_ref, sr.gate_inward_receipt_id, s.name AS supplier_name
       FROM stock_receipts sr LEFT JOIN suppliers s ON s.id = sr.supplier_id
      WHERE sr.gate_inward_receipt_id IN (${girs.map(() => '?').join(',')})`,
    girs.map(g => g.id)
  );
  const byGir = new Map();
  for (const r of receipts) {
    if (!byGir.has(r.gate_inward_receipt_id)) byGir.set(r.gate_inward_receipt_id, []);
    byGir.get(r.gate_inward_receipt_id).push(r);
  }
  return girs.map(g => ({ ...g, linked_receipts: byGir.get(g.id) || [] }));
}
...
export async function getPurchaseOrderDetail(id) {
  ...
  if (!po) return null;
  // Phase 4 (QC statutory-forms plan) — requires_* flags via the po_item's own real bom_item_id
  // link, so the PO PDF can print a compact requirement note the supplier can actually act on.
  // Unified delivery/lot-centric receiving, Phase 0 — has_children lets ProcurementWorkspace.jsx
  // know when a line's own project is a split master (only those get the lot-reference UI at all).
  const items = await queryAll(
    `SELECT poi.*, p.project_no, p.company AS project_company,
            bi.requires_heat_no, bi.requires_mtc, bi.requires_supplier_batch, bi.requires_serial_no,
            EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = poi.project_id) AS has_children,
            EXISTS (SELECT 1 FROM po_delivery_lot_items li WHERE li.po_item_id = poi.id) AS has_delivery_lots
       FROM po_items poi
       LEFT JOIN projects p ON p.id = poi.project_id
       LEFT JOIN bom_items bi ON bi.id = poi.bom_item_id
  ...
  // Phase 0's lot references — fetched separately (one-to-many per po_item) and grouped in, same
  // shape convention as getGatePasses()'s items-per-header grouping elsewhere in this file.
  const masterItemIds = items.filter(i => i.has_children).map(i => i.id);
  if (masterItemIds.length) {
    const placeholders = masterItemIds.map(() => '?').join(',');
    const refRows = await queryAll(
      `SELECT ec.po_item_id, ec.lot_label, ec.child_project_id, p.project_no, p.unit_no
         FROM bom_item_expected_children ec JOIN projects p ON p.id = ec.child_project_id
        WHERE ec.po_item_id IN (${placeholders}) ORDER BY ec.lot_label, p.unit_no`, masterItemIds);
    const byItem = new Map();
    for (const r of refRows) {
      if (!byItem.has(r.po_item_id)) byItem.set(r.po_item_id, new Map());
      const lots = byItem.get(r.po_item_id);
      if (!lots.has(r.lot_label)) lots.set(r.lot_label, []);
      lots.get(r.lot_label).push({ child_project_id: r.child_project_id, project_no: r.project_no });
    }
    for (const item of items) {
      const lots = byItem.get(item.id);
      item.expected_lots = lots ? [...lots.entries()].map(([lot_label, children]) => ({ lot_label, children })) : [];
    }
  } else {
    for (const item of items) item.expected_lots = [];
  }
  return { po, items };
}

// PO Delivery Lots — per-project/per-item expected delivery scheduling. rfq_expected_delivery_date
// is the fallback date for whatever's still unallocated ("keep the RFQ date" rule); it's computed
// for every item, the caller only displays it when unallocated_qty > 0. bom_item_id/pr_no back the
// BM-{id}/PR-{no} entity-code pills; has_children gates whether the child-project tagging UI shows.
export async function getPoDeliveryLots(poId) {
  const items = await queryAll(
    `SELECT poi.id, poi.description, poi.qty, poi.uom, poi.project_id, poi.bom_item_id, p.project_no,
            p.is_system AS project_is_system, bi.source, bi.sale_order_no, pr.pr_no,
            EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = poi.project_id) AS has_children,
            sq.expected_delivery_date AS rfq_expected_delivery_date,
            COALESCE((SELECT SUM(li.qty) FROM po_delivery_lot_items li WHERE li.po_item_id = poi.id), 0) AS allocated_qty
       FROM po_items poi
       LEFT JOIN projects p ON p.id = poi.project_id
       LEFT JOIN bom_items bi ON bi.id = poi.bom_item_id
       LEFT JOIN pr_items pi ON pi.id = bi.pr_item_id
       LEFT JOIN purchase_requisitions pr ON pr.id = pi.pr_id
       LEFT JOIN supplier_quotes sq ON sq.id = bi.selected_quote_id
      WHERE poi.po_id = ? ORDER BY poi.sort_order, poi.id`, [poId]);
  for (const it of items) it.unallocated_qty = Math.round((it.qty - it.allocated_qty) * 1e6) / 1e6;

  const lots = await queryAll(
    `SELECT id, lot_label, expected_delivery_date, notes, created_by, created_at
       FROM po_delivery_lots WHERE po_id = ? ORDER BY expected_delivery_date, id`, [poId]);
  if (lots.length) {
    const lotIds = lots.map(l => l.id);
    const placeholders = lotIds.map(() => '?').join(',');
    const lineRows = await queryAll(
      `SELECT li.id AS lot_item_id, li.lot_id, li.po_item_id, li.qty, poi.description, poi.bom_item_id,
              p.project_no, pr.pr_no
         FROM po_delivery_lot_items li
         JOIN po_items poi ON poi.id = li.po_item_id
         LEFT JOIN projects p ON p.id = poi.project_id
         LEFT JOIN bom_items bi ON bi.id = poi.bom_item_id
         LEFT JOIN pr_items pi ON pi.id = bi.pr_item_id
         LEFT JOIN purchase_requisitions pr ON pr.id = pi.pr_id
        WHERE li.lot_id IN (${placeholders}) ORDER BY li.id`, lotIds);
    // Child-project tags, grouped by lot_item_id — only rows for items that actually have any.
    const lotItemIds = lineRows.map(r => r.lot_item_id);
    const childrenByLotItem = new Map();
    if (lotItemIds.length) {
      const cph = lotItemIds.map(() => '?').join(',');
      const childRows = await queryAll(
        `SELECT c.lot_item_id, c.child_project_id, p.project_no
           FROM po_delivery_lot_item_children c JOIN projects p ON p.id = c.child_project_id
          WHERE c.lot_item_id IN (${cph})`, lotItemIds);
      for (const r of childRows) {
        if (!childrenByLotItem.has(r.lot_item_id)) childrenByLotItem.set(r.lot_item_id, []);
        childrenByLotItem.get(r.lot_item_id).push({ child_project_id: r.child_project_id, project_no: r.project_no });
      }
    }
    const byLot = new Map();
    for (const r of lineRows) {
      if (!byLot.has(r.lot_id)) byLot.set(r.lot_id, []);
      byLot.get(r.lot_id).push({
        po_item_id: r.po_item_id, qty: r.qty, description: r.description, bom_item_id: r.bom_item_id,
        project_no: r.project_no, pr_no: r.pr_no, child_projects: childrenByLotItem.get(r.lot_item_id) || [],
      });
    }
    for (const lot of lots) lot.items = byLot.get(lot.id) || [];
  }
  return { items, lots };
}

// Unified delivery/lot-centric receiving, Phase 0 — "who is this material planned for," read at
// receiving time. Siblings (a source='bom' item split across unrelated normal projects at PR-raise
// time, pr_item_projects) need no new schema; master/child recipients come from Procurement's own
// bom_item_expected_children reference (§ above), scoped to whichever lot Stores picked. Never a
// hard block either way — an empty/absent reference just means the caller falls back to the live
// remaining-need computation, exactly as every other Stores queue in this app already does.
export async function getPlannedRecipients(bomItemId, lotLabel) {
  const item = await queryOne('SELECT id, project_id, pr_item_id, item_id FROM bom_items WHERE id = ?', [bomItemId]);
  if (!item) return null;

  if (item.pr_item_id) {
    const siblings = await queryAll(
      `SELECT b.id AS bom_item_id, b.project_id, b.qty_text, b.item_id, b.requires_manufacturing,
              b.purchase_status, p.project_no,
              EXISTS (SELECT 1 FROM projects c WHERE c.master_project_id = b.project_id) AS has_children
         FROM bom_items b JOIN projects p ON p.id = b.project_id
        WHERE b.pr_item_id = ? AND b.id != ?`, [item.pr_item_id, bomItemId]);
    // Catalog-linked (item_id) matches only, per decision — a sibling whose own catalog link has
    // since drifted from this item's is excluded rather than assumed still the same material. A
    // source item with no catalog link at all has nothing to cross-check, so every real PR sibling
    // still counts (the pr_item_id share is itself the strong signal in that case). Already-terminal
    // siblings (Received/Cancelled) have nothing left to receive against.
    const filtered = (item.item_id ? siblings.filter(s => s.item_id === item.item_id) : siblings)
      .filter(s => s.purchase_status !== 'Received' && s.purchase_status !== 'Cancelled');
    if (filtered.length) {
      return {
        kind: 'sibling',
        recipients: filtered.map(s => ({
          bom_item_id: s.bom_item_id, project_id: s.project_id, project_no: s.project_no,
          qty_text: s.qty_text, requires_manufacturing: !!s.requires_manufacturing, has_children: !!s.has_children,
        })),
        lots: [],
      };
    }
  ...
  // Master/child — scoped to the most recently-created PO line covering this bom_item (po_items has
  // no timestamp of its own; id DESC is a reasonable recency proxy). ponytail: if a bom_item ever
  // spans two concurrently-relevant POs this needs real disambiguation, e.g. an explicit po_item_id
  // param — not observed in real use yet.
  const poItem = await queryOne('SELECT id FROM po_items WHERE bom_item_id = ? ORDER BY id DESC LIMIT 1', [bomItemId]);
  if (!poItem) return { kind: 'child', recipients: [], lots: [] };

  const rows = await queryAll(
    `SELECT ec.lot_label, ec.child_project_id, p.project_no
       FROM bom_item_expected_children ec JOIN projects p ON p.id = ec.child_project_id
      WHERE ec.po_item_id = ? ORDER BY ec.lot_label, p.unit_no`, [poItem.id]);
  const lots = [...new Set(rows.map(r => r.lot_label))];
  const chosen = lotLabel ?? (lots.length === 1 ? lots[0] : null);
  const recipients = chosen
    ? rows.filter(r => r.lot_label === chosen).map(r => ({ project_id: r.child_project_id, project_no: r.project_no }))
    : [];
  return { kind: 'child', recipients, lots };
}
...
// `salesMember` (plan 2a): a Sales member's username — their follow-ups only (own enquiries, or
// follow-ups planned for them). null = everything the departments allow.
export async function getDepartmentCalendar(departments, from, to, { salesMember = null } = {}) {
  const inDepts = departments.map(() => '?').join(',');
  const crmDepts = ['Sales', 'Marketing'].filter(d => departments.includes(d));
  const [tasks, milestones, followups] = await Promise.all([
    queryAll(
    ...
    // Sales CRM expansion Phase 4 — Diary follow-ups due on a given day, one crm_notes row per
    // reminder (a lead can have several planned follow-ups across time). crmDepts is empty (no
    // query needed) unless the viewer actually holds Sales or Marketing.
    crmDepts.length ? queryAll(
      `SELECT n.id, l.id AS lead_id, l.lead_name, l.company_name, l.owner_dept AS department,
              n.next_plan_date AS date, n.plan_time, n.plan_for, n.plan_of_action, n.content,
              n.in_time, n.out_time, n.location, n.contact_id, n.note_type, n.plan_note_type, n.action_taken,
              COALESCE(l.sub_location, l.district) AS lead_location, c.name AS contact_name, COALESCE(c.phone, l.phone) AS contact_phone
         FROM crm_notes n JOIN leads l ON l.id = n.lead_id LEFT JOIN contacts c ON c.id = n.contact_id
        WHERE l.owner_dept IN (${crmDepts.map(() => '?').join(',')}) AND n.next_plan_date BETWEEN ? AND ?
          ${salesMember ? "AND (l.owner_dept <> 'Sales' OR ? IN (l.account_manager, l.assigned_to, l.initiated_by, l.created_by, n.plan_for))" : ''}
        ORDER BY n.next_plan_date, n.id`,
      [...crmDepts, from, to, ...(salesMember ? [salesMember] : [])]
    ) : [],
  ]);
  return { tasks, milestones, followups };
}
  ...
  const [rows, stages] = await Promise.all([
    queryAll(
      `SELECT e.id, e.name, e.trade,
              d.status, d.project_id, d.milestone_id, d.notes, d.work_allocated
         FROM employees e
    ...
    // Job card stages that cover this date: started or finished on it, or in between.
    queryAll(
      `SELECT w.employee_id, s.name, js.jc_no FROM job_sheet_stages s
         JOIN job_sheets js ON js.id = s.sheet_id
         JOIN (SELECT id AS stage_id, fitter_employee_id AS employee_id FROM job_sheet_stages WHERE fitter_employee_id IS NOT NULL
               UNION SELECT stage_id, employee_id FROM job_sheet_stage_workers) w ON w.stage_id = s.id
        WHERE s.start_date IS NOT NULL
          AND (s.start_date = ?1 OR s.end_date = ?1 OR (s.end_date IS NOT NULL AND s.start_date < ?1 AND s.end_date > ?1)
               OR (s.end_date IS NULL AND s.start_date < ?1 AND ?1 <= date('now')))
        ORDER BY s.sort_order`, [date]),
  ]);
  const by = new Map();
  for (const s of stages) { const a = by.get(s.employee_id) || []; a.push(`${s.name} (${s.jc_no})`); by.set(s.employee_id, a); }
  return rows.map(r => ({ ...r, suggested_work: (by.get(r.id) || []).join('; ') }));
}
...
export async function getNotifications(userId, limit = 20) {
  ...
  await maybeSweepQuotationReminders(); // Sales CRM plan 2d backup path (hourly at most)
...
export async function getLeads() {
  const [leads, lines] = await Promise.all([
    queryAll('SELECT * FROM leads ORDER BY created_at DESC'),
    queryAll('SELECT * FROM lead_products ORDER BY lead_id, sort_order, id'),
  ]);
  // Plan 1e — each enquiry carries its product lines.
  const byLead = new Map();
  for (const l of lines) {
    if (!byLead.has(l.lead_id)) byLead.set(l.lead_id, []);
    byLead.get(l.lead_id).push({ ...l });
  }
  return leads.map(l => ({ ...l, products: byLead.get(l.id) || [] }));
}
...
// Sales CRM expansion Phase 0a/0b/0e — branches/product master/sales targets are small, static
// enough masters to fetch whole (same shape as getCampaigns() above), no pagination.
export async function getBranches() {
  return queryAll('SELECT * FROM branches ORDER BY name');
}

export async function getSalesProducts() {
  // Picker/list fields only — description and old-CRM attributes are fetched per product
  // (GET /api/sales-products/[id]) so /sales doesn't carry ~1,800 long descriptions.
  return queryAll(`SELECT id, product_code, legacy_code, product_name, product_type, category, price, cost_price,
      unit, hsn_code, gst_pct, warranty_days, serviceable, bom_structure_template_id, active
    FROM sales_products ORDER BY product_name`);
}
...
export async function getSalesTargets() {
  return queryAll(`
    SELECT t.*, b.name AS branch_name
     ...
     ORDER BY t.period DESC, b.name, t.account_manager
  `);
}
...
// Sales CRM expansion Phase 5 — the full Diary field set, joined to its Lead for company/owner
// context, feeding every "Sales Call"/Follow-up/Feedback report (Prospect Summary, Date/Location-
// wise, Neglected, Customer/Employee Follow-up, Feedback ×3). One shared fetch, client-side
// filtered/grouped per report — same "reuse already-fetched arrays" idiom the 6 CRM analytics
// reports already established, not a new query per report.
export async function getDiaryNotes() {
  return queryAll(`
    SELECT n.*, l.company_name, l.lead_name, l.owner_dept, l.branch_id, l.district, l.account_manager
      FROM crm_notes n JOIN leads l ON l.id = n.lead_id
...
// Every column except the old-CRM summary blobs (legacy_crm_json, products_of_interest) — those
// are read only on the customer's own detail sheet, so the ~9k-row list stays light.
export async function getCustomers() {
  return queryAll(`SELECT id, name, party_code, gst_no, pan, phone, fax, email, website, address, address2, address3,
      city, state, state_code, country, pin_code, area, excise_range, division, gst_trans_type, business_type,
      active, created_at, portal_enabled, portal_user_id, initial_email_sent_at, account_manager, source
...
export async function getProjectCosting(projectId, workOrderId = null) {
  const [material, labor, project] = await Promise.all([
    ...
    queryOne(
      `SELECT so.total AS selling_value, p.master_project_id FROM projects p
         LEFT JOIN sale_orders so ON so.id = p.sale_order_id
        WHERE p.id = ?`, [projectId]),
  ]);
  const materialCost = material.v;
  const laborCost = labor.v;
  const totalCost = materialCost + laborCost;
  // A split child inherits its master's own sale_order_id verbatim, so the join above would
  // otherwise return the WHOLE order's value on every one of N children (a real bug found and
  // fixed in getProjectProfitabilityLines, §5be follow-on) — commercial value is a master-level
  // concept per the confirmed architecture, so a child gets no selling value/margin at all here,
  // not a divided or duplicated one. materialCost/laborCost stay real per-child numbers (job cards
  // and, where genuinely raised, POs are still meaningfully scoped to this one project).
  const isChild = !!project?.master_project_id;
  const sellingValue = isChild ? null : (project?.selling_value || 0);
  const margin = isChild ? null : sellingValue - totalCost;
  return {
    materialCost, laborCost, totalCost, sellingValue, margin, isChild,
    marginPct: !isChild && sellingValue ? Math.round((margin / sellingValue) * 100) : null,
  };
}
...
export async function getProjectProfitabilityLines(company, { from, to } = {}) {
  // master_project_id IS NULL — a real gap found in a post-multi-unit-split gap audit, not caught
  // when this report was originally built (well before that feature existed) or when the split's
  // own picker-flooding bug was fixed: a split child inherits its master's own sale_order_id
  // verbatim (app/api/projects/[id]/split/route.js), so getProjectCosting(childId)'s
  // `JOIN sale_orders so ON so.id = p.sale_order_id` returns the SAME full Sale Order total for
  // every child as the master — a 50-unit split order would report its selling value 51 times over
  // in this exact aggregate (and in getCustomerProfitabilityLines, built directly on top of it).
  const conditions = ['company = ?', 'is_system = 0', 'master_project_id IS NULL'];
  const args = [company];
...
// Sales Payment Tracker log — see sale_order_payments in lib/db.js.
export async function getSalePayments() {
  return queryAll(`
    SELECT p.*, so.so_no, so.customer_name, so.total AS order_value, so.company, COALESCE(si.invoice_no, p.invoice_ref) AS invoice_no
     ...
     ORDER BY p.received_on IS NULL, p.received_on DESC, p.id DESC
  `);
}

export async function getSalesReturns() {
  return queryAll(`
    SELECT sr.*, so.so_no, so.customer_name, so.company, i.description AS inventory_description
...
export async function getPriceLists() {
  return queryAll(`
    SELECT pl.*, c.name AS customer_name,
           COALESCE(sp.product_name, i.item_name) AS item_name, COALESCE(sp.product_code, i.item_code) AS item_code,
           COALESCE(sp.unit, i.uom) AS item_uom
      FROM price_lists pl
      LEFT JOIN customers c ON c.id = pl.customer_id
      LEFT JOIN sales_products sp ON sp.id = pl.product_id
      LEFT JOIN items i ON i.id = pl.item_id
     ORDER BY COALESCE(sp.product_name, i.item_name), (pl.customer_id IS NULL), pl.valid_from DESC
  `);
}

export async function getSaleOrderDetail(id) {
  const [so, items, files] = await Promise.all([
    queryOne(`
      SELECT so.*, b.name AS branch_name
       ...
       WHERE so.id = ?
    `, [id]),
    queryAll(`SELECT soi.*, sp.product_code FROM sale_order_items soi LEFT JOIN sales_products sp ON sp.id = soi.product_id
                WHERE soi.sale_order_id = ? ORDER BY soi.sort_order, soi.id`, [id]),
    queryAll('SELECT * FROM sale_order_files WHERE sale_order_id = ? ORDER BY uploaded_at DESC', [id]),
  ]);
  if (!so) return null;
  // Sales CRM plan 1h — read-only reference fields for the PO wizard + order PDF, always looked up
  // live (never copied onto the order): our GST No / PAN / Entity Code from company_settings, the
  // customer's Customer Code / PAN / GST No, and the SOS No(s) of the project made from this order.
  const [companyRow, customer, sos] = await Promise.all([
    queryOne('SELECT gstin, pan, invoice_prefix, state_code FROM company_settings WHERE company = ?', [so.company]),
    so.customer_id ? queryOne('SELECT party_code, pan, gst_no, address, address2, address3, state_code FROM customers WHERE id = ?', [so.customer_id]) : null,
    queryAll(`SELECT s.id, p.project_no FROM scope_of_supply s JOIN projects p ON p.id = s.project_id
               WHERE p.sale_order_id = ? ORDER BY s.id`, [id]),
  ]);
  const references = {
    company_gstin: companyRow?.gstin || null, company_pan: companyRow?.pan || null,
    entity_code: companyRow?.invoice_prefix || null,
    customer_code: customer?.party_code || null, customer_pan: customer?.pan || null, customer_gst_no: customer?.gst_no || null,
    customer_address: [customer?.address, customer?.address2, customer?.address3].filter(Boolean),
    sos_numbers: sos.map(r => `SOS-${r.id}`), job_nos: [...new Set(sos.map(r => r.project_no))],
    company_state_code: companyRow?.state_code || null, customer_state_code: customer?.state_code || null,
  };
  return { ...so, items, files, references };
}
...
export async function getJobCardDetail(id) {
  const card = await queryOne(
    `SELECT jc.*, p.project_no, p.customer_name, p.master_project_id, o.name AS operation_name, w.name AS workstation_name,
            wo.wo_no, wo.product_description AS wo_product_description,
            tc.certificate_no AS test_certificate_no, tc.cast_no AS test_certificate_cast_no
       FROM job_cards jc
       LEFT JOIN projects p ON p.id = jc.project_id
       LEFT JOIN operations o ON o.id = jc.operation_id
       LEFT JOIN workstations w ON w.id = jc.workstation_id
       LEFT JOIN work_orders wo ON wo.id = jc.work_order_id
       LEFT JOIN test_certificates tc ON tc.id = jc.test_certificate_id
      WHERE jc.id = ?`,
    [id]
  );
  if (!card) return null;
  // Multi-unit split Phase F — informational only, same idiom as getPackingDetail's own drift flag.
  if (card.bom_release_revision_at_creation != null && card.master_project_id) {
    const master = await queryOne('SELECT bom_release_revision FROM projects WHERE id = ?', [card.master_project_id]);
    card.masterBomRevision = master?.bom_release_revision ?? null;
    card.bomRevisionDrift = card.masterBomRevision != null && card.masterBomRevision > card.bom_release_revision_at_creation;
  }
...
// Reusable Process Route Card templates (§5l addendum) — a real recurring stage sequence (e.g. a
// boiler model's standard traveler) saved once, applied to future Work Orders instead of rebuilt by
// hand every time. Flat, mirrors bom_templates' own list shape.
export async function getWorkOrderRouteTemplates() {
  return queryAll('SELECT * FROM work_order_route_templates ORDER BY name');
}

export async function getWorkOrderRouteTemplateDetail(id) {
  const template = await queryOne('SELECT * FROM work_order_route_templates WHERE id = ?', [id]);
  if (!template) return null;
  const items = await queryAll(
    `SELECT wrti.*, o.name AS operation_name, w.name AS workstation_name
       FROM work_order_route_template_items wrti
       LEFT JOIN operations o ON o.id = wrti.operation_id
       LEFT JOIN workstations w ON w.id = wrti.workstation_id
      WHERE wrti.template_id = ? ORDER BY wrti.seq, wrti.id`,
    [id]
  );
  return { ...template, items };
}

// Everything the printed Job Card traveler needs, one shot — project/work-order header fields plus
// one row per route step, each already carrying its fitter/welder (from job_card_time_logs — the
// existing "who + when" record, not a new field), production sign-off dates, and — only on a step
// that actually named a quality_checkpoint — the QC hold-release date/signer and linked certificate.
export async function getJobCardTravelerData(workOrderId) {
  const wo = await queryOne(
    `SELECT wo.*, p.project_no, p.customer_name
  ...
  if (!wo) return null;
  const rows = await queryAll(
    `SELECT jc.id, jc.jc_no, jc.status, jc.actual_start, jc.actual_end, jc.notes, jc.requires_qc_hold,
            jc.qc_released_at, jc.qc_released_by, wop.seq,
            COALESCE(o.name, wop.department, 'Step #' || wop.seq) AS stage,
            tc.certificate_no AS test_certificate_no,
            (SELECT GROUP_CONCAT(DISTINCT e.name) FROM job_card_time_logs tl
               JOIN employees e ON e.id = tl.employee_id WHERE tl.job_card_id = jc.id) AS workers
       FROM job_cards jc
       LEFT JOIN work_order_operations wop ON wop.id = jc.work_order_operation_id
       LEFT JOIN operations o ON o.id = jc.operation_id
       LEFT JOIN test_certificates tc ON tc.id = jc.test_certificate_id
      WHERE jc.work_order_id = ?
      ORDER BY wop.seq, jc.id`,
    [workOrderId]
  );
  return { workOrder: wo, rows };
}
...
// Cheap byId map for rollupQty()/itemRollupQty()/qtyBreakdown() (lib/bom-structure.mjs) — the
// multi-unit quantity multiplier's DB-aware half. bom_assemblies is tiny (dozens of rows total
// today, table-wide); assembly ids are globally-unique PKs and rollupQty only ever looks up by id,
// so a cross-project caller (getSourcingItems, getOpenBomItems — showing rows from several
// projects in one list) can safely pass no projectId and read the whole table in one query; a
// single-project write path (PO/reservation/packing creation) passes its own project_id to keep
// the read obviously scoped.
export async function getAssemblyRollupMap(projectId) {
  const rows = projectId
    ? await queryAll('SELECT id, parent_id, qty FROM bom_assemblies WHERE project_id = ?', [projectId])
    : await queryAll('SELECT id, parent_id, qty FROM bom_assemblies');
  return new Map(rows.map(a => [a.id, a]));
}

// Whole-BOM Unit Count's DB-aware half — same shape/scoping precedent as getAssemblyRollupMap right
// above (projects is a tiny table too), so one function serves both a single-project lookup
// (`.get(projectId) ?? 1`) and a cross-project list's per-row lookup (`.get(row.project_id) ?? 1`).
export async function getProjectUnitCounts(projectId) {
  const rows = projectId
    ? await queryAll('SELECT id, unit_count FROM projects WHERE id = ?', [projectId])
    : await queryAll('SELECT id, unit_count FROM projects');
  return new Map(rows.map(p => [p.id, p.unit_count]));
}

export async function getBomStructure(projectId) {
  const [assemblies, items, drawingCounts, weights, project] = await Promise.all([
    queryAll('SELECT * FROM bom_assemblies WHERE project_id = ? ORDER BY sort_order, id', [projectId]),
    // item_id/catalog_item_code — the real Item Master link, when this line was actually picked
    // from the catalog (never fabricated for a free-typed line) — same join getProjectBom() and
    // allBomRowsWithIdentity() already use, previously missing here so the tree had no way to show
    // it at all.
    // pr_no/pr_created_at — round 3 Phase B: the real Purchase Request a line came from, when it
    // did (bom_items.pr_item_id stamped at creation by the unified PR flow). Same join
    // getSourcingItems() already uses; NULL for PMB-imported/pre-PR-flow lines, same as there.
    queryAll(`SELECT b.id, b.assembly_id, b.material_description, b.qty_text, b.item_id, b.qty_resolved, it.item_code AS catalog_item_code,
                     b.pr_ref, pr.pr_no, pr.created_at AS pr_created_at
                FROM bom_items b LEFT JOIN items it ON it.id = b.item_id
                LEFT JOIN pr_items pi ON pi.id = b.pr_item_id
                LEFT JOIN purchase_requisitions pr ON pr.id = pi.pr_id
               WHERE b.project_id = ? AND b.assembly_id IS NOT NULL`, [projectId]),
    // BOM workspace Phase 2 — node-level drawing-link counts, so the tree can show a "3 dwg" badge
    // without a per-node fetch (a calc-count sibling used to sit here too — calc sheets now link to
    // a drawing instead of a tree node, see calc_sheet_drawings in lib/db.js).
    ...
    // Real recorded weight, never a fabricated estimate — only `consumed` stock_pieces that were
    // never themselves cut further (a source piece stays `consumed` too once it's been cut, and
    // still carries whatever bom_item_id it had from its own reservation — counting it alongside
    // its own used/remnant/scrap children would double the real weight). "Never cut further" = its
    // id never appears as another row's parent_id. Scrap rows are already excluded for free — that
    // INSERT never sets bom_item_id at all (lib/stock-pieces.js's cutPiece()). §8.
    queryAll(`SELECT sp.bom_item_id, SUM(sp.weight_kg) AS known_weight_kg, COUNT(*) AS piece_count
                FROM stock_pieces sp JOIN bom_items b ON b.id = sp.bom_item_id
               WHERE b.project_id = ? AND sp.status = 'consumed' AND sp.bom_item_id IS NOT NULL
                 AND sp.id NOT IN (SELECT DISTINCT parent_id FROM stock_pieces WHERE parent_id IS NOT NULL)
               GROUP BY sp.bom_item_id`, [projectId]),
    queryOne('SELECT unit_count FROM projects WHERE id = ?', [projectId]),
  ]);
  const byId = new Map(assemblies.map(a => [a.id, a]));
  const unitCount = project?.unit_count ?? 1;
  const weightByItem = new Map(weights.map(w => [w.bom_item_id, w]));
  const itemsByAssembly = new Map();
  for (const it of items) {
    if (!itemsByAssembly.has(it.assembly_id)) itemsByAssembly.set(it.assembly_id, []);
    const w = weightByItem.get(it.id);
    itemsByAssembly.get(it.assembly_id).push({
      ...it, rolled_qty: itemRollupQty(it.qty_text, it.assembly_id, byId, unitCount, !!it.qty_resolved),
      known_weight_kg: w?.known_weight_kg ?? null, weight_piece_count: w?.piece_count ?? 0,
    });
  }
  const drawingCountById = new Map(drawingCounts.map(r => [r.assembly_id, r.n]));
  // Weight roll-up needs the finished items[] arrays in place first (sumKnownWeight reads
  // node.items), so it's computed in a second pass over the same byId map rather than folded into
  // the single-pass rollupQty() call below.
  const byIdWithItems = new Map(assemblies.map(a => [a.id, { ...a, items: itemsByAssembly.get(a.id) || [] }]));
  const childrenByParent = groupByParent(assemblies);
  return assemblies.map(({ config_json, ...a }) => ({
    // Configuration (datasheet fields) as a parsed array; the raw JSON column is dropped so a release
    // snapshot (which freezes this output verbatim) doesn't store it twice. Static facts, safe to freeze.
    ...a, config: parseConfig(config_json),
    rollup_qty: rollupQty(a.id, byId, unitCount), items: itemsByAssembly.get(a.id) || [],
    drawing_count: drawingCountById.get(a.id) || 0,
    ...sumKnownWeight(a.id, byIdWithItems, childrenByParent),
  }));
}
...
// `projectIds` (round 3 Phase A) — optional, narrows to a checked subset from Engineering's shared
// multi-select filter; omitted/empty = every project, today's original behavior unchanged.
async function allBomRowsWithIdentity(projectIds) {
  const where = projectIds?.length ? `WHERE b.project_id IN (${projectIds.map(() => '?').join(',')})` : '';
  const rows = await queryAll(`
    SELECT b.id, b.project_id, b.item_id, b.material_description, b.moc, b.size_spec, b.qty_text,
      ...
      JOIN projects p ON p.id = b.project_id
      LEFT JOIN items it ON it.id = b.item_id
      ${where}`, projectIds?.length ? projectIds : []);
...
// the caller can show "same part, N places" directly. `projectIds` — see allBomRowsWithIdentity.
export async function getWhereUsed(query, projectIds) {
  const rows = await allBomRowsWithIdentity(projectIds);
...
// constant, easy to change if the real usage pattern turns out different. `projectIds` — filtering
// here (not client-side over an unfiltered fetch) is what keeps project_count/classification honest
// once scoped: a part's "common" status must be recomputed against the filtered set, not just have
// non-matching rows hidden from an already-computed count.
const COMMON_THRESHOLD = 2;
export async function getPartUsage(projectIds) {
  const rows = await allBomRowsWithIdentity(projectIds);
...
// `projectIds` (round 3 Phase A) — the multi-select filter's array form, alongside the pre-existing
// single `projectId`. Neither wins over the other by design — assemblyId still takes priority (a
// node's own History tab), then the single id, then the multi-select array, then unfiltered.
export async function getEngineeringChangeNotes(projectId = null, assemblyId = null, projectIds = null) {
  let where = '';
  let args = [];
  if (assemblyId) {
    ...
    where = `WHERE ecn.project_id IN (${projectIds.map(() => '?').join(',')})`;
    args = projectIds;
  }
  return queryAll(`
...
export async function getSalesInvoices({ projectId } = {}) {
  ...
  const args = [];
  if (projectId) {
    // Multi-unit split, client-confirmed 2026-09-04: one invoice covers the whole order, so it's
    // created against the master, not any one child. A child's own packing list (Dispatch's own
    // invoice-linking picker) asking "invoices for project 112" must also see its master's — a
    // bare project_id match would otherwise show "no invoices" for every real dispatched unit.
    const self = await queryOne('SELECT master_project_id FROM projects WHERE id = ?', [projectId]);
    const ids = self?.master_project_id ? [projectId, self.master_project_id] : [projectId];
    conditions.push(`si.project_id IN (${ids.map(() => '?').join(',')})`);
    args.push(...ids);
  }
...
export async function getSalesCreditNotes() {
  return queryAll(
    `SELECT scn.*, si.invoice_no, si.company FROM sales_credit_notes scn JOIN sales_invoices si ON si.id = scn.sales_invoice_id
...
export async function getSalesRegisterLines(company, { from, to } = {}) {
  ...
  return queryAll(
    `SELECT si.id, si.sale_order_id, si.quotation_id, si.created_by, si.invoice_no, si.invoice_date, c.name AS customer_name, si.subtotal, si.tax_amount, si.total
       FROM sales_invoices si JOIN customers c ON c.id = si.customer_id
      WHERE ${conditions.join(' AND ')}
...
export async function getDispatchRegisterLines(company, { from, to } = {}) {
  ...
  return queryAll(
    // pl.id is what lets the on-screen card (not the PDF/Excel export, which has no navigation
    // concept) link "Packing No" straight into /packing/{id} — Accounts' real, discoverable path
    // into the full document now that they have read access to it (see app/packing/[id]/page.js).
    `SELECT pl.id, pl.packing_no, COALESCE(pl.dispatched_at, pl.updated_at) AS dispatched_at,
...
// ---------------------------------------------------------------------------
// Job Card sheets — the 33-stage paper Job Card (lib/job-sheet-stages.mjs).
// ---------------------------------------------------------------------------
function summarizeSheet(sheet, stages) {
  const cur = currentStage(stages);
  return {
    ...sheet,
    total: stages.length,
    done: stages.filter(r => r.end_date).length,
    qc_signed: stages.filter(r => r.qc_sign_by).length,
    current_stage: cur ? { id: cur.id, name: cur.name, group_key: cur.group_key, fitter_name: cur.fitter_name || null } : null,
    waiting_on_qc: waitingOnQc(stages),
  };
}

export async function getJobSheets() {
  const [sheets, stages] = await Promise.all([
    queryAll(`SELECT js.*, p.project_no, p.customer_name AS project_name FROM job_sheets js
                LEFT JOIN projects p ON p.id = js.project_id ORDER BY js.id DESC`),
    queryAll(`SELECT s.*, e.name AS fitter_name FROM job_sheet_stages s
                LEFT JOIN employees e ON e.id = s.fitter_employee_id ORDER BY s.sheet_id, s.sort_order`),
  ]);
  const by = new Map();
  for (const s of stages) { if (!by.has(s.sheet_id)) by.set(s.sheet_id, []); by.get(s.sheet_id).push(s); }
  return sheets.map(sh => summarizeSheet(sh, by.get(sh.id) || []));
}

export async function getJobSheetDetail(id) {
  const sheet = await queryOne(
    `SELECT js.*, p.project_no, p.customer_name AS project_name FROM job_sheets js
       LEFT JOIN projects p ON p.id = js.project_id WHERE js.id = ?`, [id]);
  if (!sheet) return null;
  const stages = await queryAll(
    `SELECT s.*, e.name AS fitter_name,
            (SELECT GROUP_CONCAT(x.employee_id) FROM job_sheet_stage_workers x WHERE x.stage_id = s.id) AS extra_worker_ids,
            (SELECT GROUP_CONCAT(e2.name, ', ') FROM job_sheet_stage_workers x JOIN employees e2 ON e2.id = x.employee_id WHERE x.stage_id = s.id) AS extra_worker_names,
            tc.certificate_no AS test_certificate_no, tc.cast_no AS test_certificate_cast_no
       FROM job_sheet_stages s
       LEFT JOIN employees e ON e.id = s.fitter_employee_id
       LEFT JOIN test_certificates tc ON tc.id = s.test_certificate_id
      WHERE s.sheet_id = ? ORDER BY s.sort_order`, [id]);
  return { ...summarizeSheet(sheet, stages), stages };
}

// QC Approvals > Job Card Stages queue: stages Production finished that QC has not signed yet.
export async function getPendingJobSheetStages() {
  return queryAll(
    `SELECT s.id, s.sheet_id, s.name, s.end_date, e.name AS fitter_name, js.jc_no, js.job_number, js.project_id,
            p.project_no
       FROM job_sheet_stages s JOIN job_sheets js ON js.id = s.sheet_id
       LEFT JOIN employees e ON e.id = s.fitter_employee_id
       LEFT JOIN projects p ON p.id = js.project_id
      WHERE s.end_date IS NOT NULL AND s.qc_sign_by IS NULL
      ORDER BY s.end_date, js.id, s.sort_order`);
}
