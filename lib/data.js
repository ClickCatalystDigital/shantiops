...
export async function getCustomerView(projectId) {
  ...
  return { project, phases, estDispatch: estDispatch || null, packingListId: packing?.id || null, drawings };
}
...
export async function getQcDocumentDetail(id) {
  ...
  return { document, parts, mountings };
}
...
export async function getProjectBom(projectId) {
  ...
  const readyForPacking = pending.filter(b => b.production_done);
...
export async function getProductionFlowCounts() {
  const [
    activeWorkOrders, cards, bomReleased, woCreated, woRoute, woReleased, woExecuting, woCompleted,
    jobCardsRaised, jobCardsDone, materialIssued, piecesCut, qcOpen, changeNotes,
  ] = await Promise.all([
    ...
    queryOne("SELECT COUNT(*) AS c FROM projects WHERE status = 'active' AND bom_release_revision > 0"),
    ...
    queryOne("SELECT COUNT(*) AS c FROM job_cards WHERE work_order_id IS NOT NULL AND status = 'done'"),
    queryOne(
  ...
  // Each stage also carries the work_orders.status filter value (where one applies) so the UI can
  // link straight into WorkOrdersPanel's existing `?status=` support (app/api/work-orders/route.js
  // already reads it) instead of a new filtered view — Job Cards/Material/QC stages link into the
  // Job Card board and BOM tab instead, both of which already partition by the same real statuses.
  counts.lifecycle = {
    bomReleased: { value: bomReleased.c, href: '/projects' },
    workOrderCreated: { value: woCreated.c, href: '/production/workers?tab=workorders&wostatus=draft' },
    route: { value: woRoute.c, href: '/production/workers?tab=workorders&wostatus=draft' },
    ...
    materialIssued: { value: materialIssued.c + piecesCut.c, href: '/production/workers?tab=bom' },
    ...
    jobCardsCompleted: { value: jobCardsDone.c, href: '/production/workers?tab=jobcards' },
    workOrderCompleted: { value: woCompleted.c, href: '/production/workers?tab=workorders&wostatus=completed' },
  };
...
export async function getSaleOrders() {
  return queryAll(
    `SELECT so.*, p.id AS project_id FROM sale_orders so
...
export async function getJobCards({ projectId, status } = {}) {
  const where = [];
  const args = [];
