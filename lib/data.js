...
export async function getProjectsWithStatus() {
  ...
  return projects.map(p => {
    ...
    // Which department(s) actually have the ball right now — raw status, not effectiveStatus,
    // so an overdue-but-active milestone still counts as "current" (effectiveStatus relabels it
    // 'overdue' and would otherwise drop its department from this list). Falls back to the next
    // not-yet-started milestone's department so a project always shows at least one pill, even
    // before anyone has actually started work on it.
    ...
    const activeDepartments = [...new Set([...activeMs, ...upcoming].map(m => m.department).filter(Boolean))];
    ...
    // plain milestone done/total — same source of truth as activeDepartments and roll, no new
    // concept invented.
    const departmentProgress = activeDepartments.map(department => {
      const deptMs = ms.filter(m => m.department === department);
      const deptDone = deptMs.filter(m => m.actual_end || m.status === 'done').length;
      return { department, done: deptDone, total: deptMs.length };
    });
    return {
      ...p,
      roll: worstStatus(ms),
