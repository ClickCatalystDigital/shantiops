...
export async function getChildDerivedBom(childProjectId) {
  ...
  const [items, rollupById, allocated, routed] = await Promise.all([
    queryAll(
...
export async function getChildRoutingBoard(masterProjectId) {
  const [children, lines, rollupById, allocRows, routingRows] = await Promise.all([
  ...
  const routingKey = (bomItemId, childId) => `${bomItemId}:${childId}`;
  const routingMap = new Map(routingRows.map(r => [routingKey(r.bom_item_id, r.child_project_id), r.routed_to]));
  ...
  const cells = allocRows.map(r => {
    ...
    return {
      ...
      routed_to: routingMap.get(routingKey(r.bom_item_id, r.child_project_id)) ?? null,
    };
  });

  return { children, lines, cells };
}
