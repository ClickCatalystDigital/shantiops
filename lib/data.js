...
export async function getProjectBom(projectId) {
  ...
  // no child units of its own, so child_project_id === this project's own id). Display-only: does not
  // change readyForPacking below — gating that retroactively on a brand-new table would instantly
  // un-ready every already-packable line in the app that predates this feature.
...
export async function getPendingPackingItems() {
  ...
  // couldn't show the whole-BOM/per-node rollup wired everywhere else in §5be.
  return rows.map((b) => {
