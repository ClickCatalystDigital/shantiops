...
export async function getPackingDetail(id) {
  ...
  if (list.status === 'draft') {
    const bomRows = await queryAll(
      `SELECT id, qty_text, assembly_id FROM bom_items WHERE id IN (
    ...
    const rollupById = await getAssemblyRollupMap(list.project_id);
    items = items.map(it => {
      const b = it.bom_item_id ? bomById.get(it.bom_item_id) : null;
      return { ...it, qty_breakdown: b ? qtyBreakdown(b.qty_text, b.assembly_id, rollupById) : null };
    });
  }
...
export async function getSourcingItems() {
  ...
  return rows.map(r => ({ ...r, qty_breakdown: qtyBreakdown(r.qty_text, r.assembly_id, rollupById) }));
}
...
export async function getActiveProjectsList() {
  ...
  // default filter); existing callers ignore fields they don't ask for. Kept ordered by project_no
  // here for those callers — the QC workspace re-sorts latest-first client-side. Filtering by
  // release status happens only in the one caller that needs it, never here — this function must
  // keep returning the full active list for every other caller.
  return queryAll(
    "SELECT id, project_no, customer_name, series, created_at, bom_release_revision FROM projects WHERE status = 'active' ORDER BY project_no");
}
...
export async function getOpenBomItems() {
  ...
  return rows.map(r => ({
    ...r,
    qty_breakdown: qtyBreakdown(r.qty_text, r.assembly_id, rollupById),
    rolled_qty: itemRollupQty(r.qty_text, r.assembly_id, rollupById),
  }));
}
...
export async function getBomStructure(projectId) {
  const [assemblies, items, drawingCounts, weights] = await Promise.all([
    ...
    queryAll(`SELECT b.id, b.assembly_id, b.material_description, b.qty_text, b.item_id, it.item_code AS catalog_item_code,
  ...
  for (const it of items) {
    ...
    itemsByAssembly.get(it.assembly_id).push({
      ...it, rolled_qty: itemRollupQty(it.qty_text, it.assembly_id, byId),
