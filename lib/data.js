...
export async function getChildDerivedBom(childProjectId) {
  ...
  const [items, rollupById] = await Promise.all([
    queryAll(
  ...
  const items_out = items.map(it => ({
    ...it,
    per_unit_qty: itemRollupQty(it.qty_text, it.assembly_id, rollupById, 1, !!it.qty_resolved),
  }));
...
export async function getProjectAllocationSummary(projectId) {
  const [items, received, allocated] = await Promise.all([
    queryAll(
      `SELECT id, material_description, moc, size_spec, qty_text
  ...
  return items.map(it => {
    ...
    return { ...it, received: receivedTotal, allocated: allocatedTotal, available: receivedTotal - allocatedTotal };
  });
}
