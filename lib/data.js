...
export async function getSalesProducts() {
  return queryAll(`SELECT id, product_code, legacy_code, product_name, product_type, category, description, price, cost_price,
      unit, hsn_code, gst_pct, warranty_days, serviceable, attributes_json, active
...
export async function getSalePayments() {
  return queryAll(`
    SELECT p.*, so.so_no, so.customer_name, so.total AS order_value, COALESCE(si.invoice_no, p.invoice_ref) AS invoice_no
