...
export async function linkBomItem(bomItemId, itemId, username, { learn = true } = {}) {
  ...
  let catalogRow = null;
  if (newItemId != null) {
    catalogRow = await queryOne('SELECT id, item_name, uom FROM items WHERE id = ?', [newItemId]);
  ...
  await execute('UPDATE bom_items SET item_id = ?, needs_spec = NULL, qty_text = ? WHERE id = ?', [newItemId, unitFilled ? filled : bomItem.qty_text, bomItemId]);
