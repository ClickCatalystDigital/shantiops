...
import { notifyDepartment, notifyDepartmentHeads } from './notify';
...
import { receivePiece } from './stock-pieces';
...
export async function creditBomItemReceipt(tx, item, { qty, isFullyReceived, totalReceived, receiptId, lotLabel = null, changed, username }) {
  const receiptIns = await tx.execute({
  ...
  const bomItemReceiptId = Number(receiptIns.lastInsertRowid);

  // Inward QC/Production Approval Workflow — one row per physical delivery, created in the same
  // transaction as the receipt it reviews, so every entry point that credits a receipt always
  // produces one, with no bypass. QC's own review determines whether this delivery's material
  // becomes usable Stores inventory (lib/stock-pieces.js/maybeCreatePieceStock/
  // maybeReserveScalarStock hold it until approved) — never gates the receipt event itself.
  const approvalIns = await tx.execute({
    sql: `INSERT INTO inward_approvals (bom_item_receipt_id, bom_item_id, project_id)
          VALUES (?, ?, ?)`,
    args: [bomItemReceiptId, item.id, item.project_id],
  });
  const inwardApprovalId = Number(approvalIns.lastInsertRowid);

  if (!isFullyReceived) return { isFullyReceived: false, bomItemReceiptId, inwardApprovalId };
  ...
  return { isFullyReceived: true, grnRef, cumulativeText, bomItemReceiptId, inwardApprovalId };
}
...
export async function maybeCreatePieceStock(item, qty, receiptId, changed, username, bomItemReceiptId) {
  ...
  for (let i = 0; i < unitCount; i++) {
    try {
      // Inward QC/Production Approval Workflow — the piece is received HELD (pending_qc_inward),
      // linked to the delivery event QC will review, instead of the pre-existing "receive then
      // immediately auto-reserve against its own bom_item" behavior. Approving the delivery
      // (releasePieceFromInwardHold, lib/stock-pieces.js) is what finishes that same link — QC's
      // decision determines whether the material ever becomes usable/reservable stock at all.
      await receivePiece({
        inventoryItemId,
        kind: dims.kind,
...
export async function maybeReserveScalarStock(item, qty, username, inwardApprovalId) {
  if (DIMENSIONAL_CATEGORIES.includes(item.category)) return; // Phase 2's territory
  // 'sas' (Sales trade requests) reserves the identical way 'bom' does — the sas bom_item itself
  // already carries the real demand context (sale_order_no), so reserving against it is "reserve to
  // the sale order," no separate mechanism needed. Real gap found in Phase 2's E2E investigation
  // (2026-09-17): before this, a freshly-procured sas item was silently neither credited nor
  // reserved on receipt — the material physically arrived but the system lost track of it.
  if (!['bom', 'sas'].includes(item.source) || !item.item_id) return;
  ...
  // Inward QC/Production Approval Workflow — held, not credited. The pre-existing "credit on_hand
  // then immediately reserve" behavior now only happens once QC approves this delivery
  // (releaseScalarFromInwardHold below); until then the quantity sits on the inward_approvals row
  // itself, never touching inventory_items.on_hand at all. Best-effort, same established pattern as
  // maybeCreatePieceStock — a receipt already durably recorded in bom_item_receipts never rolls
  // back for this.
  try {
    await execute(
      'UPDATE inward_approvals SET inventory_item_id = ?, qty_scalar = ? WHERE id = ?',
      [inventoryItemId, qty, inwardApprovalId]
    );
  } catch (err) { /* best-effort — a receipt already durably recorded never rolls back for this */ }
}

// Inward QC/Production Approval Workflow — the scalar counterpart to releasePieceFromInwardHold,
// finishing exactly what maybeReserveScalarStock would have done at receipt time had the quantity
// not been held: credit inventory_items.on_hand, then reserve that same quantity against the very
// bom_item it was received for. reserveFromStock's own TOCTOU fix re-checks availability fresh at
// reserve time, so a lost race here just means the reservation covers less than qty, never an
// overcommit — the same tolerance every other receiving side effect in this file already accepts.
export async function releaseScalarFromInwardHold(inwardApprovalId) {
  const approval = await queryOne(
    'SELECT bom_item_id, inventory_item_id, qty_scalar FROM inward_approvals WHERE id = ?', [inwardApprovalId]);
  if (!approval?.inventory_item_id || !(approval.qty_scalar > 0)) return; // dimensional/piece case — nothing scalar to release
  await execute('UPDATE inventory_items SET on_hand = on_hand + ? WHERE id = ?', [approval.qty_scalar, approval.inventory_item_id]);
  await reserveFromStock({
    inventoryItemId: approval.inventory_item_id, bomItemId: approval.bom_item_id,
    qty: approval.qty_scalar, username: 'system', allowClosedStatus: true,
  });
}

// Inward QC/Production Approval Workflow — best-effort notification that a new inward_approvals
// row needs a QC Head decision. Deliberately called AFTER the receiving transaction commits (never
// from inside creditBomItemReceipt itself), matching this file's own established convention that
// notifications are side effects, not part of the durable write. Reused by the resubmit route too.
export async function notifyInwardApprovalPending(item, inwardApprovalId) {
  try {
    await notifyDepartmentHeads('QC', {
      kind: 'inward_approval_pending', title: `Material awaiting inward review: ${item.material_description}`,
      project_id: item.project_id, dedupe_key: `inward_approval:${inwardApprovalId}`,
    });
