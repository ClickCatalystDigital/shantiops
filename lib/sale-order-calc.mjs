...
// Discount can be given as a % or as an amount (Sales CRM plan 1h): a non-null `discountAmount`
// wins and the % is derived from it; it is clamped to [0, subtotal] so tax never goes negative.
export function computeSaleOrderTotals({ items, discountPct = 0, discountAmount: discountAmountIn = null, packingForwarding = 0, insurance = 0, freight = 0, other = 0, companyStateCode, customerStateCode }) {
  const subtotal = round2(items.reduce((s, it) => s + (Number(it.amount) || 0), 0));
  const useAmount = discountAmountIn !== null && discountAmountIn !== undefined && discountAmountIn !== '';
  const discountAmount = useAmount
    ? round2(Math.min(Math.max(Number(discountAmountIn) || 0, 0), subtotal))
    : round2(subtotal * (discountPct || 0) / 100);
  const effectiveDiscountPct = useAmount ? (subtotal > 0 ? round2(discountAmount / subtotal * 100) : 0) : (Number(discountPct) || 0);
  ...
  return { subtotal, discountAmount, discountPct: effectiveDiscountPct, netAfterDiscount, cgst, sgst, igst, taxAmount: round2(cgst + sgst + igst), total };
}
