...
import {
  ...
  isFullyReservedFromStock, aggregatePrGroups, procurementCategory, humanizeAssemblyName,
...
function selfcheck() {
  ...
  // --- aggregatePrGroups by item_id: same item across projects; rows with no item_id drop out ---
  const byItem = aggregatePrGroups([
    { id: 1, item_id: 9, material_description: 'MS ANGLE', qty_text: '4 Nos', reserved_qty: 0, qty_breakdown: null, project_id: 1 },
    { id: 2, item_id: 9, material_description: 'MS ANGLE', qty_text: '6 Nos', reserved_qty: 0, qty_breakdown: null, project_id: 2 },
    { id: 3, item_id: null, material_description: 'free text', qty_text: '1 No', reserved_qty: 0, qty_breakdown: null, project_id: 3 },
  ], 'item_id');
  assert.strictEqual(byItem.length, 1);
  assert.strictEqual(byItem[0].total_qty, 10);
  assert.strictEqual(byItem[0].kind, 'item');
  assert.deepStrictEqual(byItem[0].sourcing_bom_item_ids, [1, 2]);
  assert.strictEqual(procurementCategory({ requires_manufacturing: 0 }), 'bought_out');
  assert.strictEqual(procurementCategory({ requires_manufacturing: 1, category: 'standard' }), 'bought_out');
  assert.strictEqual(procurementCategory({ requires_manufacturing: 1, category: 'plate' }), 'fabrication');
  ...
  // Node Configuration (datasheet fields) rides through a template — but ONLY when present, so a node with
  // none serializes exactly as before (an existing template re-saved must stay byte-identical, otherwise
  // it would get a spurious version bump).
  const plainTree = JSON.stringify(buildTemplateTree(
    [{ id: 90, name: 'FD FAN', node_type: 'Subsystem', qty: 1, config_json: null }], new Map(), new Map()));
  assert.ok(!plainTree.includes('config'), 'no config key at all when the node has none');
  assert.strictEqual(plainTree, JSON.stringify(buildTemplateTree(
    [{ id: 90, name: 'FD FAN', node_type: 'Subsystem', qty: 1 }], new Map(), new Map())), 'undefined and NULL config serialize identically');
  const cfgTree = buildTemplateTree(
    [{ id: 91, name: 'FD FAN', node_type: 'Subsystem', qty: 1, config_json: JSON.stringify([{ label: 'TYPE', value: 'CENTRIFUGAL' }, { label: 'FLOW cfm', value: '' }]) }],
    new Map(), new Map());
  assert.deepStrictEqual(cfgTree[0].config, [{ label: 'TYPE', value: 'CENTRIFUGAL', unit: '' }, { label: 'FLOW', value: '', unit: 'cfm' }], 'config captured in order, blank values kept, unit split from a legacy label');
  assert.deepStrictEqual(flattenTemplateTree(cfgTree)[0].config, cfgTree[0].config, 'flatten carries config to the apply step');
  assert.deepStrictEqual(flattenTemplateTree(JSON.parse(plainTree.replace('[{', '[{'))) [0].config, [], 'an old template without config flattens to an empty list');
  assert.deepStrictEqual(buildTemplateTree([{ id: 92, name: 'X', node_type: 'System', qty: 1, config_json: 'garbage' }], new Map(), new Map())[0].config, undefined,
    'corrupt stored JSON is ignored, never throws');
  assert.strictEqual(computeTemplateCounts(cfgTree).itemCount, 0, 'configuration rows are not counted as items');
