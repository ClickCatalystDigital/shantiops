...
async function insertEntryWithLines({ company, entryDate, sourceType, sourceId, description, lines, createdBy, status, reversalOfId }) {
  ...
  if (status === 'posted') await assertPeriodOpen(company, entryDate); // drafts don't touch the ledger, so they're exempt
  ...
  const total = round2(lines.reduce((s, l) => s + (l.debit || 0), 0));
  const action = reversalOfId ? 'journal_entry_reversed' : status === 'draft' ? 'journal_entry_draft_created' : 'journal_entry_auto_posted';
  await audit(action, { actor: createdBy, detail: `${sourceType} #${sourceId} — ${company}, ${total}` });
  return entryId;
}
...
export async function postDraftJournalEntry(entryId) {
  const entry = await queryOne('SELECT company, entry_date FROM journal_entries WHERE id = ?', [entryId]);
  await assertPeriodOpen(entry.company, entry.entry_date);
