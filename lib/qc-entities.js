...
// Default covering-letter recipient (a folder can override to the customer instead). `address` is
// a real array of lines, not one run-on string — matches how the government office's own mailing
// block actually breaks across lines on a real filed cover letter, which a single comma-joined
// string can't reproduce (the real breaks don't line up with the commas). Sole consumer is
// CoveringLetterPage (lib/qc-folder-pdf.js) — confirmed via a repo-wide grep before changing this
// shape, so nothing else in the app is affected.
export const DIRECTOR_OF_BOILERS = {
  name: 'The Director of Boilers',
  address: [
    'H.No: 2-2-647/182/A, 3rd Floor,',
    'Azam Complex, Shivam Road,',
    'Bagh Amberpet,',
    'Hyderabad - 13,',
    'Telangana.',
  ],
