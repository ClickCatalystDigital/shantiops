...
async function migrate(client) {
  ...
  await client.execute(`CREATE TABLE IF NOT EXISTS vendor_tds_rates (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    section TEXT NOT NULL, -- 194C, 194J, ...
    description TEXT,
    rate_pct REAL NOT NULL,
  ...
  if (!hasVendorTdsRates.rows.length) {
    ...
    const tdsSeed = [
      ['194C', 'Payments to contractors (individual/HUF)', 1, 30000, '2026-04-01'],
      ['194C', 'Payments to contractors (others)', 2, 30000, '2026-04-01'],
      ['194J', 'Professional/technical fees', 10, 30000, '2026-04-01'],
    ];
    for (const [section, description, rate_pct, threshold_amount, effective_from] of tdsSeed) {
      await client.execute({
        sql: `INSERT INTO vendor_tds_rates (section, description, rate_pct, threshold_amount, effective_from) VALUES (?, ?, ?, ?, ?)`,
        args: [section, description, rate_pct, threshold_amount, effective_from],
      });
    }
  }
  ...
  const hasChartOfAccounts = await client.execute('SELECT 1 FROM chart_of_accounts LIMIT 1');
  if (!hasChartOfAccounts.rows.length) {
    ...
    for (const { company } of companyRows.rows) {
      for (const [code, name, account_type] of DEFAULT_CHART_OF_ACCOUNTS) {
        await client.execute({
          sql: 'INSERT INTO chart_of_accounts (company, code, name, account_type) VALUES (?, ?, ?, ?)',
