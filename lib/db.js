...
async function backfillDependsOnKey(client) {
  const pending = await client.execute("SELECT COUNT(*) AS c FROM milestones WHERE depends_on_key IS NULL");
  if (!pending.rows[0]?.c) return;
  const rows = (await client.execute(
    'SELECT id, project_id, milestone_key FROM milestones ORDER BY project_id, sort_order, id'
  )).rows;
  const byProject = {};
  for (const r of rows) (byProject[r.project_id] ||= []).push(r);
  for (const ms of Object.values(byProject)) {
    for (let i = 1; i < ms.length; i++) {
      await client.execute({
        sql: 'UPDATE milestones SET depends_on_key = ? WHERE id = ? AND depends_on_key IS NULL',
        args: [ms[i - 1].milestone_key, ms[i].id],
      });
    }
  }
}
...
async function migrate(client) {
  ...
  // now. New projects get it seeded directly in createProjectMilestones below; this backfills
  // existing rows once (previous row in that project's sort_order = structural predecessor, same
  // order nextBySortOrder/handoffTarget already walk). No-op after the first run.
  await backfillDependsOnKey(client);
  ...
  for (let i = 0; i < N; i++) {
    ...
    const inserted = await client.execute({
      ...
      args: [projectId, m.key, m.label, i, assignee, m.department,
        ...
        i > 0 ? MILESTONE_TEMPLATE[i - 1].key : null]
    });
