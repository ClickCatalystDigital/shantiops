...
async function migrate(client) {
  ...
  // grows from real usage rather than a hardcoded array or a dedicated template-editor screen: the
  // first stage typed against a (department, milestone_key) becomes that type's default, so the
  // next milestone of the same type — this project's or any other's — can "Apply template" instead
  // of retyping. milestone_stages is the actual per-milestone instance list; editing it (add/
  // remove) never writes back to the template, matching MILESTONE_TEMPLATE's own precedent (a
  // reusable definition copied onto real rows, edited independently after copying).
  await client.execute(`CREATE TABLE IF NOT EXISTS stage_templates (
    ...
    label TEXT NOT NULL,
    sort_order INTEGER NOT NULL DEFAULT 0,
    UNIQUE(department, milestone_key, label)
  )`);
