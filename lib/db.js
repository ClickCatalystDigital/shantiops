...
async function migrate(client) {
  ...
  if (!hasChartOfAccounts.rows.length) {
    ...
    // Codes added to DEFAULT_CHART_OF_ACCOUNTS after the above one-time seed (Fixed Assets,
    // Accumulated Depreciation, Depreciation Expense) need their own backfill per existing company —
    // the block above only ever runs once, on a completely empty table.
    const companyRows = await client.execute('SELECT company FROM company_settings');
    const existing = await client.execute('SELECT company, code FROM chart_of_accounts');
    const have = new Set(existing.rows.map(r => `${r.company} ${r.code}`));
    for (const { company } of companyRows.rows) {
      for (const [code, name, account_type] of DEFAULT_CHART_OF_ACCOUNTS) {
        if (have.has(`${company} ${code}`)) continue;
        await client.execute({
          sql: 'INSERT INTO chart_of_accounts (company, code, name, account_type) VALUES (?, ?, ?, ?)',
          args: [company, code, name, account_type],
        });
