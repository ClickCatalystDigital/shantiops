...
import { inferCategory } from './section-shapes.js';
...
async function migrate(rawClient) {
  deferredColumns = [];
  const client = withDeferredColumnReplay(rawClient);
  ...
  // Certificate linking for bought-out items — the same capability qc_document_parts already has
  // for Form IV A material, mirrored here rather than left as a Form-IV-A-only feature. A bought-out
  // mounting (a valve, a plate/tube used as a fitting) can carry a real MTC too.
  await addColumn(client, 'qc_mountings', 'test_certificate_id INTEGER REFERENCES test_certificates(id)');
  ...
  // Form IV A's own manual lettered-section grouping — replaces the earlier BOM-tree auto-derivation
  // (lib/qc-form4a-sections.mjs), which regularly left a meaningless "Ungrouped Materials" catch-all
  // and sometimes labeled a section with a BOM-tree node name that wasn't real government-form
  // wording (e.g. "Boiler Shell & Body"). Purely a QC-driven tag on top of the existing parts list —
  // no BOM-assembly matching, no auto-sync, unlike qc_iiia_groups. A document with zero rows here
  // renders Form IV A as one flat table with no section heading at all.
  await client.execute(`CREATE TABLE IF NOT EXISTS qc_form4a_groups (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    document_id INTEGER NOT NULL REFERENCES qc_documents(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    sort_order INTEGER NOT NULL DEFAULT 0
  )`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_qc_form4a_groups_document ON qc_form4a_groups(document_id)`);
  // NULL -> flat/no section (default). A part can never be in both an iiia_group AND a form4a_group
  // at once in practice (an iiia_group_id part is already excluded from Form IV A entirely before
  // this ever matters), but nothing enforces that at the DB level — same trust level as iiia_group_id.
  await addColumn(client, 'qc_document_parts', 'form4a_group_id INTEGER REFERENCES qc_form4a_groups(id)');
  ...
  // Legacy CRM import (2026-09-25): A/C manager(s) as the old CRM named them, products the party
  // asked about, the old CRM's per-organization summary (calls by stage, quote/order/collection
  // totals) as JSON, and an import tag for rollback (NULL = created in this app).
  await addColumn(client, 'customers', 'account_manager TEXT');
  await addColumn(client, 'customers', 'products_of_interest TEXT');
  await addColumn(client, 'customers', 'legacy_crm_json TEXT');
  await addColumn(client, 'customers', 'source TEXT');
  ...
  await client.execute(`CREATE TABLE IF NOT EXISTS opportunities (
    ...
    customer_name TEXT,
    title TEXT NOT NULL,
    stage TEXT NOT NULL DEFAULT 'Lead',       -- validated against sales_stages.name (DB-configurable
      -- funnel — 9 values as of Sales CRM expansion Phase 0c, see migrateSalesStagesTo9ValueFunnel).
      -- The 'Lead' literal above is this column's own SQL DEFAULT, frozen at table-creation time on
      -- an already-existing DB (SQLite can't ALTER a column's DEFAULT) — every real INSERT path
      -- explicitly supplies a real, current stage name instead of relying on it; see
      -- app/api/opportunities/route.js and app/api/leads/[id]/convert/route.js.
    value_num REAL,
    probability INTEGER,
    expected_close DATE,
  ...
  // (The two addColumn calls live right after CREATE TABLE crm_notes below — here they ran before
  // the table existed and a brand-new database failed to boot.)
  ...
  // Sales Enquiry form fields — Enquiry and Leads share this one table (a raw enquiry IS a
  // status='new' lead, SYSTEM.md §5e), but Enquiry's own creation form captures more than Leads'
  // plain name/company/phone. Plain nullable columns, same addColumn convention as every field
  // above. `territory` (State) and `industry` (Segment) are reused as-is rather than duplicated.
  await addColumn(client, 'leads', 'enquiry_date DATE');
  await addColumn(client, 'leads', 'address TEXT');
  await addColumn(client, 'leads', 'website TEXT');
  await addColumn(client, 'leads', 'product TEXT');
  await addColumn(client, 'leads', 'reference TEXT');
  await addColumn(client, 'leads', 'short_name TEXT');
  await addColumn(client, 'leads', 'district TEXT');
  await addColumn(client, 'leads', 'sub_location TEXT');
  await addColumn(client, 'leads', 'telephone TEXT');
  await addColumn(client, 'leads', 'order_expected_in TEXT');
  await addColumn(client, 'leads', 'week_number TEXT');
  await addColumn(client, 'leads', 'account_manager TEXT');
  await addColumn(client, 'leads', 'initiated_by TEXT');
  await addColumn(client, 'leads', 'district_code TEXT');
  await addColumn(client, 'leads', 'pin_code TEXT');
  ...
  await addColumn(client, 'crm_notes', 'call_type TEXT'); // incoming | outgoing (Call Log, above)
  await addColumn(client, 'crm_notes', 'duration_seconds INTEGER');

  // Sales CRM expansion Phase 1 — Diary. Extends crm_notes (not a new entity) with the legacy
  // "Update Sales Call Section" form's full field set. next_plan_date is the single field every
  // follow-up query (Home calendar, Neglected Sales Call report) reads — its own index matters
  // more than any other column added here. "last visit date" (from the checklist) is deliberately
  // NOT a column — it's MAX(visit_date) per lead, always correct, computed at read time.
  await addColumn(client, 'crm_notes', 'visit_date DATE');
  await addColumn(client, 'crm_notes', 'action_taken TEXT');
  await addColumn(client, 'crm_notes', 'is_value_addition INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'crm_notes', 'in_time TEXT');
  await addColumn(client, 'crm_notes', 'out_time TEXT');
  await addColumn(client, 'crm_notes', 'plan_date DATE');
  await addColumn(client, 'crm_notes', 'plan_time TEXT');
  await addColumn(client, 'crm_notes', 'plan_for TEXT');
  await addColumn(client, 'crm_notes', 'plan_of_action TEXT');
  await addColumn(client, 'crm_notes', 'next_plan_date DATE');
  await addColumn(client, 'crm_notes', 'alert_mode TEXT'); // captured only — no SMS gateway exists
  await addColumn(client, 'crm_notes', 'send_alert_sms TEXT'); // captured only — no SMS gateway exists
  await addColumn(client, 'crm_notes', 'contact_id INTEGER REFERENCES contacts(id) ON DELETE SET NULL');
  await addColumn(client, 'crm_notes', 'product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL');
  await addColumn(client, 'crm_notes', 'location TEXT');
  await addColumn(client, 'crm_notes', 'feedback_responded INTEGER'); // NULL = not feedback-related
  await addColumn(client, 'crm_notes', 'plan_note_type TEXT'); // Sales CRM plan 1j — Action Type of the planned follow-up
  await addColumn(client, 'sales_stages', 'probability_pct INTEGER'); // Sales CRM plan 3d — win % per stage (NULL = default rule, lib/lead-stage.mjs)
  await addColumn(client, 'crm_notes', 'alert_users TEXT'); // Sales CRM plan 2c — usernames alerted when alert_mode = 'Selected seniors' (CSV)
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_crm_notes_next_plan ON crm_notes(next_plan_date)`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_crm_notes_visit_date ON crm_notes(visit_date)`);

  await client.execute(`CREATE TABLE IF NOT EXISTS crm_note_files (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    note_id INTEGER NOT NULL REFERENCES crm_notes(id) ON DELETE CASCADE,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_crm_note_files_note ON crm_note_files(note_id)`);
  ...
  await client.execute(`CREATE TABLE IF NOT EXISTS price_lists (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    customer_id INTEGER REFERENCES customers(id),
    item_id INTEGER REFERENCES items(id), -- Sales CRM plan 4: nullable; Sales price lists key to product_id
    product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL,
    rate REAL NOT NULL,
    uom TEXT,
    valid_from DATE,
    valid_until DATE,
  ...
  await rekeyPriceListsToProducts(client);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_price_lists_item ON price_lists(item_id)`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_price_lists_product ON price_lists(product_id)`);
  ...
  // Order Acknowledgement / Scope of Supply source PDF attachment — same single-file-per-record
  // pattern as test_certificates.pdf_key/pdf_url (R2, lib/r2.js), not a multi-file table like
  // calc_drawing_files: a Sale Order has exactly one such source document at a time, replaced not
  // accumulated.
  await addColumn(client, 'sale_orders', 'pdf_key TEXT');
  await addColumn(client, 'sale_orders', 'pdf_url TEXT');
  ...
  // Sales Payment Tracker (2026-09-19) — Sales' own lightweight log of money received per Sale
  // Order (advance payments happen before any invoice exists, so it hangs off the order, with an
  // optional invoice pointer). Deliberately NOT the ledger: customer_receipts (§5w) is Accounts'
  // GL-posting settlement against an invoice; this never posts anything.
  await client.execute(`CREATE TABLE IF NOT EXISTS sale_order_payments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    sale_order_id INTEGER NOT NULL REFERENCES sale_orders(id),
    sales_invoice_id INTEGER REFERENCES sales_invoices(id),
    invoice_ref TEXT,                -- free-text invoice number, for history that predates / never had a sales_invoices row
    received_on DATE,                -- NULL = money received but the date was never recorded (legacy tracker rows)
    mode TEXT,
    amount REAL NOT NULL,
    remark TEXT,
    created_by TEXT,
  ...
  await addColumn(client, 'sale_order_payments', 'invoice_ref TEXT');
  // Order IDs must be unique (the Payment Tracker imports/matches by them). Guarded: never let a
  // legacy duplicate block boot — the index just isn't created until the duplicate is fixed.
  try { await client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_sale_orders_so_no ON sale_orders(so_no)`); } catch {}
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sale_order_payments_so ON sale_order_payments(sale_order_id)`);
  await addColumn(client, 'sale_orders', 'remarks TEXT');
  // Payment Tracker's own inline-editable fields. track_status is deliberately NOT the existing
  // `status` (open|fulfilled|cancelled — read by the Sales flow diagram and the payment route's
  // cancelled guard); the others are overrides that fall back to the derived value when NULL.
  await addColumn(client, 'sale_orders', "track_status TEXT NOT NULL DEFAULT 'Pending'"); // Pending|Ready|WIP|Dispatched|Closed
  await addColumn(client, 'sale_orders', 'order_date DATE');
  await addColumn(client, 'sale_orders', 'invoice_ref TEXT');
  // Payment Tracker Bill Value when the order has no issued Sales Invoice yet (lib/order-match.mjs).
  await addColumn(client, 'sale_orders', 'bill_value REAL');
  await addColumn(client, 'sale_orders', 'sales_person_override TEXT');
  for (const c of ['advance', 'dispatched', 'site_completed', 'commissioning', 'pending_issue', 'cleared_issue']) {
    await addColumn(client, 'sale_orders', `stage_${c} INTEGER NOT NULL DEFAULT 0`);
  }

  // ============================================================================================
  // Sales CRM expansion (2026-09-22) — Product Master, Branches, a real 9-value funnel, a Diary
  // follow-up discipline, Commercial Offer email drafting, a PO/Sale-Order wizard, and reports.
  // Phase 0 — foundational masters everything else depends on. Full plan:
  // ~/.claude/plans/now-for-the-product-piped-quail.md.
  // ============================================================================================

  // Phase 0a — real, Sales/CRM-scoped Branches (no company-wide reach, no GST-per-branch,
  // confirmed with the user). Region is a free-text grouping field on the same row, not a
  // separate Regions table (a handful of branches, casing drift is a non-issue at this scale).
  await client.execute(`CREATE TABLE IF NOT EXISTS branches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE,
    region TEXT,
    active INTEGER NOT NULL DEFAULT 1,
  ...
  await addColumn(client, 'leads', 'branch_id INTEGER REFERENCES branches(id) ON DELETE SET NULL');
  await addColumn(client, 'sale_orders', 'branch_id INTEGER REFERENCES branches(id) ON DELETE SET NULL');
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_leads_branch ON leads(branch_id)`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sale_orders_branch ON sale_orders(branch_id)`);

  // Phase 0b — the Product Master. Deliberately its own small table, not folded into the
  // raw-material `items` Item Master — same "don't force-unify near-identical small catalogs per
  // line-shape" precedent ItemSearchField vs QuotationItemField already set.
  await client.execute(`CREATE TABLE IF NOT EXISTS sales_products (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sales_products_type ON sales_products(product_type)`);

  // Phase 0c — reseed sales_stages from the old 5-value pipeline (Lead/Qualified/Quoted/Won/Lost)
  // to the user's real 9-value Sales Call funnel, used as one source of truth everywhere: the
  // Opportunities Kanban, leads.sales_call_status, and the Funnel Report. Renames rows IN PLACE
  // (never delete+insert) so no existing opportunities.stage value (a plain TEXT match, not an FK
  // id) goes orphaned.
  await migrateSalesStagesTo9ValueFunnel(client);

  // Phase 0d — leads gets the fields the PO wizard/Diary/Reports actually read.
  // sales_call_status is THE one field the checklist calls "Status"/"Order Stage"/"Funnel Stage"
  // for a Lead — validated against sales_stages.name, same non-FK idiom as opportunities.stage.
  await addColumn(client, 'leads', 'is_vip INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'leads', 'lost_reason TEXT');
  await addColumn(client, 'leads', 'sales_call_status TEXT');
  await addColumn(client, 'leads', 'sales_call_closed_at DATETIME');
  await addColumn(client, 'leads', 'sales_call_closed_by TEXT');
  await addColumn(client, 'leads', 'product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL');
  await addColumn(client, 'leads', 'expected_order_date DATE');
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_leads_sales_call_status ON leads(sales_call_status)`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_leads_closed ON leads(sales_call_closed_at)`);
  // Default every existing + future Enquiry to the funnel's own starting stage.
  await client.execute("UPDATE leads SET sales_call_status = 'Lead - Cold' WHERE sales_call_status IS NULL");

  // Sales CRM plan 1a/1c (docs/sales-crm-plan.md) — the funnel stage is the only lead status.
  // Every stage change goes through lib/crm.js setLeadStage(), which also writes one row here, so
  // time-in-stage / win-rate reports have real history instead of only the current stage.
  await client.execute(`CREATE TABLE IF NOT EXISTS lead_stage_history (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_lead_stage_history_lead ON lead_stage_history(lead_id, changed_at)`);
  await migrateLeadStatusFromStage(client);
  await migrateOpportunityStageNames(client);
  // Sales CRM plan 1b — the enquiry is the deal: it carries its own expected value, and quotations /
  // sale orders raised from it link back to it (the Opportunity record is no longer used by Sales).
  await addColumn(client, 'leads', 'expected_value REAL');
  // Set only on rows loaded by scripts/import-enquiries.mjs (old-CRM sales call list), so the import
  // can be rolled back by tag. NULL for every enquiry entered in the app.
  await addColumn(client, 'leads', 'import_tag TEXT');
  await addColumn(client, 'quotations', 'lead_id INTEGER REFERENCES leads(id) ON DELETE SET NULL');
  await addColumn(client, 'sale_orders', 'lead_id INTEGER REFERENCES leads(id) ON DELETE SET NULL');
  // Sales CRM plan 1d — Product Master skeleton: what the company SELLS (the Item Master is what
  // products are built from). Unit/HSN/GST % feed quotation and PO lines. bom_structure_template_id
  // is schema only for now — the future "this product is built from this BOM template" link.
  await addColumn(client, 'sales_products', 'unit TEXT');
  await addColumn(client, 'sales_products', 'hsn_code TEXT');
  await addColumn(client, 'sales_products', 'gst_pct REAL');
  await addColumn(client, 'sales_products', 'bom_structure_template_id INTEGER REFERENCES bom_structure_templates(id) ON DELETE SET NULL');
  // Legacy CRM import (2026-09-25): category (Standard/Premium), cost price, warranty days (feeds the
  // PO wizard's Warranty Std), serviceable flag, the old CRM's own product code (not unique there —
  // 23 codes are shared by different products), and the rest of its Y/N flags as JSON.
  await addColumn(client, 'sales_products', 'category TEXT');
  await addColumn(client, 'sales_products', 'cost_price REAL');
  await addColumn(client, 'sales_products', 'warranty_days INTEGER');
  await addColumn(client, 'sales_products', 'serviceable INTEGER');
  await addColumn(client, 'sales_products', 'legacy_code TEXT');
  await addColumn(client, 'sales_products', 'attributes_json TEXT');
  // Sales CRM plan 1e — an enquiry can ask for several products. The first line is still mirrored
  // into leads.product_id/product (lib/crm.js replaceLeadProducts) so older reports keep working.
  await client.execute(`CREATE TABLE IF NOT EXISTS lead_products (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lead_id INTEGER NOT NULL REFERENCES leads(id) ON DELETE CASCADE,
    product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL,
    description TEXT NOT NULL,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_lead_products_lead ON lead_products(lead_id, sort_order)`);
  await backfillLeadProducts(client);
  // 1f/1g — quotation lines come from the Product Master and carry their own GST %; the quotation
  // stores its CGST/SGST/IGST split (lib/sales-lines.mjs quotationTotals). Sale Order lines keep the
  // quotation line's discount.
  await addColumn(client, 'quotation_items', 'product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL');
  await addColumn(client, 'quotation_items', 'gst_pct REAL');
  await addColumn(client, 'quotations', 'cgst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'quotations', 'sgst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'quotations', 'igst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_order_items', 'discount_pct REAL NOT NULL DEFAULT 0');

  // Phase 0e — Sales Targets, so the Prospect Summary Report's Targets/T/A columns are real
  // numbers, not stubs. Monthly grain ('YYYY-MM') — a quarter/year view is just summed months.
  await client.execute(`CREATE TABLE IF NOT EXISTS sales_targets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(branch_id, account_manager, period)
  )`);
  // Deliberate second index, not redundant with the UNIQUE above: that compound index is ordered
  // (branch_id, account_manager, period), so it can't efficiently answer "every target for period
  // X" — the report's own real query needs period as its own leading column.
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sales_targets_period ON sales_targets(period)`);
  // Head-only by default — letting any team member set/edit their own numeric target would defeat
  // the report's whole point. Same seeded-requires_head precedent as engineering.ecn.approve etc.
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('Sales', 'sales.target.write', 1)");

  // Sales CRM plan 4 — quotation revisions (R1, R2 … chain back to the first), Head approval above
  // a discount %, competitors seen on an enquiry/customer.
  await addColumn(client, 'quotations', 'parent_quotation_id INTEGER REFERENCES quotations(id) ON DELETE SET NULL');
  await addColumn(client, 'quotations', 'revision_no INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'quotations', 'max_discount_pct REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'quotations', 'approval_status TEXT'); // NULL = not needed | pending | approved
  await addColumn(client, 'quotations', 'approved_by TEXT');
  await addColumn(client, 'quotations', 'approved_at DATETIME');
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('Sales', 'sales.quotation.approve_discount', 1)");
  await client.execute(`CREATE TABLE IF NOT EXISTS customer_competitors (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    customer_id INTEGER REFERENCES customers(id) ON DELETE CASCADE,
    ...
    product TEXT,
    price REAL,
    lost_to INTEGER NOT NULL DEFAULT 0, -- 1 = the order was lost to this competitor
    notes TEXT,
    created_by TEXT,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_customer_competitors_customer ON customer_competitors(customer_id)');
  await client.execute('CREATE INDEX IF NOT EXISTS idx_customer_competitors_lead ON customer_competitors(lead_id)');

  // Sales CRM expansion Phase 2 — PO/Sale-Order wizard schema (2.2/2.3/2.4). GST No/Entity Code/
  // PAN No are deliberately NOT columns — derived at render time from company_settings for the
  // order's own `company`. Customer Code/PAN reuse the existing customers.party_code/pan.
  // Naming: every new money column ends `_amount`, every new boolean is `is_`-prefixed — matches
  // this codebase's own real precedent (is_won/is_lost/is_primary/is_system).
  await addColumn(client, 'sale_orders', "create_as TEXT DEFAULT 'PO'"); // 'PO' | 'RFD' | 'Approved'
  await addColumn(client, 'sale_orders', 'br_order_control_no TEXT');
  await addColumn(client, 'sale_orders', 'expected_delivery_date DATE');
  await addColumn(client, 'sale_orders', "address_type TEXT DEFAULT 'Default'"); // 'Default' | 'Other'
  await addColumn(client, 'sale_orders', 'order_address TEXT');
  await addColumn(client, 'sale_orders', 'contact_id INTEGER REFERENCES contacts(id) ON DELETE SET NULL');
  await addColumn(client, 'sale_orders', 'contact_person TEXT');
  await addColumn(client, 'sale_orders', 'contact_mobile TEXT');
  await addColumn(client, 'sale_orders', 'order_stage TEXT'); // a snapshot of leads.sales_call_status at order-creation time
  await addColumn(client, 'sale_orders', 'dispatch_comment TEXT');
  await addColumn(client, 'sale_orders', 'installation_comment TEXT');
  await addColumn(client, 'sale_orders', 'discount_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'discount_pct REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'cgst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'sgst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'igst_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'packing_forwarding_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'insurance_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'freight_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'other_charges_amount REAL NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'is_account_clear INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'advance_with_order REAL');
  await addColumn(client, 'sale_orders', 'against_installation REAL');
  await addColumn(client, 'sale_orders', 'is_form_applicable INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_orders', 'form_type TEXT');
  await addColumn(client, 'sale_orders', 'form_due_on DATE');
  await addColumn(client, 'sale_orders', 'expected_payment_mode TEXT');
  await addColumn(client, 'sale_orders', 'cheque_dd_no TEXT');
  await addColumn(client, 'sale_orders', 'payment_plan TEXT'); // 'Credit Date' | 'Create Day'
  await addColumn(client, 'sale_orders', 'payment_plan_days INTEGER');

  await addColumn(client, 'sale_order_items', 'product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL');
  await addColumn(client, 'sale_order_items', 'warranty_std_days INTEGER');
  await addColumn(client, 'sale_order_items', 'warranty_accepted_days INTEGER');
  await addColumn(client, 'sale_order_items', 'from_date_of TEXT'); // 'D' (Delivery) | 'I' (Installation)
  await addColumn(client, 'sale_order_items', 'installation_required INTEGER NOT NULL DEFAULT 0');
  await addColumn(client, 'sale_order_items', 'preventive_maintenance TEXT');
  await addColumn(client, 'sale_order_items', 'item_tax_pct REAL NOT NULL DEFAULT 0');
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sale_order_items_product ON sale_order_items(product_id)`);

  // 2.4 Attach Files — deliberately named `sale_order_files`, distinct from the existing single-
  // file `sale_orders.pdf_key`/`pdf_url` (the Order Acknowledgement source doc, replaced not
  // accumulated) — this is a real multi-file attachment list, same shape as calc_drawing_files.
  await client.execute(`CREATE TABLE IF NOT EXISTS sale_order_files (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    sale_order_id INTEGER NOT NULL REFERENCES sale_orders(id) ON DELETE CASCADE,
    file_name TEXT NOT NULL,
    file_size INTEGER,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_sale_order_files_so ON sale_order_files(sale_order_id)`);

  // Sales CRM expansion Phase 3.2 — Commercial Offer (Quotation enrichment) + email_templates.
  await addColumn(client, 'quotations', 'quotation_type TEXT');
  await addColumn(client, 'quotations', 'sent_at DATETIME');
  await addColumn(client, 'quotation_items', 'discount_pct REAL NOT NULL DEFAULT 0');
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_email_templates_company ON email_templates(company)`);
  await addColumn(client, 'quotations', 'email_template_id INTEGER REFERENCES email_templates(id) ON DELETE SET NULL');
  // Seed one placeholder template per real company, only on a genuinely empty table (idempotent —
  // never re-seeds once a real row exists, same guard shape seedV3DemoData already uses elsewhere).
  {
    const templateCount = await client.execute('SELECT COUNT(*) AS n FROM email_templates');
    if (templateCount.rows[0].n === 0) {
      for (const company of ['Shanti Boilers', 'Shanti Techno Fab']) {
        await client.execute(
          `INSERT INTO email_templates (name, company, subject, body, regards, created_by)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [`${company} — default`, company, 'Commercial Offer — {{quotation_no}}',
  ...
  // Daily sheet: what the worker was told to do (typed in the morning). "Work done" is the existing `notes`.
  await addColumn(client, 'attendance_days', 'work_allocated TEXT');
  // Extra workers on a job card stage (the first one stays job_sheet_stages.fitter_employee_id).
  await client.execute(`CREATE TABLE IF NOT EXISTS job_sheet_stage_workers (
    stage_id INTEGER NOT NULL REFERENCES job_sheet_stages(id) ON DELETE CASCADE,
    employee_id INTEGER NOT NULL REFERENCES employees(id),
    PRIMARY KEY (stage_id, employee_id)
  )`);
  ...
  // Reservation -> Material Indent Issue reconciliation fix — a reservation is the commitment ledger
  // for its own bom_item; an Issue against that bom_item must draw down THIS row before touching the
  // general pool (lib/procurement.js's consumeReservedStock). remaining = qty - qty_issued; a row
  // stays 'active' while remaining > 0 and only flips to 'issued' once fully consumed (mirrors
  // material_indent_items.qty_released's own running-counter + CASE-derived-status precedent, so no
  // new status literal is introduced). Backfilled below for any row already 'issued' before this
  // column existed, so its own remaining reads as 0 rather than its full original qty.
  await addColumn(client, 'inventory_reservations', 'qty_issued REAL NOT NULL DEFAULT 0');
  // Stores UI pass — the only signal distinguishing a Stores-clicked manual reservation from one
  // reserveFromStock() made automatically (autoReserveFromStock, remnant-match): both pass a real
  // human created_by, so that column alone can't tell them apart. Default 'manual' matches every
  // pre-existing row's real origin (autoReserveFromStock is the only automatic caller and is opted
  // in explicitly below).
  await addColumn(client, 'inventory_reservations', "source TEXT NOT NULL DEFAULT 'manual'");
  await client.execute(
    `UPDATE inventory_reservations SET qty_issued = qty WHERE status = 'issued' AND qty_issued = 0`);
  ...
  // Internal (Head) approval attribution — mirrors customer_approved_at/by exactly, so the UI can
  // show "Approved by {name}" on both sides instead of a bare status word that needs explaining.
  // Stamped by the Head's own Approve action (status -> approved); carried through unchanged if the
  // drawing later moves to as_built, since that's the same approval, not a second one.
  await addColumn(client, 'calc_drawings', 'approved_at DATETIME');
  await addColumn(client, 'calc_drawings', 'approved_by TEXT');
  ...
  // A real client's own named production-stage sequence (shell/tube boiler fabrication, from their
  // physical Job Card traveler) — additive to the generic verbs above, not a replacement. Always run
  // (INSERT OR IGNORE against the UNIQUE(name) constraint), not gated to a fresh table, so it lands
  // on every existing DB too.
  const boilerStageSeed = [
    'Shell Belt-1 Rolling', 'Shell Belt-2 Rolling', 'Tubesheet Marking', 'Furnace-A Rolling', 'RC Rolling',
    'Furnace-B Rolling', 'Access Ring Rolling', 'Shell Belt-1 & 2 Alignment', 'Furnace Stiffener Marking',
    'Furnace Stiffener Fitup', 'RC Tubesheet Fitup', 'Shell Belt-1 & 2 Welding', 'Radiographic Testing (RT)',
    'Furnace-A & RC Alignment', 'Shell Tubesheet Welding', 'Marking on Shell', 'Boxup', 'Boxup Welding',
    'Inside Welding, Tubesheet, Gussets All', 'Tubes Insertion', 'Tubes Welding', 'Saddles Fitup',
    'Nozzles Fitup', 'Endboxes Fitup', 'Nozzles Welding', 'Nozzles Inside Welding',
    'Mudhole Fitup, Manhole Cover Fitup', 'Hydraulic Test', 'Refractory', 'Grinding, Buffing', 'Painting',
    'Firebars Setup', 'Despatch',
  ];
  for (const name of boilerStageSeed) {
    await client.execute({ sql: 'INSERT OR IGNORE INTO operations (name) VALUES (?)', args: [name] });
  }
  ...
  // "Deleting" a template that BOM lines already reference (bom_items.template_id) archives it instead:
  // hidden from every list/picker, row kept so lineage and the "via <template>" label keep resolving.
  await addColumn(client, 'bom_templates', 'archived_at DATETIME');
  ...
  // Same archive-instead-of-delete idea as bom_templates.archived_at, for templates already stamped
  // on bom_assemblies.structure_template_id.
  await addColumn(client, 'bom_structure_templates', 'archived_at DATETIME');
  // Content version: +1 whenever a template's tree is replaced with different content (never on
  // name/series/description/default edits). Applied nodes remember which version built them
  // (bom_assemblies.structure_template_version, NULL = applied before versioning) so a node can say
  // "newer version available" — applied BOMs are independent copies and are never touched by an edit.
  await addColumn(client, 'bom_structure_templates', 'version INTEGER NOT NULL DEFAULT 1');
  await addColumn(client, 'bom_assemblies', 'structure_template_version INTEGER');
  // Node "Configuration": the datasheet fields of a System/Subsystem (TYPE / FLOW / STATIC HEAD / MOTOR
  // RATING ...) as a JSON array [{label, value}] — specification, not BOM items. A column, not a table, so
  // it needs no FK handling in any node-delete path and travels with the row into duplicates, templates and
  // release snapshots. NULL = none. Helpers + rules: lib/bom-config.mjs.
  await addColumn(client, 'bom_assemblies', 'config_json TEXT');
  ...
  // Names what a line still lacks ("lug type and size") — a note for Engineering, shown only while item_id is NULL; linking clears it.
  await addColumn(client, 'bom_items', 'needs_spec TEXT');
  // Item Master: what kind of purchase this item is (fabrication | bought_out | pre_dispatch); NULL = derive from requires_manufacturing/category
  await addColumn(client, 'items', 'procurement_class TEXT');

  // Multi-unit BOM split (MULTI-UNIT-SPLIT-DESIGN.md, Phase 1 — schema only, no behavior change).
  // Design/Engineering author a BOM once (unit_count set); a deliberate "Split" action later creates
  // real per-unit child projects for departments whose work is inherently per-physical-unit
  // (QC/Production/Dispatch). The ordinary single-project workflow stays the default/primary path,
  // completely unaffected — every column/table here is nullable/unused until a project is explicitly
  // split, and nothing added in this phase is read by any existing query yet.
  //
  // master_project_id: NULL on every ordinary project and on a master itself; set on a child to
  // point back at its own master. unit_no: the child's own position (1..N) — needed so lot/sort
  // logic never has to re-parse project_no strings (e.g. "SB-1109-07" -> 7).
  await addColumn(client, 'projects', 'master_project_id INTEGER REFERENCES projects(id)');
  await addColumn(client, 'projects', 'unit_no INTEGER');

  // Model spec fields (2026-09-18) — capacity/pressure/design sit alongside the existing `series`
  // ("Model Category" in the UI) rather than a new sub-table; a project has exactly one of each,
  // same shape as `series` itself. model_design is free text (a WB/SWB/DB preset in the UI, with a
  // custom escape hatch) so a future value never needs a schema change. Purely additive — nothing
  // downstream reads these yet (deliberately not fed into project_no or any generated document).
  await addColumn(client, 'projects', 'model_capacity REAL');
  await addColumn(client, 'projects', 'model_pressure REAL');
  await addColumn(client, 'projects', 'model_design TEXT');

  // bom_item_receipts — a receipt-events ledger for scalar-tracked BOM lines (Phase 4), letting
  // Stores record more than one partial delivery against the same line. Today's
  // POST /api/bom-items/[id]/receive is a single overwrite straight to purchase_status='Received'
  // regardless of whether the entered quantity actually meets what's required (confirmed live,
  // app/api/bom-items/[id]/receive/route.js) — real bug, and the reason a project's material can't
  // honestly be tracked as "180 of 500 arrived, more expected." Once wired (Phase 4),
  // purchase_status only flips to 'Received' once SUM(qty_received) across every row for a bom_item
  // meets its required quantity (itemRollupQty(), already unit_count-aware) — every one of the 29
  // existing purchase_status consumers (audited before Phase 4 touches the receive route) keeps
  // reading exactly the same meaning of 'Received' they always have; only WHEN that transition is
  // allowed to fire changes. A line still received in one shot (today's universal case, since the
  // receive dialog pre-fills the full required qty) behaves byte-for-byte identically. Traceability
  // fields are captured per-event here (not just once on bom_items) since two different partial
  // deliveries can genuinely carry two different heat/cert numbers — bom_items' own single-slot
  // received_heat_no/etc. still get stamped with the completing event's values on full receipt,
  // unchanged from today; this ledger is what keeps the earlier partial events' own values instead
  // of silently discarding them.
  await client.execute(`CREATE TABLE IF NOT EXISTS bom_item_receipts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    stock_receipts_id INTEGER REFERENCES stock_receipts(id),
    qty_received REAL NOT NULL,
    received_heat_no TEXT,
    received_mtc_no TEXT,
    received_supplier_batch_no TEXT,
    received_serial_no TEXT,
    received_by TEXT,
    received_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_item_receipts_item ON bom_item_receipts(bom_item_id)');
  // Unified delivery/lot-centric receiving — which declared lot (bom_item_expected_children, below)
  // this specific physical delivery corresponds to, stamped once at the moment of that receiving
  // event and never rewritten afterward. NULL means no lot was declared yet when this arrived — a
  // true historical fact, not an error; that portion simply falls back to the live remaining-need
  // computation for eligibility, same as any line with no reference at all. Adding a lot to the PO
  // line later never retroactively reassigns an already-stamped (or already-NULL) row.
  await addColumn(client, 'bom_item_receipts', 'lot_label TEXT');

  // bom_item_child_allocations — the optional link (Phase 4) from received material (of either
  // tracking mode — batch-tracked inventory_batches or the scalar ledger above) to a specific child
  // unit. Always keyed to the master's own bom_item_id, never duplicated per child (children never
  // get their own cloned bom_items rows, per the confirmed architecture). A receipt is never
  // required to be allocated, and allocating never creates a new procurement requirement — this is a
  // pure bookkeeping/traceability step over stock that has already arrived, not a physical-stock or
  // procurement mechanism of its own.
  await client.execute(`CREATE TABLE IF NOT EXISTS bom_item_child_allocations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    child_project_id INTEGER NOT NULL REFERENCES projects(id),
    qty_allocated REAL NOT NULL,
    allocated_by TEXT,
    allocated_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_child_alloc_item ON bom_item_child_allocations(bom_item_id)');
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_child_alloc_child ON bom_item_child_allocations(child_project_id)');

  // bom_item_child_routing — Stores' active choice, per (master bom_item, child unit), for whether
  // allocated-and-ready material goes to Production or straight to Dispatch. Current-state (upsert,
  // not append-only, unlike the allocation ledger above): a routing decision can be re-made after
  // more allocation events, and history already lives in usb_audit via audit() — a second log table
  // would duplicate that for no benefit. No row = undecided; there is deliberately no third "none"
  // value or default derived from bom_items.requires_manufacturing, since forcing an explicit Stores
  // choice (rather than a passive default) is the whole point of this table existing.
  await client.execute(`CREATE TABLE IF NOT EXISTS bom_item_child_routing (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    child_project_id INTEGER NOT NULL REFERENCES projects(id),
    routed_to TEXT NOT NULL CHECK (routed_to IN ('production','dispatch')),
    decided_by TEXT,
    decided_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (bom_item_id, child_project_id)
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_child_routing_child ON bom_item_child_routing(child_project_id)');

  // bom_item_child_certificates — architecture-review addition (multi-unit split): which test
  // certificate(s) QC has said cover a (master bom_item, child unit) cell's allocated material.
  // Same key shape as bom_item_child_routing above, extended to a true many-to-many by putting
  // certificate_id into the UNIQUE triple rather than the pair alone — one cell can carry more than
  // one certificate (e.g. two partial lots, two heats), and one certificate can cover many cells,
  // across both items and children. A human-recorded fact (QC states which cert(s) apply), never an
  // auto-matched/inferred one. Deliberate boundary: this does not record how much of a cell's
  // allocated quantity each certificate covers — no quantity/split field. Exact-quantity lot
  // genealogy is a real, bigger, deferred feature, not attempted here.
  await client.execute(`CREATE TABLE IF NOT EXISTS bom_item_child_certificates (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    child_project_id INTEGER NOT NULL REFERENCES projects(id),
    certificate_id INTEGER NOT NULL REFERENCES test_certificates(id),
    linked_by TEXT,
    linked_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (bom_item_id, child_project_id, certificate_id)
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_child_cert_child ON bom_item_child_certificates(child_project_id)');
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_child_cert_cert ON bom_item_child_certificates(certificate_id)');

  // bom_item_expected_children — unified delivery/lot-centric receiving, Phase 0: Procurement's own
  // reference for a master-project PO line, naming which child project IDs a delivery is intended
  // for. Deliberately no quantity column — this is a plain reference for Stores' benefit (narrowing
  // "which of 50 children is this for" at receiving time), never part of the supplier-facing PO
  // mechanics (rate/qty/amount). `lot_label` is never NULL, even for the common single-lot case
  // (defaults to '1') — SQLite treats every NULL as distinct from every other NULL, so a NULL
  // default would silently defeat the UNIQUE constraint below in exactly that common case.
  await client.execute(`CREATE TABLE IF NOT EXISTS bom_item_expected_children (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    po_item_id INTEGER NOT NULL REFERENCES po_items(id),
    lot_label TEXT NOT NULL DEFAULT '1',
    child_project_id INTEGER NOT NULL REFERENCES projects(id),
    created_by TEXT,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (po_item_id, lot_label, child_project_id)
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_bom_expected_children_po_item ON bom_item_expected_children(po_item_id)');
  ...
  // A single attached file per document (2026-09-18) — the real scanned/offline SoS a customer or
  // Sales already has, distinct from this table's own priced-line-item PDF export. Same
  // key/url/content-type shape as test_certificates.pdf_key, one file per row, overwritten on
  // re-upload (the API route deletes the old R2 object first if the extension changed).
  await addColumn(client, 'scope_of_supply', 'pdf_key TEXT');
  await addColumn(client, 'scope_of_supply', 'pdf_url TEXT');
  await addColumn(client, 'scope_of_supply', 'pdf_content_type TEXT');
  ...
  // Delete Entire BOM wipes every node and item of a project in one click — Head-only by default
  // for the same reason as ECN approval (an irreversible, whole-BOM authority call, not a Member
  // edit). INSERT OR IGNORE keeps an admin's later change in Settings → Action Permissions intact.
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('Engineering', 'engineering.bom.clear', 1)");
  ...
  // One-time backfills (migrateScopeOfSupply…backfillCalcSheetCode) run at the end of migrate(),
  // after every column they read exists — on a brand-new DB some were added further down.
  ...
  // bom_category (2026-09, Item Master round) — the BOM composer's own 12-value shape taxonomy
  // (lib/section-shapes.js's CATEGORY_LABEL), stored per catalog item so a future BOM line created
  // by picking this item can default to it — a real, persisted analogue of the four requires_*
  // defaults right above it, same "seeds the BOM line, never itself enforced" precedent. Deliberately
  // NOT the same column as this table's own pre-existing `category` (the client's original ERP import
  // classification — RAW MATERIALS/BOUGHT OUT ITEM/ELECTRICALS/etc., real populated data on 1,841
  // rows, a completely different, coarser taxonomy) — that field's semantics are untouched.
  await addColumn(client, 'items', 'bom_category TEXT');

  // Three more catalog-level defaults, same "seeds the BOM line on pick, never itself enforced,
  // freely overridden per project from there" precedent as default_requires_*/bom_category above —
  // closing the remaining real gaps a catalog pick left every BOM line composer (PrWorkspace/
  // BomTable/BomTemplateManager) to retype by hand every time.
  //   default_moc — MOC is frequently already implied by the item's own identity (e.g. a catalog
  //   row named "SA 516 GR 70" has a real answer), but a pick never set it. Nullable/free text,
  //   same convention as bom_items.moc itself — an item genuinely without a fixed grade (a gasket,
  //   a generic fitting) simply leaves this blank, and a pick then leaves the line's own MOC
  //   untouched rather than clobbering it with an empty string.
  //   default_category_fields_json — the shape-*defining* dimension(s) only (plate's Thickness;
  //   round/square/octagonal's single size dimension; angle/beam/channel/pipe/tee's Size + kg/m) —
  //   never Length, and for plate never Width either, since real catalog data confirms those vary
  //   per purchase/cut and are never encoded in the item's own identity (checked live: every real
  //   dimensional catalog row already carries its shape-size in a separate SKU per size — "MS ANGLE
  //   50 X 50 X 5 MM" vs "...65 X 65 X 6 MM", "BQ PLATE 10MM..." vs "...12 MM..." — never a length).
  //   Same JSON-blob-not-sparse-columns shape as bom_items.category_fields_json itself.
  //   default_requires_manufacturing — Feature C's own flag (bom_items.requires_manufacturing) had
  //   no catalog-level default at all; NOT NULL DEFAULT 1 matches every composer's own emptyLine()
  //   default (pre-checked) so an un-set catalog item changes nothing for a pick today.
  await addColumn(client, 'items', 'default_moc TEXT');
  await addColumn(client, 'items', 'default_category_fields_json TEXT');
  await addColumn(client, 'items', 'default_requires_manufacturing INTEGER NOT NULL DEFAULT 1');
  ...
  // Printed Job Card traveler header fields — free text, typed once per job (v1; deliberately not
  // auto-derived from calc_drawings/stock_pieces yet). wo_no already serves as the Job Number.
  await addColumn(client, 'work_orders', 'drawing_approved_on DATE');
  await addColumn(client, 'work_orders', 'drg_nos TEXT');
  await addColumn(client, 'work_orders', 'boiler_plate_nos TEXT');
  await addColumn(client, 'work_orders', 'ibr_bvi TEXT');
  ...
  // Reusable Process Route Card — a real, recurring shop-floor traveler (e.g. one boiler model's
  // standard ~33-stage sequence) saved once and applied to every future Work Order of that kind,
  // same "save a real built structure, reapply it" idea as bom_structure_templates/bom_templates —
  // flat (mirrors work_order_operations' own flat, sequenced shape), not a tree. milestone_key (not
  // milestone_id) — a template is project-agnostic, resolved to a real milestone at apply time.
  await client.execute(`CREATE TABLE IF NOT EXISTS work_order_route_templates (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    series TEXT,
    description TEXT,
  ...
  await client.execute(`CREATE TABLE IF NOT EXISTS work_order_route_template_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    template_id INTEGER NOT NULL REFERENCES work_order_route_templates(id) ON DELETE CASCADE,
    seq INTEGER NOT NULL DEFAULT 0,
    operation_id INTEGER REFERENCES operations(id),
    workstation_id INTEGER REFERENCES workstations(id),
    milestone_key TEXT,
  ...
  await client.execute('CREATE INDEX IF NOT EXISTS idx_wo_route_template_items_template ON work_order_route_template_items(template_id)');
  ...
  // New Job Card = one 33-stage sheet per boiler job (lib/job-sheet-stages.mjs). Replaces the old
  // per-task job_cards for the Shop Floor UI; the old table stays so its boot migrations still run.
  await client.execute(`CREATE TABLE IF NOT EXISTS job_sheets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    jc_no TEXT NOT NULL UNIQUE,
    project_id INTEGER REFERENCES projects(id),
    job_number TEXT,
    drawing_approved_on TEXT,
    ibr_bvi TEXT,
    drg_nos TEXT,
    boiler_plate_nos TEXT,
    start_date TEXT,
    end_date TEXT,
    notes TEXT,
    production_sign_by TEXT,
    production_sign_at TEXT,
    qc_sign_by TEXT,
    qc_sign_at TEXT,
    created_by TEXT,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute(`CREATE TABLE IF NOT EXISTS job_sheet_stages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    sheet_id INTEGER NOT NULL REFERENCES job_sheets(id) ON DELETE CASCADE,
    sort_order INTEGER NOT NULL,
    name TEXT NOT NULL,
    group_key TEXT,
    fitter_employee_id INTEGER REFERENCES employees(id),
    start_date TEXT,
    end_date TEXT,
    production_sign_by TEXT,
    production_sign_at TEXT,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_job_sheet_stages_sheet ON job_sheet_stages(sheet_id)`);
  await addColumn(client, 'job_sheets', 'scan_key TEXT');
  await addColumn(client, 'job_sheets', 'scan_type TEXT');

  // Which material this stage's work was verified against — a real link to the bank (same as every
  // other cert reference in the app: stock_pieces, qc_document_parts), not free text. Distinct from
  // qc_record_id above (a pass/fail *test event*, e.g. hydro/NDE) — this is "which certified plate/
  // heat went into this operation," the client's own real "Test Certificate Number" column.
  await addColumn(client, 'job_cards', 'test_certificate_id INTEGER REFERENCES test_certificates(id)');
  ...
  // Multi-unit split Phase F (informational stamp only, confirmed scope) — records which BOM
  // revision the master was on when a batch-children action created this record, so a later viewer
  // can tell it might now be stale against a since-revised master BOM. Never read to change what any
  // route actually does — a plain flag, not a frozen historical snapshot.
  await addColumn(client, 'job_cards', 'bom_release_revision_at_creation INTEGER');
  await addColumn(client, 'qc_documents', 'bom_release_revision_at_creation INTEGER');
  await addColumn(client, 'packing_lists', 'bom_release_revision_at_creation INTEGER');

  // QC statutory-forms plan, Phase 4 — MTC fulfillment produces a real linked test_certificates
  // record instead of a free-text guess. Same pattern as stock_pieces.test_certificate_id above:
  // per-event on bom_item_receipts (a second partial delivery can carry a different cert), and the
  // "current"/re-stamped slot on bom_items (mirrors how received_mtc_no itself is already re-stamped
  // from the completing receipt event onto the parent row).
  await addColumn(client, 'bom_item_receipts', 'test_certificate_id INTEGER REFERENCES test_certificates(id)');
  await addColumn(client, 'bom_items', 'test_certificate_id INTEGER REFERENCES test_certificates(id)');

  // QC statutory-forms plan, Phase 5 — Form III's remaining §4/§9 construction facts. Previously
  // hardcoded/wrong placeholder text (Phase 1's own fix already replaced the wrong values with an
  // honest "—"); these give it a real data home. All nullable, never defaulted — "—" on the PDF
  // when blank, exactly like every other optional qc_documents field.
  // construction_heat_treatment_note is named distinctly from qc_iiia_groups.heat_treatment (a
  // different, narrower per-Form-III-A-group concept) so the two are never confused.
  await addColumn(client, 'qc_documents', 'circumferential_seams_shell TEXT');
  await addColumn(client, 'qc_documents', 'circumferential_seams_furnace TEXT');
  await addColumn(client, 'qc_documents', 'construction_repair_details TEXT');
  await addColumn(client, 'qc_documents', 'construction_heat_treatment_note TEXT');
  // Kept as two separate fields (not one free-text line) so the value can eventually cross-check
  // against the Calc module's own governing-component figure, per explicit instruction.
  await addColumn(client, 'qc_documents', 'least_pressure_component_name TEXT');
  await addColumn(client, 'qc_documents', 'least_pressure_value TEXT');

  // Form III §3/§5/§6 — previously hardcoded "Not Applicable"/"NOT APPLICABLE" in the PDF
  // generator itself (lib/qc-folder-pdf.js), regardless of the actual boiler design. That happens
  // to be true for every fire-tube/shell boiler this app has real data for, but it was baked in as
  // a constant with no way for QC to say otherwise for a future design that genuinely has drums or
  // header boxes. Real per-document fields now; the PDF still falls back to "Not Applicable" only
  // when blank (preserves today's correct output for every existing document with zero migration),
  // but it's an editable default, not a forced one.
  await addColumn(client, 'qc_documents', 'parts_outside_constructor_works TEXT');
  await addColumn(client, 'qc_documents', 'drums_details TEXT');
  await addColumn(client, 'qc_documents', 'headers_boxes_details TEXT');

  // Dynamic belt/furnace-ring seam counts — redesigned as a repeatable-rows table, not fixed
  // belt1/belt2 columns, since a real boiler can have any number of belts/rings (the reference
  // sample alone has 2 belts + a furnace-ring line). One row per belt or per ring, distinguished by
  // `location`; `sequence` is the display order (Belt 1, Belt 2, ...), independent per location.
  await client.execute(`CREATE TABLE IF NOT EXISTS qc_document_longitudinal_seams (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    document_id INTEGER NOT NULL REFERENCES qc_documents(id) ON DELETE CASCADE,
    location TEXT NOT NULL CHECK (location IN ('belt','furnace_ring')),
    sequence INTEGER NOT NULL DEFAULT 1,
    seam_count TEXT
  )`);

  // PO Delivery Lots — Procurement's own expected-delivery scheduling per PO. Distinct from
  // bom_item_expected_children (Stores' unit-routing reference: no qty, no date, and scoped to one
  // po_item at a time) — this works for any po_item, draft or issued PO, and a single lot can span
  // several po_items/projects at once (one physical delivery covering multiple items/projects).
  await client.execute(`CREATE TABLE IF NOT EXISTS po_delivery_lots (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    po_id INTEGER NOT NULL REFERENCES purchase_orders(id) ON DELETE CASCADE,
    lot_label TEXT NOT NULL DEFAULT '1',
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_po_delivery_lots_po ON po_delivery_lots(po_id)`);

  // How much of one po_item's own qty is allocated into a given lot. A po_item can appear in
  // several lots (partial qty split across different expected dates); a lot can carry several
  // po_items (one delivery spanning several items/projects on the same PO).
  await client.execute(`CREATE TABLE IF NOT EXISTS po_delivery_lot_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lot_id INTEGER NOT NULL REFERENCES po_delivery_lots(id) ON DELETE CASCADE,
    po_item_id INTEGER NOT NULL REFERENCES po_items(id),
    qty REAL NOT NULL CHECK(qty > 0),
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_po_delivery_lot_items_lot ON po_delivery_lot_items(lot_id)`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_po_delivery_lot_items_po_item ON po_delivery_lot_items(po_item_id)`);

  // Optional: which multi-unit-split child project(s) this lot's allocation for this po_item
  // covers. Reference-only (no qty split at the child level), same semantics
  // bom_item_expected_children already established for the po_item-wide case — kept as a separate
  // table since a delivery lot's child tagging is scoped per (lot, po_item) pair, not per po_item
  // alone (a po_item split across two delivery dates can legitimately cover different child units
  // on each date). Only meaningful when the po_item's own project has real children.
  await client.execute(`CREATE TABLE IF NOT EXISTS po_delivery_lot_item_children (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lot_item_id INTEGER NOT NULL REFERENCES po_delivery_lot_items(id) ON DELETE CASCADE,
    child_project_id INTEGER NOT NULL REFERENCES projects(id),
    UNIQUE(lot_item_id, child_project_id)
  )`);
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_po_delivery_lot_item_children_item ON po_delivery_lot_item_children(lot_item_id)`);

  // Procurement's "Overdues" tab — a timestamped follow-up log per overdue item. Keyed to the
  // bom_item, not a po_delivery_lots row: an overdue item that was never scheduled into a named
  // Delivery Lot at all (the RFQ-date-fallback case getOverdueDeliveries()/attachDeliveryLotDates()
  // both cover) has no lot id to attach a note to, so the item itself is the only id every overdue
  // row is guaranteed to have. Registered in lib/bom-item-guard.js's BOM_ITEM_BLOCKING_TABLES —
  // required, per that file's own documented rule, so deleting a bom_item with a follow-up note
  // logged gets a clean 409 instead of a raw FOREIGN KEY constraint error.
  await client.execute(`CREATE TABLE IF NOT EXISTS delivery_followups (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    note TEXT NOT NULL,
    created_by TEXT,
  ...
  await client.execute(`CREATE INDEX IF NOT EXISTS idx_delivery_followups_item ON delivery_followups(bom_item_id)`);

  // Stores/Inventory hardening Phase 2 — Ownership, distinct from Reservation (plan doc invariant
  // #8). Set once at physical receipt (lib/bom-receiving.js's maybeCreatePieceStock), inherited by
  // every cut child (lib/stock-pieces.js's cutPiece), mutated only by a future Ownership Transfer
  // action (Phase 4) — never touched by Reserve/Release. Deliberately NOT the pre-existing
  // stock_pieces.project_id column: that one is reservation-coupled (stamped by reservePiece(),
  // never cleared by releasePiece() — a real, separate, pre-existing staleness gap, left alone here)
  // and would have been the wrong foundation to build ownership on.
  await addColumn(client, 'stock_pieces', 'owner_project_id INTEGER REFERENCES projects(id)');

  // Hardening find, same round: the Vendor Bill costing join (app/api/vendor-bills/[id]/route.js)
  // already silently assumes one inventory_items row per item_id; Phase 2's find-or-create logic
  // now relies on the identical assumption in a new (write) place. Made real rather than implicit.
  await client.execute(
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_inventory_items_item_id_unique ON inventory_items(item_id) WHERE item_id IS NOT NULL`
  );

  // Inward + Pre-Dispatch QC/Production Approval Workflow (2026-09-13) — the two-stage gate: QC
  // reviews every physical delivery before it becomes usable Stores inventory; QC + Production both
  // review a Packing List before Dispatch may finalize it. Two purpose-built tables, not one
  // polymorphic one — same "usb_requests vs browser_requests" precedent this codebase already
  // documents (SYSTEM.md §16): the two flows have genuinely different shapes (single QC-Head
  // approver keyed to one delivery event, vs. dual QC-Head+Production-Head approvers keyed to one
  // packing list), so forcing them into one table would only cost clarity for no reuse.

  // inward_approvals — one row per bom_item_receipts event (one physical delivery of one BOM line),
  // created inside creditBomItemReceipt() itself so every entry point that credits a receipt always
  // produces one, with no bypass. inventory_item_id/qty_scalar are only ever set for the scalar-
  // stock case (lib/bom-receiving.js's maybeReserveScalarStock) — a dimensional/piece-tracked
  // receipt instead links its held stock_pieces rows via their own bom_item_receipt_id column.
  // reason is deliberately free text, never required server-side, per the user's own original
  // wording ("optional free-text reason"). resubmission_of_id never overwrites a decided row — a
  // rejected approval stays exactly as it was, and resubmitting inserts a fresh row pointing back at
  // it, same idiom as ncr_records'/job_cards' own rework lineage.
  await client.execute(`CREATE TABLE IF NOT EXISTS inward_approvals (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bom_item_receipt_id INTEGER NOT NULL REFERENCES bom_item_receipts(id),
    bom_item_id INTEGER NOT NULL REFERENCES bom_items(id),
    project_id INTEGER REFERENCES projects(id),
    inventory_item_id INTEGER REFERENCES inventory_items(id),
    qty_scalar REAL,
    ...
    decided_at DATETIME,
    reason TEXT,
    resubmission_of_id INTEGER REFERENCES inward_approvals(id),
    submitted_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_inward_approvals_bom_item ON inward_approvals(bom_item_id)');
  await client.execute('CREATE INDEX IF NOT EXISTS idx_inward_approvals_status ON inward_approvals(status)');

  // pre_dispatch_approvals — one row per review CYCLE (not per reviewer): a single packing list can
  // have several cycles over time (reject -> resubmit), but each cycle's two independent decision
  // slots (qc_*/production_*) live on the one row, so "what's this list's status right now" is
  // always a single-row read, mirroring ncr_records' own shape of two independent facts on one row.
  // The dispatch gate (app/api/packing/[id]/route.js) reads only the LATEST row for a packing_list_id.
  await client.execute(`CREATE TABLE IF NOT EXISTS pre_dispatch_approvals (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    production_decided_by TEXT,
    production_decided_at DATETIME,
    production_reason TEXT,
    submitted_by TEXT,
    submitted_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    resubmission_of_id INTEGER REFERENCES pre_dispatch_approvals(id)
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_pre_dispatch_approvals_list ON pre_dispatch_approvals(packing_list_id)');

  // Withholding mechanism, piece side — a piece received against a BOM line pending QC inward
  // review sits here instead of at 'available' (lib/stock-pieces.js's receivePiece, called from
  // lib/bom-receiving.js's maybeCreatePieceStock). Every existing status='available' filter
  // (rollUpOnHand, findCandidates, reservePiece) needs zero changes to correctly exclude it — it
  // simply isn't 'available' yet. Cleared only by releasePieceFromInwardHold() on QC approval.
  await addColumn(client, 'stock_pieces', 'bom_item_receipt_id INTEGER REFERENCES bom_item_receipts(id)');

  // Standing, per-decision authority — approving/rejecting inward material or a pre-dispatch
  // packing list is a real business decision (does this material become usable stock? may this
  // shipment go out?), not a Member-level action. Same seeded-Head-only idiom as
  // engineering.ecn.approve/qc.ncr.disposition just above. dispatch.packing.submit_approval
  // (Dispatch's own submit/resubmit action) is deliberately left open by default, same as every
  // other routinely-open Dispatch action.
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('QC', 'qc.inward.decide', 1)");
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('QC', 'qc.predispatch.decide', 1)");
  await client.execute(
    "INSERT OR IGNORE INTO action_permissions (department, action_key, requires_head) VALUES ('Production', 'production.predispatch.decide', 1)");

  // BOM-import categorization "learns" from a human correction (2026-09-13) — closes the gap SYSTEM.md
  // §5cc's own typo fixes (TINNER/PALTE) left open: those were hand-added regex variants for
  // *already-known* mistakes; this table is what makes a *future* unknown typo need fixing only once.
  // One row per corrected word, keyed case-insensitively — a human explicitly resolving a spelling
  // suggestion during the mandatory import preview (app/api/projects/[id]/bom/import/route.js) is a
  // real, deliberate decision, not an inferred/probabilistic one (unlike tc_item_match_approvals'
  // multi-approval promotion, §5at) — so one confirmation is enough to promote it, no frequency count
  // needed. `INSERT OR REPLACE` lets a later correction for the same word supersede an earlier one.
  // A human's category for a whole description ("body shell material" -> plate), remembered for every later import; latest answer wins.
  await client.execute(`CREATE TABLE IF NOT EXISTS category_memory (alias_key TEXT PRIMARY KEY, category TEXT NOT NULL, confirmed_by TEXT, confirmed_at TEXT DEFAULT CURRENT_TIMESTAMP)`);
  await client.execute(`CREATE TABLE IF NOT EXISTS category_word_corrections (
  ...
  // Item Master link memory (2026-09-21) — what a person's link of a BOM line to a catalog row teaches the next import.
  // 'exact' = this description + MOC + size -> this catalog row. 'family' = this description + MOC is a family of catalog rows
  // that differ only by size (BQ PLATE MATERIAL -> the BQ PLATE rows), item_id is one anchor row of that family; the line's size
  // then picks the row. Written ONLY from explicit human decisions (lib/item-link.js), never from the system's own guesses, so
  // it cannot reinforce itself. Confidence is Laplace-smoothed (lib/item-attributes.mjs memoryConfidence): one approval acts,
  // one rejection pulls it back to suggest-only.
  await client.execute(`CREATE TABLE IF NOT EXISTS item_link_memory (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    kind TEXT NOT NULL CHECK (kind IN ('exact', 'family')),
    alias_key TEXT NOT NULL,
    moc_key TEXT NOT NULL DEFAULT '',
    ...
    updated_by TEXT,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (kind, alias_key, moc_key, size_key, item_id)
  )`);
  await client.execute('CREATE INDEX IF NOT EXISTS idx_item_link_memory_alias ON item_link_memory(alias_key)');

  // Phase 6 (cutting-cost accounting, 2026-09-13) — a piece's OWN known cost, not a shared running
  // average. Sidesteps a real, confirmed pre-existing corruption: the Vendor Bill route's
  // weightedAverageCost()/on_hand+=qty update conflicts with rollUpOnHand()'s independent,
  // ownership-scoped overwrite of inventory_items.on_hand for a piece-tracked row (owned pieces —
  // the norm for PMB/PR material — are never counted by rollUpOnHand's unowned-only filter, so
  // on_hand for these rows is essentially meaningless, and using it as the weighted-average's
  // "existing quantity" basis corrupts avg_cost too). Set via lib/stock-pieces.js's
  // costUncostedPieces() at Vendor Bill approval (app/api/vendor-bills/[id]/route.js), inherited by
  // every cutPiece() child (used/remnant/scrap) the same way heat_no/owner_project_id already are —
  // a remnant cut again later still carries its true original cost forward, not a blended average.
  // Never set for the generic manual Stores receive path (receivePiece() called directly) — stays
  // uncosted, same tolerance issueMaterial() already accepts elsewhere in this app.
  await addColumn(client, 'stock_pieces', 'unit_cost REAL');

  // Planning module's own Backlog note ("Cut-code lineage renumbering") — root_id lets cutPiece()
  // number every descendant of one physical delivery flat against the ROOT's own code (PL-0042-U2,
  // not the compounding PL-0042-R1-U1), instead of string-appending onto the immediate parent's
  // code at every cut generation. Self-referencing on every row inserted from here on
  // (receivePiece() sets root_id = its own id); a pre-existing row left NULL is still handled
  // correctly by cutPiece()'s own `source.root_id || source.id` fallback the first time it's cut.
  await addColumn(client, 'stock_pieces', 'root_id INTEGER REFERENCES stock_pieces(id)');

  // Inventory dimensional model (gentle-snuggling-wozniak.md §4) — inventory_items had zero
  // structured-dimension storage, only a flattened free-text `spec` string. Reuses the exact same
  // JSON shape bom_items.category_fields_json/items.default_category_fields_json already use
  // (GEOMETRY_SHAPES/ROLLED_CATEGORIES field sets, lib/section-shapes.js) — never a new schema.
  await addColumn(client, 'inventory_items', 'category_fields_json TEXT');
  ...
  await backfillItemBomCategory(client);
  await fixBoiItemBomCategory(client);
  await backfillPrRaisedPendingReview(client);
  await retireOldJobCards(client);
  await backfillDrawingCode(client);
  await backfillCalcSheetCode(client);

  await replayDeferredColumns(client);
}
...
// items.bom_category backfill (2026-09, Item Master round) — high-confidence only, same bar as the
// PMB importer's own per-row suggestion: either the pre-existing ERP `category` maps unambiguously
// onto our shape taxonomy, or inferCategory() gets a real keyword match on item_name. Everything
// else is left NULL — no domain-guessing at catalog scale, a human resolves it at BOM-creation time
// via the now-mandatory category field instead. Marker-gated so this runs exactly once.
const ERP_CATEGORY_TO_BOM_CATEGORY = {
  'BOUGHT OUT ITEM': 'standard',
  ELECTRICALS: 'other', CONSUMABLES: 'other', TOOLS: 'other', HARDWARE: 'other',
  'MACH MAINT': 'other', ASSET: 'other',
  // 'BOI' is the same real ERP concept as 'BOUGHT OUT ITEM' above, just abbreviated (confirmed
  // live, 2026-09-07: every real BOI-tagged catalog row also carries `group_name: 'MOUNTING'` —
  // valves, gauges, fusible plugs) — was wrongly mapped to 'other' here, silently defaulting a
  // large share of the real catalog's bought-out mounting parts to the wrong bucket on every future
  // pick. See fixBoiItemBomCategory() below for the one-time correction of rows already backfilled
  // wrong before this fix.
  BOI: 'standard',
};
// One-time: the old per-task job cards are replaced by job_sheets (2026-09-26). Rows were exported to
// scripts/data/job-cards-backup-2026-09-26.json first. Tables stay; dangling ids are nulled.
async function retireOldJobCards(client) {
  const key = 'retire_old_job_cards_v1';
  if ((await client.execute({ sql: 'SELECT 1 FROM system_migrations WHERE migration_key = ?', args: [key] })).rows.length) return;
  for (const sql of [
    // Turso enforces FKs: clear everything that points at job_cards first, then the cards.
    'UPDATE material_issues SET job_card_id = NULL WHERE job_card_id IS NOT NULL',
    'UPDATE material_indents SET job_card_id = NULL WHERE job_card_id IS NOT NULL',
    'UPDATE stock_pieces SET job_card_id = NULL WHERE job_card_id IS NOT NULL',
    'UPDATE ncr_records SET job_card_id = NULL, rework_job_card_id = NULL',
    'UPDATE job_cards SET rework_of_job_card_id = NULL',
    'DELETE FROM job_card_time_logs', 'DELETE FROM job_card_consumables', 'DELETE FROM job_cards',
  ]) await client.execute(sql);
  await client.execute({ sql: 'INSERT INTO system_migrations (migration_key) VALUES (?)', args: [key] });
}

async function backfillItemBomCategory(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'item_bom_category_v1'");
  if (marker.rows.length) return;

  const rows = await client.execute("SELECT id, item_name, category FROM items WHERE bom_category IS NULL");
  let n = 0;
  for (const row of rows.rows) {
    const bomCategory = ERP_CATEGORY_TO_BOM_CATEGORY[row.category] || inferCategory(row.item_name);
    if (bomCategory) {
      await client.execute({ sql: 'UPDATE items SET bom_category = ? WHERE id = ?', args: [bomCategory, row.id] });
      n++;
    }
  }
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('item_bom_category_v1')");
}

// One-time correction for rows the original backfill already ran against before the BOI mapping fix
// above — backfillItemBomCategory's own WHERE bom_category IS NULL guard would never touch these
// again (they're already 'other', not NULL), so this is a separate, narrowly-scoped UPDATE rather
// than a re-run of the general backfill. Marker-gated, same idiom as every other one-time fix here.
async function fixBoiItemBomCategory(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'item_bom_category_boi_fix_v1'");
  if (marker.rows.length) return;
  await client.execute("UPDATE items SET bom_category = 'standard' WHERE category = 'BOI' AND bom_category = 'other'");
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('item_bom_category_boi_fix_v1')");
}

// One-time backfill for the "PR-raised lines skip Stores Review + release_bom" product decision
// (2026-09-12) — app/api/purchase-requisitions/route.js stopped setting pending_review=1 on new
// 'bom'-source PR-raised lines going forward, but existing PR-raised rows already carrying
// pending_review=1 (raised before this change, in Manual allocation mode) would otherwise stay
// stuck until someone in Stores manually clicked Procure. Scoped strictly to bom_items.pr_item_id
// IS NOT NULL — a PMB-imported/manual/template row's pending_review is completely untouched, exactly
// as designed (only PR-raised lines get this exemption). The release_bom side of the same decision
// needs no backfill at all — lib/data.js's getSourcingItems() already checks pr_item_id IS NOT NULL
// live, on every read, so an existing PR-raised row becomes visible the moment this deploys.
async function backfillPrRaisedPendingReview(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'pr_raised_pending_review_v1'");
  if (marker.rows.length) return;
  await client.execute("UPDATE bom_items SET pending_review = 0 WHERE pr_item_id IS NOT NULL AND pending_review = 1");
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('pr_raised_pending_review_v1')");
}
...
// Sales CRM expansion Phase 0c — replaces the original 5-value sales_stages seed
// (Lead/Qualified/Quoted/Won/Lost) with the user's real 9-value Sales Call funnel. Renames rows
// IN PLACE (UPDATE, never DELETE+INSERT) because opportunities.stage matches by name, not an FK
// id (same idiom bom_items.purchase_status already uses) — a delete+insert would silently orphan
// every existing opportunity currently sitting at one of the 5 old stage names. Then inserts the 4
// genuinely new stages. Marker-gated so this runs exactly once and a PM's own later stage edits
// (via the existing add-only POST /api/sales-stages) are never clobbered by a second run.
async function migrateSalesStagesTo9ValueFunnel(client) {
  ...
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'sales_stages_9value_funnel_v1'");
  if (marker.rows.length) return;

  // old name -> [new name, sort_order, is_won, is_lost]
  const remap = {
    Lead:      ['Lead - Cold', 0, 0, 0],
    Qualified: ['Proposals', 3, 0, 0],
    Quoted:    ['Hot Offers', 4, 0, 0],
    Won:       ['Order Received', 5, 1, 0],
    Lost:      ['Order Lost', 6, 0, 1],
  };
  for (const [oldName, [newName, sortOrder, isWon, isLost]] of Object.entries(remap)) {
    await client.execute({
      sql: 'UPDATE sales_stages SET name = ?, sort_order = ?, is_won = ?, is_lost = ? WHERE name = ?',
      args: [newName, sortOrder, isWon, isLost, oldName],
    });
  }
  // The 4 stages with no old-5 equivalent — plain inserts, guarded by sales_stages.name's own
  // UNIQUE constraint in case a PM already added a same-named custom stage before this ran.
  const newStages = [
    ['Lead - Hot', 1, 0, 0],
    ['Lead Project - Dropped', 2, 0, 1],
    ['Follow up stage', 7, 0, 0],
    ['OEM Follow Ups Monthly', 8, 0, 0],
  ];
  for (const [name, sortOrder, isWon, isLost] of newStages) {
    await client.execute({
      sql: 'INSERT OR IGNORE INTO sales_stages (name, sort_order, is_won, is_lost) VALUES (?, ?, ?, ?)',
      args: [name, sortOrder, isWon, isLost],
    });
  }
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('sales_stages_9value_funnel_v1')");
}

// Sales CRM plan 1a — leads.status used to be a hand-set new|contacted|qualified|converted|lost
// field that could disagree with the funnel stage. It is now a system-maintained summary of the
// stage (open|won|lost, lib/lead-stage.mjs). One-time rewrite of any existing rows; the live DB had
// zero leads when this shipped (2026-09-25), so this is a correctness guarantee, not a data change.
async function migrateLeadStatusFromStage(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'lead_status_from_stage_v1'");
  if (marker.rows.length) return;
  await client.execute(`
    UPDATE leads SET status = CASE
      WHEN EXISTS (SELECT 1 FROM sales_stages s WHERE s.name = leads.sales_call_status AND s.is_won = 1) THEN 'won'
      WHEN EXISTS (SELECT 1 FROM sales_stages s WHERE s.name = leads.sales_call_status AND s.is_lost = 1) THEN 'lost'
      ELSE 'open' END`);
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('lead_status_from_stage_v1')");
}

// migrateSalesStagesTo9ValueFunnel renamed the sales_stages rows but never the opportunities that
// pointed at the old names (opportunities.stage is plain TEXT, not an FK), so every pre-existing
// opportunity sat in a stage that no longer existed and fell off the Pipeline Kanban. Same old→new
// mapping as that migration.
async function migrateOpportunityStageNames(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'opportunity_stage_names_v1'");
  if (marker.rows.length) return;
  const remap = { Lead: 'Lead - Cold', Qualified: 'Proposals', Quoted: 'Hot Offers', Won: 'Order Received', Lost: 'Order Lost' };
  for (const [oldName, newName] of Object.entries(remap)) {
    await client.execute({ sql: 'UPDATE opportunities SET stage = ? WHERE stage = ?', args: [newName, oldName] });
  }
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('opportunity_stage_names_v1')");
}

// Sales CRM plan 1e — an enquiry's single product (leads.product_id / free-text leads.product)
// becomes its first lead_products line. The live DB had zero leads when this shipped (2026-09-25);
// this guarantees any enquiry created before the table existed still shows its product.
async function backfillLeadProducts(client) {
  const marker = await client.execute("SELECT 1 FROM system_migrations WHERE migration_key = 'lead_products_backfill_v1'");
  if (marker.rows.length) return;
  await client.execute(`
    INSERT INTO lead_products (lead_id, product_id, description, unit, rate, gst_pct, sort_order)
    SELECT l.id, l.product_id, COALESCE(NULLIF(TRIM(l.product), ''), sp.product_name), sp.unit, sp.price, sp.gst_pct, 0
      FROM leads l LEFT JOIN sales_products sp ON sp.id = l.product_id
     WHERE (l.product_id IS NOT NULL OR NULLIF(TRIM(l.product), '') IS NOT NULL)
       AND NOT EXISTS (SELECT 1 FROM lead_products lp WHERE lp.lead_id = l.id)
       AND COALESCE(NULLIF(TRIM(l.product), ''), sp.product_name) IS NOT NULL`);
  await client.execute("INSERT INTO system_migrations (migration_key) VALUES ('lead_products_backfill_v1')");
}
...
// A few addColumn calls sit above their table's CREATE (the table was added later in migrate()'s
// history). On an existing DB that's harmless; on a brand-new DB the table doesn't exist yet, so the
// call is parked here and replayed at the end of migrate() (replayDeferredColumns).
let deferredColumns = [];
async function addColumn(client, table, columnDef) {
  try {
    ...
    if (msg.includes('no such table')) { deferredColumns.push([table, columnDef]); return; }
    throw e;
  }
}
// Wraps the migrate() client so a parked column is added right after its CREATE TABLE runs —
// later statements in migrate() may already read or write it.
function withDeferredColumnReplay(client) {
  return new Proxy(client, {
    get(target, prop) {
      if (prop !== 'execute') return typeof target[prop] === 'function' ? target[prop].bind(target) : target[prop];
      return async (stmt, ...rest) => {
        const result = await target.execute(stmt, ...rest);
        const sql = typeof stmt === 'string' ? stmt : stmt?.sql;
        const m = sql && /CREATE TABLE IF NOT EXISTS (\w+)/i.exec(sql);
        if (m && deferredColumns.some(([t]) => t === m[1])) await replayDeferredColumns(target, m[1]);
        return result;
      };
    },
  });
}
async function replayDeferredColumns(client, onlyTable = null) {
  const pending = deferredColumns.filter(([t]) => !onlyTable || t === onlyTable);
  deferredColumns = deferredColumns.filter(([t]) => onlyTable && t !== onlyTable);
  for (const [table, columnDef] of pending) {
    try {
      await client.execute(`ALTER TABLE ${table} ADD COLUMN ${columnDef}`);
    } catch (e) {
...
async function seedV3DemoData(client) {
  ...
  if (oppCount.rows[0].n === 0) {
    // Stage names updated for the 9-value Sales Call funnel (Sales CRM expansion Phase 0c) —
    // this block only ever runs against a genuinely empty opportunities table, so it can't affect
    // real data, but a fresh dev/test DB seeded after that migration should never start with
    // Kanban stage values that no longer exist in sales_stages.
    const demoOpps = [
      ['Asian Brown — 8T boiler upgrade', 'Asian Brown', 'Hot Offers', 3200000, 60, 'Sales'],
      ['HKM Charitable — annual AMC renewal', 'HKM Charitable Trust', 'Proposals', 850000, 40, 'Sales'],
      ['Virchow Biotech — steam line expansion', 'Virchow Biotech', 'Lead - Cold', 5400000, 20, 'Sales'],
      ['Trade fair 2026 — inbound enquiries batch', null, 'Lead - Cold', 1200000, 10, 'Marketing'],
      ['Regional distributor tie-up — West zone', null, 'Order Received', 2100000, 100, 'Marketing'],
    ];
...
async function seedV3HrData(client) {
  ...
  // workers is retired and never created on a brand-new DB — nothing to absorb there.
  const hasWorkers = await client.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='workers'");
  const workers = hasWorkers.rows.length ? await client.execute('SELECT * FROM workers') : { rows: [] };
...
// `next dev` bundles each route separately, so this module (and its module-level initPromise) is
// instantiated once PER ROUTE — the full migrate() (hundreds of statements over the network to
// Turso) used to re-run the first time every route was hit, 2–3 minutes each. Caching the promise
// on globalThis shares it across route bundles in the same process. Keyed by migrate()'s own source
// length so editing a migration in dev still re-runs it instead of reusing a stale promise.
const INIT_KEY = `__shantiOpsInitDB_${migrate.toString().length}`;

export async function initDB() {
  if (!initPromise) {
    if (!globalThis[INIT_KEY]) {
      const client = getClient();
      globalThis[INIT_KEY] = migrate(client).then(() => client).catch((err) => {
        delete globalThis[INIT_KEY]; // let the next request retry instead of caching a failure
        initPromise = null;
        throw err;
      });
    }
    initPromise = globalThis[INIT_KEY];
  }
  return initPromise;
}
...
export async function queryAll(sql, params = []) {
  ...
  // Plain objects, not libsql Row instances: server pages pass these lists straight to client
  // components, and Next warns (in dev, once per row — thousands for the customer list) on class
  // instances. Same columns, same values.
...
// Generic get/set over the single key/value app_settings table (stores_allocation_mode,
// remnant_match_tolerances, ...) — one shared upsert helper instead of a bespoke pair per setting.
...
// Sales CRM plan 4 — price_lists.item_id was NOT NULL (keyed to the engineering Item Master).
// Sales prices products, so the table is rebuilt once with item_id nullable and a product_id
// column. SQLite can't drop NOT NULL in place; rows are copied with their ids. Runs only while the
// old NOT NULL shape is still there, so it's safe on every boot and on a fresh DB (no-op).
async function rekeyPriceListsToProducts(client) {
  const cols = (await client.execute('PRAGMA table_info(price_lists)')).rows;
  const itemCol = cols.find(c => c.name === 'item_id');
  if (!itemCol || !Number(itemCol.notnull)) {
    await addColumn(client, 'price_lists', 'product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL');
    return;
  }
  await client.batch([
    `CREATE TABLE price_lists_v2 (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      customer_id INTEGER REFERENCES customers(id),
      item_id INTEGER REFERENCES items(id),
      product_id INTEGER REFERENCES sales_products(id) ON DELETE SET NULL,
      rate REAL NOT NULL, uom TEXT, valid_from DATE, valid_until DATE, notes TEXT,
      created_by TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
    )`,
    `INSERT INTO price_lists_v2 (id, customer_id, item_id, rate, uom, valid_from, valid_until, notes, created_by, created_at)
       SELECT id, customer_id, item_id, rate, uom, valid_from, valid_until, notes, created_by, created_at FROM price_lists`,
    'DROP TABLE price_lists',
    'ALTER TABLE price_lists_v2 RENAME TO price_lists',
  ], 'write');
}
