...
// Column widths (label, width%, value(part)) — sum ~100.
const COLS = [
  ['P.No', 3, p => p.part_no],
  ['Part', 15, p => p.part_name],
  ['Cast No', 8, p => p.tc_cast_no || '—'],
  ['Plate No', 8, p => p.tc_plate_no || '—'],
  ['Size', 8, p => [p.size_t, p.size_w, p.size_l].filter(Boolean).join(' × ') || '—'],
  ['Qty', 3, p => p.qty],
  ['Spec', 7, p => p.material_spec || '—'],
  ['Steel Maker', 9, p => p.steel_maker || '—'],
  ['Cert No', 9, p => p.certificate_no || '—'],
  ['C', 3, p => p.chem_c],
  ['Mn', 3, p => p.chem_mn],
  ['P', 3, p => p.chem_p],
  ['S', 3, p => p.chem_s],
