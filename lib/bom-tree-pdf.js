...
export function flattenBomTree(assemblies, unassignedItems) {
  ...
  function walk(node, depth) {
    ...
    // The node's Configuration (datasheet fields) right under it — specification lines, not items (no quantity,
    // not counted in the item total). Old release snapshots have no `config`, hence the fallback.
    for (const [i, c] of (node.config || []).entries()) rows.push({ _key: `c-${node.id}-${i}`, _kind: 'config', depth: depth + 1, config: c });
...
function descriptionCell(r) {
  ...
  if (r._kind === 'config') return `${indent}Config - ${r.config.label}: ${r.config.value ? r.config.value + (r.config.unit ? ' ' + r.config.unit : '') : '-'}`;
  return `${indent}BM-${r.item.id} ${r.item.material_description}`;
}

function qtyCell(r) {
  if (r._kind === 'config') return '';
...
const COLS = [
  ['Description', 38, descriptionCell],
  ['Item Code', 14, (r) => (r._kind === 'item' ? r.item.catalog_item_code || '—' : '—')],
  // round 3 Phase B — real pr_no wins over the legacy free-text pr_ref, same precedent as
  // BomTreeReadOnly.jsx/BomTable's own PR display.
  ['PR No.', 14, (r) => (r._kind === 'item' ? r.item.pr_no || r.item.pr_ref || '—' : '—')],
  ['Qty', 16, qtyCell, 'right'],
  ['Roll-up total', 18, rollupCell, 'right'],
];
