...
import { parseConfig, mergeConfig, serializeConfig } from './bom-config.mjs';

const GENERIC_SHEET_NAME = /^sheet\s*\d*$/i;

// Does a configuration entry (a datasheet row: TYPE / FLOW cfm…) have a node to live on? Same rule the
// grouping below uses for items: a generic CSV sheet ("Sheet1") has no System wrapper, so a row with no
// heading has no home. The import route turns such entries back into plain items BEFORE calling the
// builder, so a datasheet row is never silently dropped.
export function configHasHome(entry) {
  return !(GENERIC_SHEET_NAME.test(String(entry?.section || '').trim()) && !String(entry?.group_label || '').trim());
}
...
export async function buildAssemblyTreeFromImport({ tx, projectId, importId, username, configEntries = [] }) {
  ...
  // Configuration entries (datasheet rows) join the grouping as virtual rows: they carry the same
  // section/heading an item row would have, so the tree comes out exactly as it did when these rows were
  // still items (a heading holding only datasheet rows still gets its node), and each entry is then
  // merged onto the node it groups under.
  const configRows = configEntries.map(e => ({ id: null, section: e.section, group_label: e.group_label, group_name: null, _config: e }));
  const rows = [...res.rows, ...configRows];
  if (!rows.length) return { nodesCreated: 0, nodesReused: 0, itemsAssigned: 0, configsAdded: 0, configsUpdated: 0, configsUnassigned: 0 };
  ...
  const assignments = new Map(); // assemblyId -> row[] (item rows and virtual configuration rows)
  const queueAssign = (assemblyId, row) => {
    if (!assignments.has(assemblyId)) assignments.set(assemblyId, []);
    assignments.get(assemblyId).push(row);
  };
  ...
  for (const [section, sectionRows] of bySection) {
    ...
    if (labelGroups.size > 1 || (labelGroups.size === 1 && hasExistingChildren)) {
      ...
      for (const [label, labelRows] of labelGroups) {
        ...
        record(node.created);
        for (const row of labelRows) queueAssign(node.id, row);
      }
      ...
      if (!isGeneric) for (const row of unlabeled) queueAssign(systemNodeId, row);
      ...
      if (isGeneric) {
        ...
        for (const row of labelRows) queueAssign(systemNodeId, row);
        for (const row of unlabeled) queueAssign(systemNodeId, row);
      }
    } else if (!isGeneric) {
  ...
  const configByNode = new Map(); // assemblyId -> [{label, value}] in file order
  for (const [assemblyId, assigned] of assignments) {
    for (const row of assigned) {
      if (row._config) {
        if (!configByNode.has(assemblyId)) configByNode.set(assemblyId, []);
        configByNode.get(assemblyId).push({ label: row._config.label, value: row._config.value, unit: row._config.unit });
      } else {
        await tx.execute({ sql: 'UPDATE bom_items SET assembly_id = ? WHERE id = ?', args: [assemblyId, row.id] });
        itemsAssigned++;
      }
    }
  }

  // Merge (never overwrite) each node's Configuration: same label + non-empty imported value updates it,
  // a blank imported value never blanks an existing one, other rows are untouched — so a Replace re-import
  // is idempotent and can't duplicate labels.
  let configsAdded = 0, configsUpdated = 0, placed = 0;
  for (const [assemblyId, entries] of configByNode) {
    placed += entries.length;
    const cur = await tx.execute({ sql: 'SELECT config_json FROM bom_assemblies WHERE id = ?', args: [assemblyId] });
    const merged = mergeConfig(parseConfig(cur.rows[0]?.config_json), entries);
    if (merged.added || merged.updated) {
      await tx.execute({ sql: 'UPDATE bom_assemblies SET config_json = ? WHERE id = ?', args: [serializeConfig(merged.list), assemblyId] });
    }
    configsAdded += merged.added; configsUpdated += merged.updated;
  }

  return { nodesCreated, nodesReused, itemsAssigned, configsAdded, configsUpdated, configsUnassigned: configEntries.length - placed };
}
