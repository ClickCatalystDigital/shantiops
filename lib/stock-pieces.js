...
const DEFAULT_DENSITY = 7850; // kg/m^3, mild steel — same default the user's own spec quoted

// Weight from geometry, not a hand-typed number (the whole point — see the source chat: "weight =
// volume × density"). plate: L x W x T x density, all mm -> m. linear: length x kg/m (a standard
// section's per-metre weight, since a non-rectangular profile's cross-section isn't L x W x T).
...
// Stores receives new dimensional stock (a bought plate/section, not a remnant).
export async function receivePiece({ inventoryItemId, kind, length_mm, width_mm, thickness_mm, density, kg_per_m, username }) {
  ...
  const { lastId } = await execute(
    `INSERT INTO stock_pieces (inventory_item_id, kind, length_mm, width_mm, thickness_mm, density, kg_per_m, weight_kg, status, source)
    ...
    for (const u of used) {
      ...
      const ins = await tx.execute({
        sql: `INSERT INTO stock_pieces (inventory_item_id, kind, length_mm, width_mm, thickness_mm, density, kg_per_m,
                                          weight_kg, status, source, parent_id, project_id, bom_item_id, job_card_id, cut_by, cut_at)
...
export async function listPieces({ inventoryItemId, bomItemId }) {
  if (bomItemId) {
    return queryAll('SELECT * FROM stock_pieces WHERE bom_item_id = ? ORDER BY id DESC', [bomItemId]);
  }
  return queryAll(
    'SELECT * FROM stock_pieces WHERE inventory_item_id = ? ORDER BY id DESC',
    [inventoryItemId]
  );
}
