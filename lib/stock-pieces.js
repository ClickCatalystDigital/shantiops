...
// PL-0007 / LN-0007[-H<heat>] — the traceability id the source discussion asked for. Generated
// from the row's own id after insert (simplest unique, sequential, human-readable code available
// without a separate counter table) plus the heat number when one was captured at receipt —
// direct request: the heat should be readable straight off the physical tag, not only looked up in
// a separate field. Sanitized to A-Z0-9 only (a free-text heat number can carry slashes/spaces that
// would otherwise break the code's own "-" separator convention) and capped at 10 chars so the code
// stays a short, printable tag rather than growing unbounded.
function sanitizeHeatForCode(heatNo) {
  return String(heatNo || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 10);
}
...
// Planning's Backlog note, closed: every descendant of one physical delivery is numbered flat
// against the ROOT's own code — PL-0042-U2, PL-0042-R2 — never compounding onto whichever
// immediate parent happened to be cut (PL-0042-R1-U1). Counted with the same atomic
// INSERT...ON CONFLICT...RETURNING shape lib/db.js's nextCounterValue() already proves out for
// every other sequence in this app, run directly against the caller's own transaction (not via that
// shared helper, which uses a separate, non-transactional connection) so a rolled-back cut can never
// leave a permanent gap the way a failed PO/GIR create already can for those counters.
async function nextPieceSeq(tx, rootId, letter) {
  const result = await tx.execute({
    sql: `INSERT INTO counters (name, value) VALUES (?, 1)
          ON CONFLICT(name) DO UPDATE SET value = value + 1
          RETURNING value`,
    args: [`piece_seq:${rootId}:${letter}`],
  });
...
//
// heldForInwardApproval / bomItemReceiptId (Inward QC/Production Approval Workflow, optional) —
// the physical-stock half of the new withholding mechanism. When set, this piece is inserted at
// 'pending_qc_inward' instead of 'available' and linked to the bom_item_receipts event it arrived
// on — every existing status='available' filter (rollUpOnHand, findCandidates, reservePiece)
// already correctly excludes it, since it simply isn't 'available' yet. Cleared only by
// releasePieceFromInwardHold() below, once QC approves the delivery. Omitted (every other caller —
// the generic Stores manual-receive path, cut remnants) = today's exact behavior, unaffected.
export async function receivePiece({ inventoryItemId, kind, length_mm, width_mm, thickness_mm, density, kg_per_m, heat_no, test_certificate_id, bomItemId, receiptId, ownerProjectId, heldForInwardApproval = false, bomItemReceiptId, username }) {
  ...
  const { lastId } = await execute(
    `INSERT INTO stock_pieces (inventory_item_id, kind, length_mm, width_mm, thickness_mm, density, kg_per_m, weight_kg, status, source, heat_no, test_certificate_id, receipt_id, owner_project_id, bom_item_receipt_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'purchase', ?, ?, ?, ?, ?)`,
    [inventoryItemId, kind, length_mm || null, width_mm || null, thickness_mm || null,
      ...
      round2(weight), heldForInwardApproval ? 'pending_qc_inward' : 'available',
      heat_no || null, test_certificate_id || null, receiptId || null, ownerProjectId || null,
      heldForInwardApproval ? (bomItemReceiptId || null) : null]
  );
  const id = Number(lastId);
  const code = rootCode(kind, id, heat_no);
  // root_id = self — every piece this one is ever cut into references it, no separate walk needed.
  await execute('UPDATE stock_pieces SET code = ?, root_id = ? WHERE id = ?', [code, id, id]);
  await execute('UPDATE inventory_items SET track_pieces = 1 WHERE id = ?', [inventoryItemId]);
  // A held piece isn't 'available' — rollUpOnHand's own filter already excludes it, so this is a
  // harmless no-op recount for the held case, kept unconditional to avoid a second code path.
...
// Phase 6 (cutting-cost accounting) — prices whichever of this item's pieces are still uncosted,
// called from app/api/vendor-bills/[id]/route.js on approval instead of the broken weighted-average/
// on_hand+=qty update (see lib/db.js's stock_pieces.unit_cost comment for the full corruption trace:
// rollUpOnHand()'s own ownership-scoped overwrite of on_hand makes it an unusable basis for
// weighted-average costing here). Deliberately NOT a precise bill-to-piece link — no such link
// exists (vendor_bill_items only references bom_item_id, and one bom_item can have several
// bom_item_receipts, §5bw) — instead, self-correcting: prices up to `qty` pieces still genuinely
// uncosted (oldest id first), so a LATER bill for the same catalog item, at a different rate, only
// ever touches pieces THAT bill's own delivery actually created, never re-pricing an earlier lot
// that's already been priced. Never touches on_hand — rollUpOnHand() remains its sole owner.
export async function costUncostedPieces(inventoryItemId, qty, unitCost) {
  if (!(qty > 0) || !(unitCost >= 0)) return 0;
  const pieces = await queryAll(
    `SELECT id FROM stock_pieces
      WHERE inventory_item_id = ? AND unit_cost IS NULL AND status NOT IN ('consumed', 'scrap')
      ORDER BY id ASC LIMIT ?`,
    [inventoryItemId, Math.round(qty)]
  );
  for (const p of pieces) {
    await execute('UPDATE stock_pieces SET unit_cost = ? WHERE id = ?', [unitCost, p.id]);
  }
...
export async function cutPiece({ sourcePieceId, used = [], remnants = [], projectId, bomItemId, jobCardId, username }) {
  ...
  // Planning's Backlog note, closed — every child's code is built off the ROOT's own code, not
  // source.code (which, for a remnant being cut a second time, is already itself a child code like
  // PL-0042-R1). Falls back to source.code if the lookup somehow comes up empty — never worse than
  // the pre-fix behavior, just not yet flattened for that one row.
  const rootId = source.root_id || source.id;
  const root = rootId === source.id ? source : await queryOne('SELECT code FROM stock_pieces WHERE id = ?', [rootId]);
  const rootCodeStr = root?.code || source.code;
  ...
  // Collected inside the transaction (only meaningful once it durably commits — see the
  // best-effort Stores notification after withTransaction resolves, below).
  const remnantCodes = [];
  ...
  const result = await withTransaction(async tx => {
    ...
    for (const u of used) {
      ...
      const ins = await tx.execute({
        sql: `INSERT INTO stock_pieces (inventory_item_id, kind, length_mm, width_mm, thickness_mm, density, kg_per_m,
                                          weight_kg, status, source, parent_id, root_id, project_id, bom_item_id, job_card_id, cut_by, cut_at, heat_no, test_certificate_id, part_name, owner_project_id, unit_cost)
        ...
        args: [source.inventory_item_id, source.kind, u.length_mm || null, u.width_mm || null, u.thickness_mm || null,
          source.density, source.kg_per_m, round2(w), sourcePieceId, rootId, projectId || null, bomItemId || null, jobCardId || null, username || null,
          source.heat_no || null, source.test_certificate_id || null, u.part_name || null, source.owner_project_id || null, source.unit_cost ?? null],
      });
      const seq = await nextPieceSeq(tx, rootId, 'U');
      await tx.execute({ sql: 'UPDATE stock_pieces SET code = ? WHERE id = ?', args: [`${rootCodeStr}-U${seq}`, Number(ins.lastInsertRowid)] });
    }
    for (const r of remnants) {
      ...
      const ins = await tx.execute({
        sql: `INSERT INTO stock_pieces (inventory_item_id, kind, length_mm, width_mm, thickness_mm, density, kg_per_m,
                                          weight_kg, status, source, parent_id, root_id, heat_no, test_certificate_id, owner_project_id, unit_cost)
        ...
        args: [source.inventory_item_id, source.kind, r.length_mm || null, r.width_mm || null, r.thickness_mm || null,
          source.density, source.kg_per_m, round2(w), sourcePieceId, rootId, source.heat_no || null, source.test_certificate_id || null, source.owner_project_id || null, source.unit_cost ?? null],
      });
      const seq = await nextPieceSeq(tx, rootId, 'R');
      const remnantCode = `${rootCodeStr}-R${seq}`;
      await tx.execute({ sql: 'UPDATE stock_pieces SET code = ? WHERE id = ?', args: [remnantCode, Number(ins.lastInsertRowid)] });
      remnantCodes.push(remnantCode);
    }
    if (scrapWeight > 0) {
      const ins = await tx.execute({
        sql: `INSERT INTO stock_pieces (inventory_item_id, kind, weight_kg, status, source, parent_id, root_id, heat_no, test_certificate_id, owner_project_id, unit_cost)
              VALUES (?, ?, ?, 'scrap', 'remnant', ?, ?, ?, ?, ?, ?)`,
        args: [source.inventory_item_id, source.kind, scrapWeight, sourcePieceId, rootId, source.heat_no || null, source.test_certificate_id || null, source.owner_project_id || null, source.unit_cost ?? null],
      });
      const seq = await nextPieceSeq(tx, rootId, 'S');
      await tx.execute({ sql: 'UPDATE stock_pieces SET code = ? WHERE id = ?', args: [`${rootCodeStr}-S${seq}`, Number(ins.lastInsertRowid)] });
    }
    ...
    return { sourceId: sourcePieceId, usedWeight: round2(usedWeight), remnantWeight: round2(remnantWeight), scrapWeight, remnantCodes: [...remnantCodes] };
  });

  // Phase 6 — best-effort cutting-cost accounting, after the transaction has durably committed,
  // same placement precedent as maybeCreatePieceStock/notifyInwardApprovalPending
  // (lib/bom-receiving.js) and issueMaterial()'s own inline posting (lib/material-issues.js). Costs
  // the "used" fraction of THIS SPECIFIC piece's own known unit_cost — never
  // inventory_items.avg_cost, which the Vendor Bill route's weighted-average update genuinely
  // corrupts for a piece-tracked row (lib/db.js's unit_cost column comment). Remnants/scrap post
  // nothing — they haven't left inventory, only used material has. Skipped cleanly, no error, when
  // the piece was never costed (unit_cost null — e.g. received via the generic manual Stores path,
  // or no Vendor Bill recorded yet) or company can't be resolved (a standalone/project-less cut,
  // §5k addendum) — same "not every consumption is costed" tolerance issueMaterial() already
  // accepts. sourceId alone is a safe idempotency key for postJournalEntry — the CAS above already
  // guarantees a piece can only ever be cut once.
  if (usedWeight > 0 && source.unit_cost != null && projectId) {
    try {
      const proj = await queryOne('SELECT company FROM projects WHERE id = ?', [projectId]);
      const amount = round2((usedWeight / source.weight_kg) * source.unit_cost);
      if (proj?.company && amount > 0) {
        await postJournalEntry({
          company: proj.company,
          entryDate: todayISO(),
          sourceType: 'stock_piece_cut',
          sourceId: sourcePieceId,
          description: `Material cut — piece ${source.code || sourcePieceId}`,
          lines: materialConsumptionLines({ amount }),
          createdBy: username,
        });
      }
    } catch (err) { /* best-effort — a cut already durably recorded never rolls back for this */ }
  }

  // Remnant-return notification (gentle-snuggling-wozniak.md §8/§9) — the one real gap in an
  // otherwise-complete pending_receipt -> confirmPieceReceipt() -> available lifecycle: Production
  // never told anyone a remnant now needs Stores' attention. Best-effort, after the transaction has
  // durably committed, same placement precedent as the cost-accounting block just above.
  if (remnantCodes.length) {
    try {
      await notifyDepartment('Stores', {
        kind: 'remnant_pending_receipt', title: 'Remnant cut — needs confirming in Stores',
        body: `${remnantCodes.join(', ')} — cut from ${source.code}, awaiting Confirm receipt`,
        dedupe_key: `remnant_pending:${sourcePieceId}`,
      });
...
// Final Phase 0-7 audit gap-fix (2026-09-13) — a whole piece-tracked item dispatched WITHOUT ever
// being cut (a bought fitting/valve/gauge, piece-tracked but shipped as-is — routed straight to
// Dispatch, §5bi) previously closed out at app/api/packing/[id]/route.js's dispatch-time UPDATE
// with zero accounting: cutPiece()'s own posting above only ever covers the cutting path, and this
// is a genuinely separate "material leaves the books" moment (confirmed the only two write paths
// that ever set stock_pieces.status='consumed' anywhere in the app). Posts the piece's FULL
// unit_cost, not a fraction — the entire piece leaves, not a cut portion of it. Same best-effort
// tolerance as cutPiece()'s own posting: skipped cleanly when unit_cost/company don't resolve.
// sourceType is deliberately distinct from cutPiece()'s 'stock_piece_cut' — same piece id could in
// principle need both idempotency keys at different points in its life, though never both for the
// same piece in practice (a piece is either cut or dispatched whole, never both).
export async function postDispatchConsumption(pieces, company, username) {
  for (const p of pieces) {
    if (!(p.unit_cost > 0) || !company) continue;
    try {
      await postJournalEntry({
        company,
        entryDate: todayISO(),
        sourceType: 'stock_piece_dispatch',
        sourceId: p.id,
        description: `Material dispatched — piece ${p.code || p.id}`,
        lines: materialConsumptionLines({ amount: round2(p.unit_cost) }),
        createdBy: username,
      });
    } catch (err) { /* best-effort — a dispatch already durably recorded never rolls back for this */ }
  }
}
...
// Inward QC/Production Approval Workflow — the one place a piece moves out of the QC-inward hold.
// Mirrors confirmPieceReceipt()'s own CAS shape (a genuinely-held row is the only thing that can be
// released, so a double-approval can't double-count the rollup), then finishes exactly what
// maybeCreatePieceStock() would have done at receipt time had the piece not been held: reserve it
// against the very bom_item it was received for (allowClosedStatus:true — the line's own
// purchase_status is already 'Received' by construction, same reasoning reservePiece()'s own
// comment already documents for the non-held case).
export async function releasePieceFromInwardHold(pieceId, { projectId, bomItemId }) {
  const res = await execute(
    "UPDATE stock_pieces SET status = 'available' WHERE id = ? AND status = 'pending_qc_inward'",
    [pieceId]
  );
  if (res.changes !== 1) throw new Error('Piece is not pending QC inward review');
  const piece = await queryOne('SELECT inventory_item_id FROM stock_pieces WHERE id = ?', [pieceId]);
  await rollUpOnHand(piece.inventory_item_id);
  await reservePiece({ pieceId, projectId, bomItemId, allowClosedStatus: true });
  return { id: pieceId };
}
...
export async function transferPieceOwnership(pieceId, { toProjectId = null, reason, username }) {
  ...
  // Final Phase 0-7 audit gap-fix — a real, confirmed bug this guard prevents: releasePiece() below
  // is a no-op for anything but 'reserved', so a 'pending_qc_inward' piece would pass straight
  // through to the owner_project_id UPDATE with no other side effect — leaving its
  // inward_approvals.project_id (still the ORIGINAL project, unchanged by a transfer) pointing at a
  // piece now owned elsewhere. When QC later approves and releasePieceFromInwardHold() calls
  // reservePiece() with that original project id, reservePiece()'s own ownership guard would throw
  // ("belongs to another project"), inside an unguarded loop in the decide route — permanently
  // stuck mid-approval, no way to recover. Blocked outright rather than allowed: material still
  // awaiting its inward review isn't settled enough to transfer yet.
  if (piece.status === 'pending_qc_inward') {
    throw new Error("Can't transfer ownership — this piece is still pending QC inward review");
  }
...
export async function listPieces({ inventoryItemId, bomItemId }) {
  ...
  // owner_project_no (Phase 8, tying Ownership Transfer into the UI) — sp.owner_project_id was never
  // joined out anywhere before this; "For" (project_no above) reflects the RESERVATION project
  // (sp.project_id), a different, independent concept (Phase 0's own contract: Ownership ≠
  // Reservation) — a piece can be owned by one project while unreserved, or reserved without ever
  // having been explicitly owned (common-pool stock). `op` since `p`/`pr` are already taken.
  const select = `SELECT sp.*, tc.certificate_no AS certificate_no,
                          ...
                          op.project_no AS owner_project_no,
                     ...
                     LEFT JOIN projects op ON op.id = sp.owner_project_id
