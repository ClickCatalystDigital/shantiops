...
export function memoryKeys(line) {
  const tokens = String(line.material_description ?? '').toLowerCase().split(/[^a-z0-9]+/);
  const alias = tokens.filter(w => w.length >= 2).join(' ');
  const moc = normalizeMaterial(line.moc);
  // Single-digit tokens are dropped from the alias (existing memory is keyed that way) but they can be the whole difference
  // between two lines ("LUGS 1.5 SQ MM" / "LUGS 2.5 SQ MM" collided). When the size cell says nothing, they become the size key.
  const dropped = tokens.filter(w => w.length === 1 && /\d/.test(w)).join('');
  const size = normalizeMaterial(line.size_spec) || (dropped ? `#${dropped}` : '');
