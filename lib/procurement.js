...
import { getAssemblyRollupMap } from './data';
...
export async function addItemToDraftPO(bomItem, quote) {
  ...
  const rollupById = await getAssemblyRollupMap(bomItem.project_id);
  const qty = itemRollupQty(bomItem.qty_text, bomItem.assembly_id, rollupById) ?? 1;
...
// `byId` is optional — a loop caller processing many items in one project (matchProjectPlainStock)
// builds the rollup map once and passes it down to avoid re-querying bom_assemblies per item; a
// single-item caller omits it and this self-fetches, scoped to the item's own project.
export async function autoReserveFromStock(bomItem, username = 'system', byId = null) {
  ...
  const requested = itemRollupQty(bomItem.qty_text, bomItem.assembly_id, rollupById) ?? 0;
...
export async function matchProjectPlainStock(projectId, username = 'system') {
  ...
  // One rollup map for the whole project, not one per item — a Release BOM can process 100+ lines.
  const rollupById = await getAssemblyRollupMap(projectId);
  const results = [];
  for (const item of items) {
    const r = await autoReserveFromStock(item, username, rollupById);
