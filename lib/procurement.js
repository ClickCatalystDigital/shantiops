...
import { execute, queryOne, queryAll, nextCounterValue, withTransaction, getAppSetting, setAppSetting } from './db';
...
import { releasePiece } from './stock-pieces';
import { materialMismatchReason } from './match-utils';
...
// Stores/Inventory hardening Phase 3 — optional `tx`: when the caller is running inside its own
// transaction (reserveFromStock's TOCTOU fix, below), this participates in it instead of using the
// module-level connection; every other caller (matchAndReserve, which has its own established
// non-transactional precedent, unchanged) omits it and behaves exactly as before.
export async function cloneBomItemForSplit(bomItem, { qtyText, pendingReview = false }, tx = null) {
  const run = tx
    ? (sql, args) => tx.execute({ sql, args }).then(r => ({ lastId: r.lastInsertRowid }))
    : execute;
  const { lastId } = await run(
    `INSERT INTO bom_items (project_id, material_description, moc, size_spec, section, qty_text,
                             ...
                             pending_review, qty_resolved, pr_item_id, import_id, template_id,
                             drawing_id, named_parts_json, assembly_id, group_label, make, remarks)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [bomItem.project_id, bomItem.material_description, bomItem.moc, bomItem.size_spec, bomItem.section,
      ...
      // Stores/Inventory hardening Phase 1 — import_id/template_id complete demandOrigin()'s
      // (lib/bom-fields.mjs) coverage for this clone (pr_item_id was already carried, above).
      // drawing_id/named_parts_json also previously dropped: unlike the assembly duplicate route,
      // this function clones a *quantity split of the same line*, not a different branch — there's
      // no rationale for losing the drawing link or named-parts breakdown here.
      bomItem.import_id ?? null, bomItem.template_id ?? null,
      bomItem.drawing_id ?? null, bomItem.named_parts_json ?? null,
      // The clone stays in the same BOM node with the same heading/make/remarks — without assembly_id every partial
      // reserve or remnant match left an unassigned line that then blocked Release BOM. (Procurement/receipt state is
      // deliberately NOT copied: this is an explicit column list, not INSERT...SELECT.)
      bomItem.assembly_id ?? null, bomItem.group_label ?? null, bomItem.make ?? null, bomItem.remarks ?? null]
  );
...
// allowClosedStatus (Inward QC/Production Approval Workflow, default false — every existing caller
// unaffected) — the one legitimate exception to the closed-status guard below, same reasoning
// lib/stock-pieces.js's reservePiece() already documents for its own identical parameter: the
// system completing the link a receipt already implies (here: releaseScalarFromInwardHold in
// lib/bom-receiving.js, running once QC approves a delivery whose bom_item is already 'Received' by
// construction), not a human's manual reserve action against a line the business already closed.
export async function reserveFromStock({ inventoryItemId, bomItemId, qty, username, allowClosedStatus = false, source = 'manual' }) {
  ...
  // Stores/Inventory hardening gap-fix — the one real check missing at this codebase's actual choke
  // point (every Reserve path — manual, auto-match, and receipt-driven — funnels through this one
  // function). Never blocks on missing category/moc (that would misfire on the majority of real
  // rows, see materialMismatchReason's own comment); only on the two sides actively disagreeing.
  const mismatch = materialMismatchReason(bomItem, invItem);
  if (mismatch) throw new Error(`Can't reserve — ${mismatch}`);
  ...
  if (!allowClosedStatus && isClosedStatus(bomItem.purchase_status || DEFAULT_PURCHASE_STATUS)) {
  ...
  // Stores/Inventory hardening Phase 3 — TOCTOU fix. The whole "compute available -> decide
  // reserveQty/shortfall -> split if needed -> insert reservation" sequence now runs inside one
  // transaction, re-reading on_hand/active-reservations fresh here rather than trusting an
  // earlier, pre-transaction read — closes a real race where two concurrent calls could both see
  // the same available qty and both reserve it, overcommitting stock. Same withTransaction shape
  // the batch branch previously used only for its own allocation step, generalized here to cover
  // the split-clone decision too (previously computed outside any transaction, even for batch).
  return withTransaction(async tx => {
    // Reservation/Issue reconciliation fix — an already-issued PORTION of a partially-issued
    // reservation has already left on_hand (issueMaterial's own decrement), so counting its full
    // original qty here would double-subtract it and understate what's genuinely still available to
    // reserve. qty - qty_issued is this row's true outstanding commitment.
    const freshReserved = await tx.execute({
      sql: `SELECT COALESCE(SUM(qty - qty_issued), 0) AS reserved FROM inventory_reservations WHERE inventory_item_id = ? AND status = 'active'`,
      args: [inventoryItemId],
    });
    const freshOnHand = await tx.execute({ sql: 'SELECT on_hand FROM inventory_items WHERE id = ?', args: [inventoryItemId] });
    const available = (freshOnHand.rows[0]?.on_hand || 0) - (freshReserved.rows[0]?.reserved || 0);
    ...
    if (shortfall > 0) {
      ...
      // what was just reserved), not a per-instance base one; a future re-read must never re-apply
      // the rollup multiplier on top of it (that's the split-qty double-counting bug this column
      // fixes).
      await tx.execute({ sql: 'UPDATE bom_items SET qty_text = ?, qty_resolved = 1 WHERE id = ?', args: [remainingQtyText, bomItemId] });
      targetBomItemId = await cloneBomItemForSplit(bomItem, { qtyText: reservedQtyText }, tx);
    }

    const ins = await tx.execute({
      sql: `INSERT INTO inventory_reservations (inventory_item_id, bom_item_id, qty, status, created_by, source) VALUES (?, ?, ?, 'active', ?, ?)`,
      args: [inventoryItemId, targetBomItemId, reserveQty, username, source],
    });
    const reservationId = Number(ins.lastInsertRowid);

    // Batch-tracked (Phase 3, §3.2): a batch is a pool that may need splitting across several
    // physical batches/heats — allocate inside this same transaction (issueBatch's own CAS guard
    // applies later, at issue time; allocation itself just needs the read-then-insert above and
    // this allocation to be atomic together, same lesson as Phase 0's cutPiece).
...
// for its own bom_item — Issue against that bom_item must draw this down first, never treat it as
// generic unavailable-elsewhere stock the way the pre-fix general floor check did (the confirmed
// self-block: a bom_item's own auto-reserved-at-receipt qty made every later Material Indent release
// against that same line throw "Insufficient stock", since the floor check counted that reservation
// as belonging to someone else). Claims this bom_item's own active reservations only, oldest row
// first, CAS-guarded per row so a concurrent claim against the same row can't be lost. Never touches
// on_hand or bom_items.purchase_status — the caller (issueMaterial) still owns both of those, exactly
// once, for the full requested qty regardless of how much of it came from a reservation vs the
// general pool.
export async function consumeReservedStock({ bomItemId, qty }) {
  ...
  for (const r of rows) {
    ...
    const rowRemaining = Number(r.qty) - Number(r.qty_issued);
    if (!(rowRemaining > 0)) continue; // already fully claimed by a concurrent request
    ...
    // A concurrent claim on this exact row lost the CAS (changes === 0) — its qty_issued moved
    // since we read it. Move on to the next row rather than retry; the caller's own shortfall
    // handling covers whatever this call couldn't claim.
  }
...
export async function issueReservation(reservationId, { username, jobCardId } = {}) {
  ...
  // Reservation/Issue reconciliation fix — this row may already have been partially drawn down by a
  // Material Indent release (consumeReservedStock) since it was created; "Issue" from this button
  // means "issue whatever of it is still outstanding," not the row's original full qty a second time.
  const remaining = Number(res.qty) - Number(res.qty_issued);
  ...
  // CAS, same discipline as consumeReservedStock's own per-row claim — a concurrent partial Material
  // Indent release against this exact row must not be silently clobbered by this full-issue claim.
  const claim = await execute(
    `UPDATE inventory_reservations SET qty_issued = qty, status = 'issued', issued_at = CURRENT_TIMESTAMP
      WHERE id = ? AND status = 'active' AND qty_issued = ?`,
    [reservationId, res.qty_issued]
  );
  if (claim.changes !== 1) throw new Error('Reservation changed concurrently — reload and try again');
  ...
  if (invItem?.tracking_mode === 'batch') {
    ...
    await consumeStock({
      trackingMode: 'batch', inventoryItemId: res.inventory_item_id, qty: remaining,
      bomItemId: res.bom_item_id, jobCardId, username, existingAllocations: activeAllocations,
    });
  } else {
    await execute('UPDATE inventory_items SET on_hand = on_hand - ? WHERE id = ?', [remaining, res.inventory_item_id]);
  }
  await execute(
...
export async function releaseReservationsForItem(bomItemId) {
  ...
  // Stores/Inventory hardening Phase 3 — a dimensional line matched automatically
  // (lib/remnant-match.js's matchAndReserve) reserves a real stock_pieces row directly, never
  // creating an inventory_reservations row either — a second, previously-unhandled blind spot this
  // function had (only the indent-linked reserve-piece path was released, via the indent's own
  // cascade elsewhere). Without this, cancelling a bom_item left its matched piece permanently
  // 'reserved' against a line that no longer exists. releasePiece() correctly clears
  // bom_item_id/indent_item_id and returns the piece to 'available' — deliberately NOT the
  // reopen-demand/notify behavior the scalar releaseReservation() path has, since a cancelled
  // line's demand is intentionally being killed, not reopened elsewhere (that distinction matters
  // for Phase 4's own, different use of releasePiece()).
  const reservedPieces = await queryAll("SELECT id FROM stock_pieces WHERE bom_item_id = ? AND status = 'reserved'", [bomItemId]);
  for (const p of reservedPieces) await releasePiece(p.id);
}
...
export async function setAllocationMode(mode) {
  ...
  await setAppSetting('stores_allocation_mode', value);
  return value;
}

// Remnant-match tolerances — category-specific, since a category's dimensional identity isn't
// always "L/W/thickness": plate is the one category with a real measured-thickness variance
// (rolling tolerance); angle/beam/channel/pipe are matched on an exact profile designation
// (lib/remnant-match.js's findCandidates, the `else` branch) with no measured-tolerance concept at
// all, so they carry no entry here. A generic global L/W/thickness knob was explicitly rejected —
// see the conversation this shipped from — in favor of per-category config that only exists where
// it's meaningful; add a category's own key here (not a new generic field) if one is ever needed.
const DEFAULT_REMNANT_MATCH_TOLERANCES = { plate: { thickness_mm: 0.3 } };
export async function getRemnantMatchTolerances() {
  const raw = await getAppSetting('remnant_match_tolerances');
  if (!raw) return DEFAULT_REMNANT_MATCH_TOLERANCES;
  try {
    return { ...DEFAULT_REMNANT_MATCH_TOLERANCES, ...JSON.parse(raw) };
  } catch {
    return DEFAULT_REMNANT_MATCH_TOLERANCES;
  }
}
export async function setPlateThicknessTolerance(mm) {
  const value = Number(mm);
  if (!(value >= 0)) throw new Error('Tolerance must be a non-negative number');
  const current = await getRemnantMatchTolerances();
  const next = { ...current, plate: { ...current.plate, thickness_mm: value } };
  await setAppSetting('remnant_match_tolerances', JSON.stringify(next));
  return next;
}
...
export async function autoReserveFromStock(bomItem, username = 'system', byId = null, unitCount = null) {
  ...
  const invItem = await queryOne(
    // Reservation/Issue reconciliation fix — qty - qty_issued, same reasoning as reserveFromStock's
    // own available computation above.
    `SELECT i.id, i.on_hand - COALESCE((SELECT SUM(r.qty - r.qty_issued) FROM inventory_reservations r
  ...
  let result;
  try {
    result = await reserveFromStock({ inventoryItemId: invItem.id, bomItemId: bomItem.id, qty: requested, username, source: 'auto' });
  } catch (err) {
  ...
  // Stores/Inventory hardening Phase 1 — the one real audit gap found auditing this round: manual
  // Reserve and split-order Allocate both already write a usb_audit row, this (Auto-allocation-mode)
  // path didn't, so the per-line stock commitment itself had no record beyond the umbrella
  // pr_raised/bom_released events. Mirrors the manual-reserve route's own call shape.
  await audit('bom_item_auto_reserved', {
    actor: username,
    detail: `bom_item #${result.bomItemId} <- inventory_item #${invItem.id}: ${result.reservedQty}${result.shortfall ? ` (shortfall ${result.shortfall})` : ''}`,
  });
