...
// Sale Order discount as an amount (plan 1h): the amount wins, % is derived, clamped to subtotal.
const byAmt = computeSaleOrderTotals({ items: [{ amount: 1000, item_tax_pct: 18 }], discountAmount: 100, discountPct: 50, companyStateCode: '36', customerStateCode: '36' });
assert.equal(byAmt.discountAmount, 100); assert.equal(byAmt.discountPct, 10); assert.equal(byAmt.cgst, 81); assert.equal(byAmt.total, 1062);
assert.equal(computeSaleOrderTotals({ items: [{ amount: 1000, item_tax_pct: 0 }], discountAmount: 5000 }).discountAmount, 1000);
assert.equal(computeSaleOrderTotals({ items: [{ amount: 1000, item_tax_pct: 0 }], discountPct: 5 }).discountPct, 5);
