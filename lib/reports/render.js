...
export const CALIBRATION_STATUS_COLS = [
  ['Type', 12, (i) => i.type === 'jig_fixture' ? 'Jig/Fixture' : 'Instrument'],
  ['Name', 24, (i) => i.name],
  ['Identifier', 16, (i) => i.identifier || '—'],
  ['Last Calibrated', 14, (i) => i.last_calibrated_on ? fmtDate(i.last_calibrated_on) : '—'],
  ['Due Date', 14, (i) => i.due_date ? fmtDate(i.due_date) : '—'],
  ['Status', 12, (i) => i.status.replace('_', ' ')],
];
export function calibrationStatusTable(result) {
  return { cols: CALIBRATION_STATUS_COLS, rows: result.items };
}

export const JOB_WORK_INSPECTION_REGISTER_COLS = [
  ['Project', 12, (j) => j.project_no],
  ['Job Worker', 20, (j) => j.job_worker_name],
  ['Sent', 12, (j) => fmtDate(j.sent_date)],
  ['Expected Return', 12, (j) => j.expected_return_date ? fmtDate(j.expected_return_date) : '—'],
  ['Sent Qty', 10, (j) => j.sent_qty ?? '—', 'right'],
  ['Received Qty', 10, (j) => j.received_qty ?? '—', 'right'],
  ['Variance', 10, (j) => j.variance_qty ?? '—', 'right'],
  ['Received', 12, (j) => j.received_date ? fmtDate(j.received_date) : '—'],
  ['Result', 10, (j) => j.result],
];
export function jobWorkInspectionRegisterTable(result) {
  return { cols: JOB_WORK_INSPECTION_REGISTER_COLS, rows: result.lines };
}
