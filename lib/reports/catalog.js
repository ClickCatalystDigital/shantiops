...
// never ahead of it.
//
// Entry shape: { key, title, department, compute, toTable, totals?, subtitle?, heavy?, needsCompany?, hasOwnControls? }
...
// - hasOwnControls: true means this entry has no compute/toTable pair at all — its Screen component
//   (components/ReportsWorkspace.jsx's SCREEN map) renders itself entirely, gets the raw data props
//   passed straight through (see app/reports/page.js's `crmData`), and never gets a PDF button (the
//   export route 400s if hit directly). Used by the 6 CRM analytics reports below and by
//   app/reports/page.js's own Management reports (kept out of this catalog on purpose — no
//   department to file them under).
...
import { computeEwayBillRegister } from '@/app/api/reports/eway-bill-register/route.js';
import { computeFreightCostSummary } from '@/app/api/reports/freight-cost-summary/route.js';
import { computeDispatchAging } from '@/app/api/reports/dispatch-aging/route.js';
import { computeTestCertificateRegister } from '@/app/api/reports/test-certificate-register/route.js';
import { computeQcInspectionSummary } from '@/app/api/reports/qc-inspection-summary/route.js';
import { computeNcrRegister } from '@/app/api/reports/ncr-register/route.js';
import { computeCalibrationStatus } from '@/app/api/reports/calibration-status/route.js';
import { computeJobWorkInspectionRegister } from '@/app/api/reports/job-work-inspection-register/route.js';
import {
  ...
  ewayBillRegisterTable, freightCostSummaryTable, dispatchAgingTable,
  testCertificateRegisterTable, qcInspectionSummaryTable, ncrRegisterTable,
  calibrationStatusTable, jobWorkInspectionRegisterTable,
} from './render.js';
...
export const REPORTS = [
  ...
  // CRM analytics (2026-08-23, SYSTEM.md §5an) — folded in from the standalone /crm-reports tab so
  // Sales/Marketing's Reports entry is one real catalog department like every other, not a second
  // "Reports" tab pointing somewhere else. hasOwnControls: true — see the note above, no PDF export.
  { key: 'sales_pipeline', title: 'Sales Pipeline', department: 'Sales', hasOwnControls: true },
  { key: 'by_department', title: 'By Department', department: 'Sales', hasOwnControls: true },
  { key: 'agent_performance', title: 'Agent Performance', department: 'Sales', hasOwnControls: true },
  { key: 'lead_funnel', title: 'Lead Funnel', department: 'Marketing', hasOwnControls: true },
  { key: 'leads_by_source', title: 'Leads by Source', department: 'Marketing', hasOwnControls: true },
  { key: 'campaign_performance', title: 'Campaign Performance', department: 'Marketing', hasOwnControls: true },
  // Sales CRM expansion Phase 5 (13 reports, "Enquiry Analysis Management Report" group + the
  // remaining 8) — client-rendered off the same crmData fetch app/reports/page.js's getCrmData()
  // widened for this phase, same hasOwnControls idiom as the 6 CRM analytics reports above (a real,
  // deliberate simplification vs. the plan's own suggestion to make 2 of these SQL-backed catalog
  // entries with their own PDF export — every one of these 13 already works fully on-screen, and a
  // second export pipeline wasn't worth building for the marginal PDF-download nicety).
  { key: 'sales_call_prospect_summary', title: 'Sales Call Prospect Summary', department: 'Sales', hasOwnControls: true },
  { key: 'sales_call_date_wise', title: 'Date-wise Sales Call', department: 'Sales', hasOwnControls: true },
  { key: 'sales_call_location_wise', title: 'Location-wise Sales Call', department: 'Sales', hasOwnControls: true },
  { key: 'sales_overview', title: 'Sales Overview', department: 'Sales', hasOwnControls: true },
  { key: 'order_book', title: 'Order Book & Collections', department: 'Sales', hasOwnControls: true },
  { key: 'employee_performance_360', title: 'Employee Performance 360', department: 'Sales', hasOwnControls: true },
  { key: 'competitor_analysis', title: 'Competitor Analysis', department: 'Sales', hasOwnControls: true },
  { key: 'sales_call_funnel', title: 'Sales Call Funnel', department: 'Sales', hasOwnControls: true },
  { key: 'neglected_sales_call', title: 'Neglected Sales Call', department: 'Sales', hasOwnControls: true },
  { key: 'customer_follow_up', title: 'Customer Follow-up', department: 'Sales', hasOwnControls: true },
  { key: 'client_feedback', title: 'Client Feedback', department: 'Sales', hasOwnControls: true },
  { key: 'employee_follow_up', title: 'Employee Follow-up', department: 'Sales', hasOwnControls: true },
  { key: 'quotation_listing', title: 'Quotation Listing', department: 'Sales', hasOwnControls: true },
  { key: 'feedback_not_responded', title: 'Feedback Not Responded', department: 'Sales', hasOwnControls: true },
  { key: 'feedback_response', title: 'Feedback Response', department: 'Sales', hasOwnControls: true },
  { key: 'employee_expense', title: 'Employee-wise Expense', department: 'Sales', hasOwnControls: true },
  { key: 'sales_call_customize', title: 'Customize Sales Call List', department: 'Sales', hasOwnControls: true },
  {
  ...
  {
    key: 'eway-bill-register',
    title: 'E-Way Bill Register',
    department: 'Dispatch',
    compute: computeEwayBillRegister,
    toTable: ewayBillRegisterTable,
    totals: (result) => [['E-Way Bills', String(result.total)]],
  },
  {
    key: 'freight-cost-summary',
    title: 'Freight Cost Summary',
    department: 'Dispatch',
    compute: computeFreightCostSummary,
    toTable: freightCostSummaryTable,
    totals: (result) => [
      ['Total Freight', fmt(result.totalFreight)],
      ['Paid by Us', fmt(result.byUs)],
      ['Paid by Customer', fmt(result.byCustomer)],
    ],
  },
  {
    key: 'dispatch-aging',
    title: 'Pending vs Dispatched Aging',
    department: 'Dispatch',
    compute: computeDispatchAging,
    toTable: dispatchAgingTable,
    totals: (result) => [['Pending Shipments', String(result.total)]],
  },
  {
    key: 'test-certificate-register',
    title: 'Test Certificate Register',
    department: 'QC',
    orientation: 'landscape', // 10 columns of mechanical properties, needs the width
    compute: computeTestCertificateRegister,
    toTable: testCertificateRegisterTable,
    totals: (result) => [['Certificates', String(result.total)]],
  },
  {
    key: 'qc-inspection-summary',
    title: 'Inspection Pass/Fail Summary',
    department: 'QC',
    compute: computeQcInspectionSummary,
    toTable: qcInspectionSummaryTable,
    totals: (result) => [
      ['Pass', String(result.totalPass)],
      ['Fail', String(result.totalFail)],
      ['Pending', String(result.totalPending)],
    ],
  },
  {
    key: 'ncr-register',
    title: 'NCR Register',
    department: 'QC',
    compute: computeNcrRegister,
    toTable: ncrRegisterTable,
    totals: (result) => [
      ['Total NCRs', String(result.total)],
  ...
  },
  {
    ...
    department: 'QC',
    needsCompany: false, // equipment bank, not project/company-scoped — same reasoning as Stock Valuation
    compute: computeCalibrationStatus,
    toTable: calibrationStatusTable,
    totals: (result) => [
      ['Expired', String(result.expired)],
      ['Due Soon', String(result.due_soon)],
      ['OK', String(result.ok)],
      ['Blocked', String(result.blocked)],
    ],
  },
  {
    key: 'job-work-inspection-register',
    title: 'Job-Work Inspection Register',
    department: 'QC',
    orientation: 'landscape', // job worker name + 3 date columns is tight on portrait
    compute: computeJobWorkInspectionRegister,
    toTable: jobWorkInspectionRegisterTable,
    totals: (result) => [
      ['Sent Qty', String(result.totalSent)],
      ['Received Qty', String(result.totalReceived)],
      ['Variance', String(result.totalVariance)],
    ],
  },
  {
