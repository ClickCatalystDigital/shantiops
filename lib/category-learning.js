...
// remember=false for bulk sweeps whose categories may be machine guesses: only a person's own pick may be remembered.
export async function learnCategoryIfConfirmed(description, category, confirmedBy, { remember = true } = {}) {
  if (!description || !category) return false;
  if (remember) await rememberCategory(description, category, confirmedBy);
