...
async function resolveJobCard(full) {
  const row = await queryOne(
    `SELECT jc.id, jc.project_id, jc.job_number, p.project_no,
            (SELECT COUNT(*) FROM job_sheet_stages s WHERE s.sheet_id = jc.id AND s.end_date IS NOT NULL) AS done,
            (SELECT COUNT(*) FROM job_sheet_stages s WHERE s.sheet_id = jc.id) AS total
       FROM job_sheets jc LEFT JOIN projects p ON p.id = jc.project_id WHERE jc.jc_no = ?`, [full]);
  if (!row) return null;
  return {
    // Job Cards render on /production/shop (WorkersPanel.jsx), not the project page — the
    ...
    type: 'job_card', id: row.id, label: full, href: withHighlight('/production/shop?tab=jobcards', full), project_no: row.project_no,
    detail: { status: `${row.done}/${row.total} stages`, meta: [
      { label: 'Job number', value: row.job_number },
    ] },
  };
}
...
async function resolveWorkOrder(full) {
  ...
  if (!row) return null;
  return {
    // No Work Orders screen on Shop Floor anymore — plain text, same href:null treatment as GRN.
    type: 'work_order', id: row.id, label: full, href: null, project_no: row.project_no,
...
async function resolveIndent(full) {
  const row = await queryOne(
    `SELECT mi.id, mi.status, mi.requested_by, p.project_no
       FROM material_indents mi LEFT JOIN projects p ON p.id = mi.project_id WHERE mi.indent_no = ?`, [full]);
  if (!row) return null;
  return {
    // Stores' own "Material Indents" tab is the one screen that actually lists indents by number.
    type: 'material_indent', id: row.id, label: full, href: withHighlight('/stores?tab=indents', full), project_no: row.project_no,
    detail: { status: row.status, meta: [
...
async function resolveBomItem(full) {
  ...
  if (!row) return null;
  return {
    // Project View redesign — full BOM editing lives on /engineering now (the unified Project View
    // only shows a read-only, collapsed reference copy), so this points there instead of the old
    // per-department project-page tab. ?project= pre-selects it (app/engineering/page.js's own
    // initialProject, same pattern the ?tab= deep link already used).
    type: 'bom_item', id: row.id, label: full,
    href: row.project_id ? withHighlight(`/engineering?tab=structure&project=${row.project_id}`, full) : null,
...
async function resolveDrawing(full) {
  ...
  if (!row) return null;
  return {
    // Project View redesign — the canonical Drawings card is still on the unified project page
    // (ProjectDesignRow.jsx), just no longer behind a ?dept= tab selection, so the param is dropped.
    type: 'drawing', id: row.id, label: full,
    href: row.project_id ? withHighlight(`/projects/${row.project_id}`, full) : null,
    project_no: row.project_no,
    detail: { status: row.status, meta: [
...
async function resolveGatePass(full) {
  ...
  if (!row) return null;
  return {
    // Stores IA redesign — Gate Passes moved to Dispatch's own workspace.
    type: 'gate_pass', id: row.id, label: full, href: withHighlight('/dispatch?tab=gatepasses', full),
    detail: { status: row.status, meta: [
...
export async function searchEntityRefs(type, q, user) {
  ...
  switch (type) {
    ...
    case 'material_indent':
      return queryAll(
        `SELECT mi.indent_no AS code, mi.indent_no || ' · ' || COALESCE(mi.requested_by, 'Unknown') AS label, p.project_no
           FROM material_indents mi LEFT JOIN projects p ON p.id = mi.project_id
          WHERE mi.indent_no LIKE ? OR p.project_no LIKE ?
          ORDER BY mi.id DESC LIMIT ${LIMIT}`, [needle, needle]);
    case 'bom_item':
      return queryAll(
