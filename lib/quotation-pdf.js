...
const COLS = [['S.No', 5], ['Description', 32], ['HSN', 8], ['Qty', 7], ['UoM', 7], ['Rate', 11], ['Disc %', 9], ['Rate after Disc', 11], ['Amount', 10]];
...
function Row({ it, i }) {
  ...
  const vals = [i + 1, it.item_description, it.hsn_code, it.qty, it.uom, fmt(rate),
    discountPct ? `${discountPct}%` : '—', fmt(rateAfterDiscount), fmt(it.amount)];
  return (
