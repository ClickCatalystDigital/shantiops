...
// Multi-unit split — a split child inherits the FULL milestone template (createProjectMilestones has
// no per-child subset), but its own copies of these 9 keys (Design's 4 + Procurement's 5) are
// structurally never actionable at the child level: that work happens once, on the master's own BOM/
// drawings, not per physical unit. getCustomerViewSplitOrder() (below) already reads Design straight
// from the master and derives Procurement from the allocation/routing board instead of trusting a
// child's own orphaned copies — this is that same classification, exported once so
// getProjectsWithStatus()/getProjectDetail() (lib/data.js) can exclude these 9 keys from a split
// child's own roll-up too, instead of counting milestones that can never close.
export const MASTER_LEVEL_MILESTONE_KEYS = [
  ...CUSTOMER_PHASES.find(ph => ph.key === 'design').keys,
  ...CUSTOMER_PHASES.find(ph => ph.key === 'procurement').keys,
];
