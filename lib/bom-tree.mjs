...
// The exact set of nodes whose OWN name matches — as opposed to expandedIdsForSearch's return
// value, which also includes every ancestor of a match (needed to reveal it, but not itself a
// match to highlight). Read-only Final BOM card only: highlighting an ancestor that merely
// contains a match would be misleading, and the editable tree's own search never highlighted
// anything to begin with, so this stays a separate function rather than widening the one above.
export function matchingNodeNameIds(query, assemblies) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return new Set();
  return new Set(assemblies.filter(a => a.name.toLowerCase().includes(q)).map(a => a.id));
}
...
// What a node that was built from a Structure Template should say about it (NodeOverviewTab's hint).
// `stamped` = bom_assemblies.structure_template_version (NULL = applied before versioning), `current` =
// the template's live version, `found` = false when the template is no longer in the active list
// (archived/removed). Applied BOMs are independent copies, so 'stale' only informs — it never implies
// the node changed or must be redone.
//   'removed' — template archived/deleted since   'unknown' — stamped before versioning existed
//   'stale'   — template has a newer version      'current' — up to date
export function templateVersionState({ stamped, current, found }) {
  if (!found) return 'removed';
  if (stamped == null || current == null) return 'unknown';
  return current > stamped ? 'stale' : 'current';
}
