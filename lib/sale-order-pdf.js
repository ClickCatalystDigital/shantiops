...
const s = StyleSheet.create({
  ...
  cell: { paddingVertical: 3, paddingHorizontal: 3, borderRightWidth: 1, borderColor: '#ddd', fontSize: 7 },
...
// Sales CRM plan 1h — the same columns as the wizard's Items On Order table. Amount is the line's
// pre-tax value after its own discount; tax is shown in the totals block.
const COLS = [['SL', 4], ['Code', 9], ['Description', 22], ['Warranty Std/Acc (days)', 9], ['From (D/I)', 5], ['Inst Req', 5],
  ['Prev. Maint.', 9], ['Qty', 7], ['Unit Price', 10], ['Disc %', 5], ['Tax %', 5], ['Amount', 10]];
...
function Row({ it, i }) {
  const warranty = it.warranty_std_days || it.warranty_accepted_days ? `${it.warranty_std_days ?? '—'} / ${it.warranty_accepted_days ?? '—'}` : '—';
  const vals = [i + 1, it.product_code || '—', it.item_description, warranty, it.from_date_of || '—', it.installation_required ? 'Yes' : 'No',
    it.preventive_maintenance || '—', [it.qty, it.uom].filter(Boolean).join(' ') || '—',
    it.rate != null ? fmt(it.rate) : '—', it.discount_pct ? `${it.discount_pct}%` : '—', it.item_tax_pct ? `${it.item_tax_pct}%` : '—', fmt(it.amount)];
  return (
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.metaRow}>
          <View style={s.metaCol}>
            ...
            {so.address_type === 'Other' && so.order_address
              ? <Text>{so.order_address}</Text>
              : (ref.customer_address || []).map((l, i) => <Text key={i}>{l}</Text>)}
            {so.contact_person && <Text>Contact: {so.contact_person}{so.contact_mobile ? ` (${so.contact_mobile})` : ''}</Text>}
            <View style={{ marginTop: 4 }}>
              <Meta label="Customer Code" value={ref.customer_code} />
              <Meta label="Customer PAN" value={ref.customer_pan} />
              <Meta label="Customer GST No" value={ref.customer_gst_no} />
            </View>
          </View>
          <View style={s.metaCol}>
            ...
            <Meta label="Our GST No" value={ref.company_gstin} />
            <Meta label="Entity Code" value={ref.entity_code} />
            <Meta label="Our PAN" value={ref.company_pan} />
            <Meta label="SOS No" value={(ref.sos_numbers || []).join(', ')} />
            <Meta label="A/C Manager" value={so.sales_person_override} />
          </View>
        </View>
