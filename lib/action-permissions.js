...
export const ACTION_CATALOG = {
  Engineering: [
    ...
    { key: 'engineering.bom.link_item', label: 'Link a BOM item to the Item Master catalog' },
    { key: 'engineering.item_master.write', label: 'Add or edit an Item Master catalog record' },
    ...
    { key: 'engineering.bom.clear', label: 'Delete an entire BOM (every node and item)' },
  ...
  Procurement: [
    ...
    { key: 'procurement.po.delivery_lots.write', label: 'Create or edit a PO delivery lot' },
    { key: 'procurement.custom_item.add', label: 'Add a custom item (not from any PMB/PR)' },
    { key: 'procurement.delivery_followup.write', label: 'Add a follow-up note on an overdue delivery' },
  ...
  Stores: [
    { key: 'stores.bom.receive', label: 'Receive a BOM item (mark it Received)' },
    { key: 'stores.bom.allocate', label: 'Allocate received material to a child unit project' },
    { key: 'stores.bom.route', label: 'Route allocated material to Production or Dispatch for a unit' },
    { key: 'stores.indent.release', label: 'Release or reserve against a material indent' },
    ...
    { key: 'stores.piece.transfer_ownership', label: "Transfer a stock piece's ownership to another project or the common pool" },
    ...
    // Gates exactly one field on the existing Item Master route (app/api/item-master/[id]/route.js)
    // — items.default_requires_manufacturing, the same field Engineering's own Item Master edit UI
    // already writes. Never widens Stores' access to any other Item Master field (gentle-
    // snuggling-wozniak.md §8/§10).
    { key: 'stores.bom.set_manufacturing_default', label: "Correct a catalog item's manufacturing default" },
  ],
  QC: [
    { key: 'qc.jobsheet.sign', label: 'QC-sign a Job Card stage' },
    ...
    { key: 'qc.ncr.write', label: 'Raise or edit an NCR' },
    { key: 'qc.ncr.disposition', label: 'Disposition an NCR (rework / repair / scrap / use-as-is)' },
    { key: 'qc.ncr.verify', label: 'QC-verify a dispositioned NCR before it can close' },
    { key: 'qc.ncr.close', label: 'Close an NCR' },
    { key: 'qc.hold.release', label: 'Release a QC hold point on a job card' },
    { key: 'qc.inward.decide', label: 'Approve or reject received material (Inward Review)' },
    { key: 'qc.predispatch.decide', label: 'Approve or reject a Packing List (Pre-Dispatch Review)' },
  ],
  Dispatch: [
    ...
    { key: 'dispatch.packing.eway_bill.generate', label: 'Generate an e-way bill for a packing list' },
    { key: 'dispatch.packing.eway_bill.cancel', label: 'Cancel a generated e-way bill' },
    { key: 'dispatch.packing.acknowledge', label: 'Log a delivery acknowledgment' },
    { key: 'dispatch.packing.submit_approval', label: 'Submit a Packing List for QC/Production review' },
  ],
  Installation: [
  ...
  Production: [
    { key: 'production.jobsheet.write', label: 'Create or edit a Job Card sheet and its stages' },
    ...
    { key: 'production.indent.create', label: 'Raise a material indent' },
    ...
    { key: 'production.predispatch.decide', label: 'Approve or reject a Packing List (Pre-Dispatch Review)' },
  ],
  ...
  Sales: [
    ...
    { key: 'sales.payment.write', label: 'Log a payment received against a Sale Order' },
    ...
    // Sales CRM expansion (2026-09-22) — Branches/Product Master/Sales Targets/Diary.
    { key: 'sales.branch.write', label: 'Add or edit a Branch' },
    { key: 'sales.product.write', label: 'Add or edit a Product Master entry' },
    { key: 'sales.target.write', label: 'Set a Sales Target' }, // seeded Head-only, migrate()
    { key: 'sales.diary.write', label: 'Log a Diary entry' },
    // requireCrmAction only grants an action to whichever of Sales/Marketing the caller holds when
    // that department's own bucket lists the key (2026-09-23 isolation fix) — Campaigns is a
    // genuinely shared CRM master (salesOnly:false in SalesWorkspace.jsx), so Sales needs its own
    // listing here even though the write route itself lives under Marketing's real ownership.
  ...
  Marketing: [
    ...
    { key: 'crm.task.create', label: 'Create a CRM task' },
    { key: 'crm.task.write', label: 'Edit or complete a CRM task' },
    // Same reasoning as marketing.campaign.write above, mirrored: Branches/Products are shared CRM
    // masters (Masters group, salesOnly:false) their own route comments already call "CRM-wide" —
    // Marketing needs these listed here or requireCrmAction denies it outright.
    { key: 'sales.branch.write', label: 'Add or edit a Branch' },
    { key: 'sales.product.write', label: 'Add or edit a Product Master entry' },
  ],
  ...
  Accounts: [
    ...
    // Payment reconciliation only — marking an invoice paid and recording the payment reference.
    // Deliberately narrower than sales.invoice.status: Sales/Marketing keep sole ownership of the
    // commercial/issuance fields (draft/issued/cancelled, due_date, notes) — confirmed ownership
    // split, not a full hand-off of invoice management to Accounts.
    { key: 'accounts.invoice.mark_paid', label: 'Mark a Sales Invoice paid and record the payment reference' },
    // Same ownership split as above, for the two existing settlement routes that already render an
    // Accounts-facing UI (AccountsWorkspace's AR/AP settlement card) but only ever accepted Sales/
    // Marketing/PM (receipts) or Procurement/PM (payments) as the actor — Accounts got a 403 on its
    // own workspace's own button. Recording a receipt/payment never touches an invoice's or bill's
    // commercial fields (just amount/date/reference), so no field-level split is needed here.
    { key: 'accounts.invoice.receipt.write', label: 'Record a receipt against a Sales Invoice' },
    { key: 'accounts.vendor_bill.payment.write', label: 'Record a payment against a Vendor Bill' },
    // Standing, low-frequency, high-consequence action (a new legal entity, its own GL/invoice
    // series/statutory registration) — seeded Head-only in migrate(), same precedent as
    // engineering.ecn.approve/qc.ncr.disposition. Deliberately a separate key from the routinely-
    // open accounts.company_settings.write (editing an existing company's fields).
    { key: 'accounts.company.create', label: 'Create a new company (legal entity)' },
  ],
};
...
// against whichever of the two the user actually has AND whose own ACTION_CATALOG bucket lists
// this actionKey — Sales first (matches real ownership). Fixed 2026-09-23: this used to resolve to
// "whichever department the user holds, Sales first" unconditionally, then trust
// canPerformAction's open-by-default — but canPerformAction has no notion of "this key doesn't
// belong to this department," only "open" or "open-to-Head-only," so a Marketing-only caller
// silently passed every Sales-exclusive action (create/convert a quotation, edit a customer, set a
// Sales Target with no Head check at all) since no row was ever seeded to deny it. Now: a caller
// who holds only the department that lacks this key in its own catalog is denied outright, not
// silently treated as if they held the other department.
export async function requireCrmAction(user, actionKey) {
  if (isPM(user)) return requireAction(user, 'Sales', actionKey);
  const dept = canAccessDepartment(user, 'Sales') && (ACTION_CATALOG.Sales || []).some(a => a.key === actionKey) ? 'Sales'
    : canAccessDepartment(user, 'Marketing') && (ACTION_CATALOG.Marketing || []).some(a => a.key === actionKey) ? 'Marketing'
    : null;
  if (!dept) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  return requireAction(user, dept, actionKey);
}

// Same pattern for BOM add/import routes — Design got the same BOM-entry capability as Engineering
// (2026-08-25), so these routes gate on whichever of the two the user actually has.
export async function requireBomAction(user, actionKey) {
  const dept = canAccessDepartment(user, 'Engineering') ? 'Engineering' : 'Design';
  return requireAction(user, dept, actionKey);
}

// The BOM assembly tree / bom-items CRUD / ECN surface (app/api/bom-assemblies, bom-items,
// engineering-change-notes) is fully shared between Design and Engineering — a Design head gets
// the same write access as an Engineering head, not just the read-only workspace view. Unlike
// requireBomAction, the action_permissions lookup always stays keyed to 'Engineering' (the only
// bucket these keys live in, ACTION_CATALOG.Engineering) rather than switching to 'Design' —
// switching would silently drop the one real Head gate on this surface (engineering.ecn.approve)
// the moment a Design-only, non-head caller hit it, since no action_permissions row exists under
// department='Design' to enforce it. A Head of either department satisfies a Head-gated action here.
export async function requireEngineeringAction(user, actionKey) {
  ...
  const row = await queryOne(
    'SELECT requires_head FROM action_permissions WHERE department = ? AND action_key = ?',
    ['Engineering', actionKey]
  );
  if (!row?.requires_head) return null;
  if (isDepartmentHead(user, 'Engineering') || isDepartmentHead(user, 'Design')) return null;
  return NextResponse.json({ error: 'Forbidden — this action requires the department Head' }, { status: 403 });
}
