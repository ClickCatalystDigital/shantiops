...
import { fieldsFromSizeSpec } from './section-shapes.js';
import assert2 from 'node:assert/strict';
assert2.deepEqual(fieldsFromSizeSpec('plate', '1250X2500X3.15 MM THICK'), { thickness: 3.15, length: 2500, width: 1250 });
assert2.deepEqual(fieldsFromSizeSpec('plate', '8 MM THK X 500 X 1200'), { thickness: 8, length: 1200, width: 500 });
assert2.deepEqual(fieldsFromSizeSpec('flat', '30 X 5 mm'), { width: 30, thickness: 5 });
assert2.deepEqual(fieldsFromSizeSpec('square', 'SQ.10x10- 5000 LG.'), { side: 10, length: 5000 });
assert2.deepEqual(fieldsFromSizeSpec('plate', 'AS PER DRAWING'), {});
console.log('fieldsFromSizeSpec ok');
assert2.deepEqual(fieldsFromSizeSpec('round', 'ø45 x 700 Lg ø63 x 65 Lg'), {});
assert2.deepEqual(fieldsFromSizeSpec('channel', 'ISMC 100 x 50 ISA50x50 X5thk'), {});
assert2.deepEqual(fieldsFromSizeSpec('round', 'Φ63.5 x 3.66 THK - 2780 LG'), { diameter: 63.5, length: 2780 });
console.log('fieldsFromSizeSpec bundled guards ok');
assert2.deepEqual(fieldsFromSizeSpec('round', '65X65-400Lg'), {});
assert2.deepEqual(fieldsFromSizeSpec('flat', '25 x 40'), {});
assert2.deepEqual(fieldsFromSizeSpec('flat', '40 X 5T FLAT'), { width: 40, thickness: 5 });
console.log('fieldsFromSizeSpec round/flat guards ok');
