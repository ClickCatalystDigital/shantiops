...
import { queryAll, queryOne, execute, nextNumber } from './db';
...
// Read-only widening for GET /api/calc-drawings — QC's Form III A drawing picker (the DG- reversal
// made a real calc_drawings row canonical there) needs to list a project's drawings without QC
// gaining write access to Calc. Still no cross-department create/edit; that stays requireCalcAccess.
export function requireCalcReadAccess(user) {
  if (canAccessDepartment(user, 'QC')) return null;
  return requireCalcAccess(user);
}
...
export async function createCalcSheet(projectId, name, createdBy) {
  const csNo = await nextNumber('calc_sheet_no', 'CS');
  const { lastId } = await execute(
    `INSERT INTO calc_sheets (project_id, name, created_by, cs_no) VALUES (?, ?, ?, ?)`,
    [Number(projectId), name, createdBy || null, csNo]
  );
...
// `statuses` (BOM workspace Phase 2, optional) — the node-drawing linking picker's approval-state
// filter (defaults to approved/as_built only, see components/bom-structure/NodeDrawingsTab.jsx).
// Filtered in JS, not SQL: a project's drawing list is always small, and every other caller keeps
// calling this with no third argument at all, so their behavior is untouched.
export async function getCalcDrawings(projectId, statuses = null) {
  ...
  return drawings.filter(d => !statuses || statuses.includes(d.status)).map((d) => ({
    ...
    dgNo: d.dg_no,
    approvedAt: d.approved_at, approvedBy: d.approved_by,
...
export async function addDrawing({ projectId, name, description, drawingType, assignedTo }) {
  const dgNo = await nextNumber('drawing_no', 'DG');
  const { lastId } = await execute(
    `INSERT INTO calc_drawings (project_id, name, description, drawing_type, dg_no, assigned_to) VALUES (?, ?, ?, ?, ?, ?)`,
    [Number(projectId), name, description || null, drawingType || null, dgNo, assignedTo || null]
  );
  return Number(lastId);
}

// Design-department eligibility for drawing assignment — mirrors getDesignTeamMembers()'s own
// eligibility exactly (lib/data.js): an employee counts as Design either by their primary
// `department` column OR by being cross-granted Design access via `access_departments` (the same
// mechanism §5f documents for cross-department system access). A plain `department = 'Design'`
// check silently excludes the second group — found live via a real employee (department=NULL,
// access_departments='Design') who showed up correctly in the assignment picker but then failed
// every one of the checks below. One shared predicate now, not three copies that can drift apart.
const DESIGN_EMPLOYEE_WHERE = "(department = 'Design' OR INSTR(',' || COALESCE(access_departments, '') || ',', ',Design,') > 0) AND active = 1";

export async function findDesignEmployeeByName(name) {
  if (!name) return null;
  return queryOne(`SELECT id, name FROM employees WHERE name = ? AND ${DESIGN_EMPLOYEE_WHERE}`, [name]);
}

// The current user's own Design employee record, if any — used to auto-assign a drawing someone
// other than the Head creates to themselves.
export async function findMyDesignEmployee(userId) {
  if (!userId) return null;
  return queryOne(`SELECT name FROM employees WHERE user_id = ? AND ${DESIGN_EMPLOYEE_WHERE}`, [userId]);
}

// The assigned Designer owns their own drawing's files and submit/withdraw action — a plain
// department-wide Design grant isn't narrow enough once a drawing is assigned to one specific
// person. `assigned_to` is a plain name string (matches how the Head's own assignment picker and
// the PATCH route's validation already store/check it), so this resolves it the same way.
export async function isAssignedDesigner(user, assignedTo) {
  if (!assignedTo || !user?.id) return false;
  const row = await queryOne(
    `SELECT id FROM employees WHERE name = ? AND user_id = ? AND ${DESIGN_EMPLOYEE_WHERE}`,
    [assignedTo, user.id]
  );
  return !!row;
}

// name -> the employee's own user_id, for notifying whoever a drawing is assigned to (the Head's
// approve/send-back/mark-as-built actions all target this person).
export async function resolveAssignedUserId(assignedTo) {
  if (!assignedTo) return null;
  const row = await queryOne(`SELECT user_id FROM employees WHERE name = ? AND ${DESIGN_EMPLOYEE_WHERE}`, [assignedTo]);
  return row?.user_id || null;
}

const DRAWING_FIELDS = ['name', 'description', 'drawing_type', 'status', 'assigned_to', 'due_date', 'notes', 'customer_visible', 'customer_visible_since', 'customer_notified_at', 'approved_at', 'approved_by'];
