...
const s = StyleSheet.create({
  ...
  // second orange rule and the motto line sit under the whole row (logo + wordmark), not just the
  // text column — the outer wrapper is `alignSelf:'flex-start'`, so it shrink-wraps to the row's
  // own natural width and both rule/motto (no explicit width) stretch to match it automatically;
  ...
  sbTagline: { fontSize: 6, color: '#5B6B7C', letterSpacing: 2.4, marginTop: 1 },
  sbRuleFull: { height: 2, backgroundColor: '#F45D20', marginTop: 3, marginBottom: 3 },
  // Explicit width (not left to flex-stretch) — @react-pdf/renderer's own textAlign:'center' only
  // centers within a Text node's *declared* width; a width it merely inherits from being stretched
  // by its flex parent is measured too late for centering to apply, so the line renders flush left
  // instead (confirmed via an isolated render before landing on this fix). 163pt matches the row's
  // own real rendered width (logo + wordmark) at this font size — re-measure if either changes.
  sbMotto: { fontSize: 6.5, fontWeight: 'bold', color: '#0F2A4D', letterSpacing: 0.6, textAlign: 'center', width: 163 },
