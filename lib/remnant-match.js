// lib/remnant-match.js — automatic remnant-to-BOM matching, triggered when Design releases a BOM
// (app/api/projects/[id]/release-bom/route.js) and again whenever a single dimensional line is
// added to an already-released project (app/api/bom-items/route.js). Checks a BOM line's required
// material/profile/thickness/dimensions against available stock_pieces (lib/stock-pieces.js),
// reserves whatever fits (plate rotation allowed), and keeps the matched line out of Procurement —
// see lib/data.js's getSourcingItems, which already treats pending_review=1 as permanently hidden.
//
// Scope boundary (confirmed): only BOM lines that already carry a structured category +
// category_fields_json (entered through the PR/BOM composer) can be matched — the dominant
// bulk-Excel-import path writes pure free text and is unaffected, exactly as before this feature.
import { execute, queryAll } from './db';
import { splitQtyText, cloneBomItemForSplit, getRemnantMatchTolerances } from './procurement';
import { normalizeMaterial } from './match-utils';
import { DIMENSIONAL_CATEGORIES } from './bom-fields.mjs';
import { itemRollupQty } from './bom-structure.mjs';
import { getAssemblyRollupMap, getProjectUnitCounts } from './data';

export { normalizeMaterial, DIMENSIONAL_CATEGORIES };

function parseNum(v) {
  if (v == null) return null;
  const m = String(v).match(/-?\d+(?:\.\d+)?/);
  return m ? Number(m[0]) : null;
}

// A BOM line's required geometry, read from its category + category_fields_json (the "shape
// varies, read/written whole" JSON blob CALC-CHANGES2.md §F introduced). Returns null for anything
// that isn't a dimensional category or doesn't parse to real numbers — those lines are left
// untouched, the confirmed best-effort boundary.
export function parseDims(bomItem) {
  if (!bomItem?.category || !bomItem?.category_fields_json) return null;
  let f;
  try { f = JSON.parse(bomItem.category_fields_json); } catch { return null; }

  if (bomItem.category === 'plate') {
    const length_mm = parseNum(f.length), width_mm = parseNum(f.width), thickness_mm = parseNum(f.thickness);
    if (!(length_mm > 0 && width_mm > 0 && thickness_mm > 0)) return null;
    return { kind: 'plate', length_mm, width_mm, thickness_mm };
  }
  if (DIMENSIONAL_CATEGORIES.includes(bomItem.category)) {
    const length_mm = parseNum(f.length);
    const profile = normalizeMaterial(f.size || f.section_type || '');
    if (!(length_mm > 0) || !profile) return null;
    return { kind: 'linear', length_mm, profile };
  }
  return null;
}

// Available stock_pieces that satisfy a BOM line's material + profile/thickness + dimensions,
// sorted least-waste first. `inventory_items.spec` doubles as the linear profile designation
// (e.g. "ISMB 150", "50x50x6") — reused rather than adding a new column, same field Stores' item
// form already collects.
// `tolerances` is optional — a whole-project sweep (matchProjectBom) fetches it once and passes it
// down; a single-item caller omits it and this self-fetches (one extra query, negligible off the
// sweep's hot path).
export async function findCandidates(bomItem, tolerances = null) {
  const req = parseDims(bomItem);
  if (!req) return [];
  const reqMoc = normalizeMaterial(bomItem.moc);
  // A picked-from-catalog line already has a real identity (item_id) to match on even with moc
  // left blank; only a free-typed line with neither is an actual guess, and that's what this skips.
  if (!reqMoc && !bomItem.item_id) return [];
  const tol = tolerances || await getRemnantMatchTolerances();
  const thicknessToleranceMm = tol.plate?.thickness_mm ?? 0.3;

  // owner_project_id filter — Stores/Inventory hardening Phase 2, ownership guard #2. A common
  // (unowned) piece is fair game for anyone; a piece already owned by this same demand's own
  // project is fair game for it (the normal "reuse our own procured-but-unused material" case);
  // another project's owned piece must never surface as a candidate — that's the one thing an
  // Ownership Transfer (Phase 4) exists to do explicitly, never a side effect of matching.
  const rows = await queryAll(
    `SELECT sp.*, i.moc AS inv_moc, i.spec AS inv_spec, i.item_id AS inv_item_id
       FROM stock_pieces sp JOIN inventory_items i ON i.id = sp.inventory_item_id
      WHERE sp.status = 'available' AND i.track_pieces = 1 AND i.category = ?
        AND (sp.owner_project_id IS NULL OR sp.owner_project_id = ?)`,
    [bomItem.category, bomItem.project_id]
  );

  const scored = [];
  for (const p of rows) {
    // Item Master identity (§3.2's real join key, items.id) is the strong signal — checked before
    // dimensions and ahead of the free-text moc fallback. Both sides only carry item_id when
    // picked from the same catalog, so it's a real identity match, not a guess; either side
    // missing it (still the common case on free-typed rows) falls back to the moc string compare
    // exactly as before this round, so existing free-text workflows keep matching unchanged.
    const identityMatch = bomItem.item_id && p.inv_item_id && bomItem.item_id === p.inv_item_id;
    if (!identityMatch && normalizeMaterial(p.inv_moc) !== reqMoc) continue;
    if (req.kind === 'plate') {
      if (Math.abs(Number(p.thickness_mm) - req.thickness_mm) > thicknessToleranceMm) continue;
      const straight = p.length_mm >= req.length_mm && p.width_mm >= req.width_mm;
      const rotated = p.length_mm >= req.width_mm && p.width_mm >= req.length_mm; // plate rotation allowed
      if (!straight && !rotated) continue;
      scored.push({ piece: p, waste: (p.length_mm * p.width_mm) - (req.length_mm * req.width_mm) });
    } else {
      if (normalizeMaterial(p.inv_spec) !== req.profile) continue; // section/angle profile must match exactly
      if (!(p.length_mm >= req.length_mm)) continue;
      scored.push({ piece: p, waste: p.length_mm - req.length_mm });
    }
  }
  scored.sort((a, b) => a.waste - b.waste); // least-waste (smallest sufficient piece) first
  return scored.map(s => s.piece);
}

// Manual-only hint, never auto-reserved — for a plate line findCandidates() found nothing big
// enough for, same-material/same-thickness pieces that exist but are individually too small (e.g.
// two remnants that could be cut-and-welded together). Deliberately skips the L/W fit check that
// findCandidates() enforces; surfacing this is a suggestion for a human (Stores/Production) to act
// on manually — auto-reservation must never combine pieces on its own (§ conversation, 2026-09-17).
export async function findCombinablePlateCandidates(bomItem, tolerances = null) {
  const req = parseDims(bomItem);
  if (!req || req.kind !== 'plate') return [];
  const reqMoc = normalizeMaterial(bomItem.moc);
  if (!reqMoc && !bomItem.item_id) return [];
  const tol = tolerances || await getRemnantMatchTolerances();
  const thicknessToleranceMm = tol.plate?.thickness_mm ?? 0.3;

  const rows = await queryAll(
    `SELECT sp.*, i.moc AS inv_moc, i.item_id AS inv_item_id
       FROM stock_pieces sp JOIN inventory_items i ON i.id = sp.inventory_item_id
      WHERE sp.status = 'available' AND i.track_pieces = 1 AND i.category = 'plate'
        AND (sp.owner_project_id IS NULL OR sp.owner_project_id = ?)`,
    [bomItem.project_id]
  );
  return rows.filter(p => {
    const identityMatch = bomItem.item_id && p.inv_item_id && bomItem.item_id === p.inv_item_id;
    if (!identityMatch && normalizeMaterial(p.inv_moc) !== reqMoc) return false;
    return Math.abs(Number(p.thickness_mm) - req.thickness_mm) <= thicknessToleranceMm;
  });
}

// Attaches `combinable_piece_count` (mutates in place, same convention as lib/data.js's
// attachDeliveryLotDates) to any still-unmatched plate line in a Material Demand list, so Stores
// sees "3 pieces of matching material/thickness exist, just none big enough alone" without a
// client-side fetch per row. Only runs for the rows it could possibly apply to — cheap for the
// common case (most lines aren't unmatched plate lines).
export async function attachCombinablePlateHints(rows) {
  const candidates = rows.filter(r => r.category === 'plate' && !r.reserved_piece_count);
  if (!candidates.length) return rows;
  const tolerances = await getRemnantMatchTolerances();
  await Promise.all(candidates.map(async r => {
    const pieces = await findCombinablePlateCandidates(r, tolerances);
    r.combinable_piece_count = pieces.length;
  }));
  return rows;
}

// The core action: reserve up to the BOM line's required qty from available matching pieces.
// - No match at all: line is untouched, proceeds through the normal Stores-review/Procure path.
// - Full match: no row split needed, the line itself is force-gated (pending_review=1) so it never
//   satisfies getSourcingItems() even once release_bom is done.
// - Partial match: splits like reserveFromStock — original row keeps the unmet remainder (still
//   open, still reviewable/procurable by Stores), a cloned row carries the matched qty and is what
//   the reserved pieces point at.
// Each reservation UPDATE is a conditional `WHERE status='available'` — the same guard
// reserveFromStock's available-pool math relies on — so two lines racing for the same piece can
// never both win it, no wrapping transaction required (same non-transactional precedent
// reserveFromStock itself already uses at this codebase's scale).
// `byId`/`unitCount` are optional — matchProjectBom (a whole-project loop) builds both maps once
// and passes them down; a single-item caller (PR raise, template apply, single BOM-item add) omits
// them and this self-fetches, scoped to the item's own project.
export async function matchAndReserve(bomItem, username = 'system', byId = null, unitCount = null, tolerances = null) {
  const req = parseDims(bomItem);
  if (!req) return { matched: 0 };

  const rollupById = byId || await getAssemblyRollupMap(bomItem.project_id);
  const effectiveUnitCount = unitCount ?? (await getProjectUnitCounts(bomItem.project_id)).get(bomItem.project_id) ?? 1;
  const required = itemRollupQty(bomItem.qty_text, bomItem.assembly_id, rollupById, effectiveUnitCount, !!bomItem.qty_resolved) ?? 0;
  if (!(required > 0)) return { matched: 0 };

  const candidates = await findCandidates(bomItem, tolerances);
  if (!candidates.length) return { matched: 0 };

  // Stores/Inventory hardening Phase 3 — write-path consistency, not a full consolidation onto
  // reservePiece(). Considered calling reservePiece() here directly, but this loop's own final
  // bom_item_id isn't known until AFTER seeing how many candidates actually won the CAS race below
  // (K vs required decides whether a split-clone is needed, a few lines down) — reservePiece()
  // requires bomItemId up front for its own validation, so a real sequencing mismatch, not just
  // structural preference, makes forcing that call unsafe here. What WAS a genuine bug: this raw
  // UPDATE never set `project_id` the way reservePiece() does, leaving it null on every
  // matchAndReserve-origin piece — fixed by setting it here too, so both write paths now produce the
  // identical row shape even though they remain two statements.
  const reservedIds = [];
  for (const c of candidates) {
    if (reservedIds.length >= required) break;
    const res = await execute(
      "UPDATE stock_pieces SET status = 'reserved', project_id = ? WHERE id = ? AND status = 'available'",
      [bomItem.project_id, c.id]
    );
    if (res.changes === 1) reservedIds.push(c.id);
  }
  const K = reservedIds.length;
  if (K === 0) return { matched: 0 };

  let targetBomItemId = bomItem.id;
  if (K < required) {
    const shortfall = required - K;
    const [remainingQtyText, reservedQtyText] = splitQtyText(bomItem.qty_text, shortfall, K);
    // qty_resolved=1 — see lib/procurement.js's reserveFromStock, the identical split-qty
    // double-counting fix: shortfall is already total-space (pieces), never re-multiply it.
    await execute('UPDATE bom_items SET qty_text = ?, qty_resolved = 1 WHERE id = ?', [remainingQtyText, bomItem.id]);
    targetBomItemId = await cloneBomItemForSplit(bomItem, { qtyText: reservedQtyText, pendingReview: true });
  } else {
    await execute('UPDATE bom_items SET pending_review = 1 WHERE id = ?', [bomItem.id]);
  }
  for (const id of reservedIds) {
    await execute('UPDATE stock_pieces SET bom_item_id = ? WHERE id = ?', [targetBomItemId, id]);
  }
  return { matched: K, shortfall: Math.max(0, required - K), targetBomItemId };
}

// Every dimensional line on a project — the release-bom hook's entry point. Non-dimensional/
// free-text-only lines are skipped inside matchAndReserve (parseDims returns null), not filtered
// here, so a mixed BOM never needs special-casing at the call site.
// pending_review=0 — real gap found and fixed alongside the split-qty fix above: this query had no
// such filter at all, unlike its sibling matchProjectPlainStock (which already excludes pending_review=1
// rows for exactly this reason). Without it, an already-fully-matched dimensional clone
// (cloneBomItemForSplit forces pendingReview:true on it, a few lines up) could be re-selected on a
// later Release BOM and matched AGAIN — a real double-reservation risk, not just a display bug. A
// still-open remainder row keeps pending_review=0, so it correctly stays eligible for further matching.
export async function matchProjectBom(projectId, username = 'system') {
  const placeholders = DIMENSIONAL_CATEGORIES.map(() => '?').join(',');
  const items = await queryAll(
    `SELECT * FROM bom_items WHERE project_id = ? AND category IN (${placeholders}) AND pending_review = 0`,
    [projectId, ...DIMENSIONAL_CATEGORIES]
  );
  // One rollup map (+ one project unit-count lookup) for the whole project, not one per item — a
  // Release BOM can process 100+ lines.
  const [rollupById, unitCounts, tolerances] = await Promise.all(
    [getAssemblyRollupMap(projectId), getProjectUnitCounts(projectId), getRemnantMatchTolerances()]);
  // Number(...) matters here: projectId reaches this function as a route-param STRING when called
  // from release-bom/route.js (params.id is never auto-coerced by Next), but unitCounts' keys are
  // real DB-row integers — a bare `.get(projectId)` would silently miss and fall back to 1. Same
  // fix as lib/procurement.js's matchProjectPlainStock, found live via the identical symptom.
  const unitCount = unitCounts.get(Number(projectId)) ?? 1;
  const results = [];
  for (const item of items) {
    const r = await matchAndReserve(item, username, rollupById, unitCount, tolerances);
    if (r.matched > 0) results.push({ bomItemId: item.id, ...r });
  }
  return results;
}
