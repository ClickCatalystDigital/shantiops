...
// `byId` is optional — matchProjectBom (a whole-project loop) builds the rollup map once and
// passes it down; a single-item caller (PR raise, template apply, single BOM-item add) omits it
// and this self-fetches, scoped to the item's own project.
export async function matchAndReserve(bomItem, username = 'system', byId = null) {
  const req = parseDims(bomItem);
  if (!req) return { matched: 0 };

  const rollupById = byId || await getAssemblyRollupMap(bomItem.project_id);
  const required = itemRollupQty(bomItem.qty_text, bomItem.assembly_id, rollupById) ?? 0;
...
export async function matchProjectBom(projectId, username = 'system') {
  ...
  const items = await queryAll(
    `SELECT * FROM bom_items WHERE project_id = ? AND category IN (${placeholders})`,
  ...
  for (const item of items) {
    const r = await matchAndReserve(item, username, rollupById);
