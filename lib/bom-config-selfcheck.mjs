...
import {
  isConfigLabel, isAlwaysConfigLabel, classifyConfigRow, continuationConfig, splitLabelValue, splitValueUnit, configParts, parseConfig, validateConfigInput, normalizeConfig, serializeConfig, mergeConfig, CONFIG_LIMITS,
} from './bom-config.mjs';
...
// --- CHIMNEY SIZE: — real STF-IBR-055/057/060 data, moc + qty attached but still configuration
assert.ok(isAlwaysConfigLabel('CHIMNEY SIZE:'));
assert.ok(!isAlwaysConfigLabel('CHIMNEY SUPPORT GUSSETS'), 'a real chimney bracket item must not be swept in by a loose match');
assert.deepStrictEqual(
  classifyConfigRow({ material_description: 'CHIMNEY SIZE:', moc: 'MILD STEEL', qty_text: '30 Mtr',
    size_spec: 'TOP DIA : 550 MM  BOTTOM DIA : 1100 MM  TAPPERED PIECE: 10T MM X 10000 = 1  No  STRAIGHT PIECE: 8T MM X 10000 = 1  No' }),
  { label: 'CHIMNEY SIZE', value: 'TOP DIA : 550 MM BOTTOM DIA : 1100 MM TAPPERED PIECE: 10T MM X 10000 = 1 No STRAIGHT PIECE: 8T MM X 10000 = 1 No', unit: '' },
  'real moc + qty attached, still configuration — the trailing colon is stripped from the stored label');
assert.strictEqual(
  classifyConfigRow({ material_description: 'CHIMNEY SUPPORT GUSSETS', moc: 'MS', qty_text: '10 Nos', size_spec: '240 x 450 x 12 THK' }),
  null, 'a genuinely different chimney label with the same moc/qty shape stays a real item — not swept in by CHIMNEY SIZE\'s override');
assert.strictEqual(
  classifyConfigRow({ material_description: 'CHIMNEY SIZE', moc: 'MILD STEEL', size_spec: '...', make: 'X', remarks: 'Y' }),
  null, 'two value cells filled is still ambiguous even for an always-config label — rule 4 is not bypassed');
