...
// A sales call that was closed while still at an open stage (Create PO with "don't continue", or an
// old-CRM enquiry imported as history) is no longer pipeline: kept in the Leads list and on the
// customer, left out of the board and the funnel. A closed call at a won/lost stage still counts.
export function isClosedCall(lead, stages) {
  return !!lead?.sales_call_closed_at && leadStateForStage(stages, lead.sales_call_status || DEFAULT_STAGE) === 'open';
}
...
export function funnelRows(leads = [], stages = []) {
  const live = leads.filter(l => !isClosedCall(l, stages));
  return [...stages].sort((a, b) => a.sort_order - b.sort_order).map(s => {
    const matching = live.filter(l => l.sales_call_status === s.name);
