...
export async function consumeStock({ trackingMode, inventoryItemId, qty, bomItemId, jobCardId, username, unitCost, totalCost, existingAllocations, existingSerialIds }) {
  return withTransaction(async tx => {
    const ins = await tx.execute({
      sql: `INSERT INTO material_issues (bom_item_id, job_card_id, qty, issued_by, unit_cost, total_cost) VALUES (?, ?, ?, ?, ?, ?)`,
      args: [bomItemId, jobCardId || null, qty, username || null, unitCost ?? null, totalCost ?? null],
    });
