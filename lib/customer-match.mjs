...
export const nameWords = s => [...new Set(normalizeWords(s).filter(w => !SUFFIX.has(w)))];
// Order-independent key used to merge exact duplicates on import ("M/s. X Pvt Ltd" = "X Limited").
export const customerKey = s => nameWords(s).sort().join(' ');
