...
// rows, not a smarter heuristic here. Cancelled lines are dead and excluded from both buckets, and so
// are attribute-only rows (isAttributeOnly, below) — a sub-heading's own spec fields, not real items.
import { getProjectBom } from './data';
import { DIMENSIONAL_CATEGORIES } from './bom-fields.mjs';

// `other` (category-taxonomy audit, SYSTEM.md) — a line that's genuinely neither raw pressure
// material nor an IBR mounting/fitting (electrical/panel components, refractory/insulation/
// consumables, rotating equipment — real examples found auditing SB-1109-01-50). Excluded from
// BOTH syncQcPartsFromBom and syncMountingsFromBom below, same as Cancelled/attribute-only rows —
// forcing these onto `standard` would pollute the real statutory Mounting & Fittings list.
function classify(b) {
  if (b.category === 'other') return null;
...
// A PMB sub-heading's own attribute rows (e.g. under "F.D.FAN BLOWER": TYPE/FLOW cfm/STATIC HEAD/
// SPEED RPM/MEDIUM/OPERATING TEMP — confirmed against the client's real PMB-3 TPH.xlsx) are not
// purchasable items at all — they describe the equipment named in the row above them, using the
// Size column to hold a spec value with no material/make/qty of their own. They're never
// distinguishable from real material by category (never categorized) but ARE reliably distinguishable
// by having none of moc/make/qty_text set — every genuine line, however sparse, in the one real
// project checked always carries at least one. Excluded from BOTH Form IV A/III A and Mountings, not
// routed to either — appearing in neither is correct here, unlike every other classify() outcome.
// `// ponytail:` heuristic, not a stored flag — verify against a wider dataset before trusting fully.
...
// Retroactively route already-existing ungrouped parts into a III A group that now matches them.
// syncQcPartsFromBom's INSERT OR IGNORE only sets iiia_group_id at insert time — a document whose
// Form IV A parts were already synced (the normal case, since IV A auto-syncs on document creation)
// would otherwise never see those existing rows move into a group created afterward, which is why
// "Sync from BOM" alone looked like it did nothing for Form III A. Called both after creating a group
// (so it claims matching parts immediately, no extra sync click needed) and at the end of every
// syncQcPartsFromBom run (so a later BOM assembly/group_label change also gets picked up).
export async function reconcileIiiaGroups(tx, documentId) {
  const groups = await tx.execute({ sql: 'SELECT * FROM qc_iiia_groups WHERE document_id = ?', args: [documentId] });
  if (!groups.rows.length) return 0;
  const ungrouped = await tx.execute({
    sql: `SELECT qdp.id, bi.assembly_id, bi.group_label FROM qc_document_parts qdp
          JOIN bom_items bi ON bi.id = qdp.bom_item_id
          WHERE qdp.document_id = ? AND qdp.iiia_group_id IS NULL`,
    args: [documentId],
  });
  let moved = 0;
  for (const row of ungrouped.rows) {
    const g = groups.rows.find(g => row.assembly_id != null && g.assembly_id === row.assembly_id)
      || groups.rows.find(g => row.group_label && g.group_label === row.group_label);
    if (!g) continue;
    await tx.execute({ sql: 'UPDATE qc_document_parts SET iiia_group_id = ? WHERE id = ?', args: [g.id, row.id] });
    moved++;
  }
  return moved;
}
...
export async function syncQcPartsFromBom(tx, documentId, projectId) {
  ...
  if (!qualifying.length) {
    await reconcileIiiaGroups(tx, documentId);
    await reconcilePartsCertificates(tx, documentId);
    await reconcileUnitCertificates(tx, documentId);
    return 0;
  }
...
export async function syncMountingsFromBom(tx, documentId, projectId) {
  const { bom } = await getProjectBom(projectId);
  const qualifying = bom.filter(b => classify(b) === 'mounting' && b.purchase_status !== 'Cancelled' && !isAttributeOnly(b));
  // Reconcile still runs even when nothing new needs inserting — same gap already fixed once for
  // syncQcPartsFromBom's twin (§5bj): an early return here would silently skip picking up a fresh
  // unit-certificate assignment made after this document's mountings were already fully synced.
  if (!qualifying.length) { await reconcileMountingCertificates(tx, documentId); return 0; }
  ...
  await reconcileMountingCertificates(tx, documentId);
  return added;
}

// Multi-unit split — the qc_mountings twin of reconcileUnitCertificates below. A cell QC has already
// explicitly assigned exactly one certificate to (via /qc's "Assign to Units" panel,
// bom_item_child_certificates) is a confirmed fact for a mounting row too, not just a Form IV A
// material row — the panel itself doesn't distinguish the two (it lists every allocated BOM item
// regardless of classify()'s outcome), so this only needed the write side extended. Same rules
// exactly: WHERE test_certificate_id IS NULL never overwrites a human link; 2+ certificates on a
// cell is genuine ambiguity, left unlinked.
async function reconcileMountingCertificates(tx, documentId) {
  const doc = await tx.execute({ sql: 'SELECT project_id FROM qc_documents WHERE id = ?', args: [documentId] });
  const childProjectId = doc.rows[0]?.project_id;
  if (!childProjectId) return;

  const unlinked = await tx.execute({
    sql: `SELECT id, bom_item_id FROM qc_mountings WHERE document_id = ? AND test_certificate_id IS NULL AND bom_item_id IS NOT NULL`,
    args: [documentId],
  });
  for (const row of unlinked.rows) {
    const certs = await tx.execute({
      sql: 'SELECT DISTINCT certificate_id FROM bom_item_child_certificates WHERE bom_item_id = ? AND child_project_id = ?',
      args: [row.bom_item_id, childProjectId],
    });
    if (certs.rows.length !== 1) continue;
    await tx.execute({
      sql: 'UPDATE qc_mountings SET test_certificate_id = ? WHERE id = ? AND test_certificate_id IS NULL',
      args: [certs.rows[0].certificate_id, row.id],
    });
  }
}
...
// Multi-unit split — the (bom_item, child) cell-level equivalent of reconcilePartsCertificates
// above, same rules exactly: a cell QC has already explicitly assigned exactly one certificate to
// (via /qc's "Assign to Units" panel, bom_item_child_certificates) is a confirmed fact, not a guess —
// this auto-populates the document part directly, so re-confirming it in the picker's own suggestion
// box is never required. A cell with 2+ certificates is genuine, unresolved ambiguity and is
// deliberately left unlinked here, exactly as reconcilePartsCertificates already leaves a
// multi-cast stock-piece match unlinked — the picker's own SUGGESTED list (untouched by this
// function) is still where a human resolves that case. `WHERE test_certificate_id IS NULL` is the
// same never-overwrite-a-human-link guard the sibling function already uses. Every part matches
// against its own bom_item_id here (not just named-part rows, unlike reconcilePartsCertificates,
// since a cell-level assignment applies to the whole line regardless of any named-part breakdown).
async function reconcileUnitCertificates(tx, documentId) {
  const doc = await tx.execute({ sql: 'SELECT project_id FROM qc_documents WHERE id = ?', args: [documentId] });
  const childProjectId = doc.rows[0]?.project_id;
  if (!childProjectId) return;

  const unlinked = await tx.execute({
    sql: `SELECT id, bom_item_id FROM qc_document_parts WHERE document_id = ? AND test_certificate_id IS NULL AND bom_item_id IS NOT NULL`,
    args: [documentId],
  });
  for (const row of unlinked.rows) {
    const certs = await tx.execute({
    ...
    if (certs.rows.length !== 1) continue;
    await tx.execute({
      sql: 'UPDATE qc_document_parts SET test_certificate_id = ? WHERE id = ? AND test_certificate_id IS NULL',
      args: [certs.rows[0].certificate_id, row.id],
    });
  }
}
