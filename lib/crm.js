...
export async function resolveLeadToCustomer(lead, { title, username, customerId: chosenId = null } = {}) {
  ...
  if (customer) {
    ...
    // Carry the enquiry's address details across — the order, its PDF and Customer 360 read them
    // from the customer, and they used to come out blank.
    const { lastId } = await execute(
      `INSERT INTO customers (name, phone, email, address, city, pin_code, website, account_manager)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [companyName, lead.phone || lead.telephone || null, lead.email || null, lead.address || null,
        lead.district || null, lead.pin_code || null, lead.website || null, lead.account_manager || null]
    );
