...
// keyField='item_id' groups the same Item Master item across projects instead (PMB view, rows with
// no PR behind them); rows with no item_id are dropped, same as a missing pr_item_id.
export function aggregatePrGroups(items, keyField = 'pr_item_id') {
  const groups = new Map();
  for (const it of items) {
    if (!it[keyField]) continue;
    if (!groups.has(it[keyField])) groups.set(it[keyField], []);
    groups.get(it[keyField]).push(it);
  }
  ...
  for (const [prItemId, rows] of groups) {
    ...
    for (const c of sourcing) {
      ...
      if (unit == null) unit = u;
      else if (u.toLowerCase().replace(/s$/, '') !== unit.toLowerCase().replace(/s$/, '')) unitMismatch = true; // No vs Nos is not a mismatch
    }
    ...
    out.push({
      pr_item_id: prItemId, group_key: prItemId, kind: keyField === 'item_id' ? 'item' : 'pr', pr_no: representative.pr_no, pr_created_at: representative.pr_created_at,
      raised_by_dept: representative.pr_raised_by_dept || null,
...
// Procurement's Fabrication / Bought Out split, from what's already on the line: Stores/Engineering
// mark a bought-out line requires_manufacturing=0, and 'standard' is the bought-out shape category.
export function procurementCategory(it) {
  if (['fabrication', 'bought_out', 'pre_dispatch'].includes(it.procurement_class)) return it.procurement_class; // set on the Item Master
  return it.requires_manufacturing === 0 || it.requires_manufacturing === false || it.category === 'standard'
    ? 'bought_out' : 'fabrication';
}
...
// --- PMB/CSV import: auto-build the assembly tree from section/group_label ---
// Turns a raw PMB-import `section` (Excel sheet name) or `group_label` (in-sheet heading row) into
// a readable bom_assemblies node name, for lib/bom-tree-from-import.js. Strips a trailing "@ ..."
// annotation clause (real hand-made sheets append pressure/spec notes this way — "BOILER MOUNTING &
// FITTINGS @ W.P.: 10.54 Kg/cm^2"), a leading "for " ("FOR FEED LINE" -> "Feed Line"), then plain
// title-cases every word and strips leftover trailing punctuation ("NOZZLE:" -> "Nozzle").
// ponytail: no acronym-preservation attempt (SDC/WPH become "Sdc"/"Wph", not "SDC"/"WPH") — a
// length/vowel-based heuristic to detect acronyms was tried and found to misclassify real short
// words (e.g. "FAN" in "FD FAN & ID FAN" reads exactly like a short acronym) with no bound on where
// it'd need to stop; renaming a node is already free, so plain, uniform title-casing is the more
// robust choice over an ever-growing acronym dictionary.
const JOINER_WORDS = new Set(['and', 'or', 'of', 'the', 'in', 'for', 'with', 'to', 'a', 'an']);
export function humanizeAssemblyName(raw) {
  let s = String(raw ?? '').replace(/\s+/g, ' ').trim();
  if (!s) return s;
  const stripped = s.replace(/\s*@.*$/, '').replace(/^for\s+/i, '').trim();
  if (stripped) s = stripped; // only adopt the stripped form if something real is left
  const titled = s.split(' ').map((w, i) => {
    if (!w) return w;
    const lower = w.toLowerCase();
    if (i > 0 && JOINER_WORDS.has(lower)) return lower;
    return w[0].toUpperCase() + w.slice(1).toLowerCase();
  }).join(' ');
  return titled.replace(/[:,.\-]+$/, '').trim();
}
...
export function buildTemplateTree(rootNodes, childrenByParent, itemsByAssembly) {
  function nodeToJson(node) {
    ...
    // The node's Configuration (datasheet fields) travels with the template. Emitted ONLY when non-empty:
    // a node without configuration must serialize exactly as before, so re-saving an existing template
    // stays byte-identical and never raises a spurious "newer version" hint.
    const config = parseConfig(node.config_json);
    if (config.length) json.config = config;
    return json;
  }
  ...
  function walk(nodes, tempParentId) {
    for (const node of nodes || []) {
      ...
      flat.push({
        ...
        items: node.items || [],
        config: parseConfig(node.config), // also splits a legacy {label,value} row (no unit key) saved before units existed
      });
