...
import { classifyConfigRow } from './bom-config.mjs';
...
function parseSheet(name, ws) {
  ...
  for (let r = dataStart; r < rows.length; r++) {
    ...
    const buildItem = () => ({
      ...
      category: inferCategory(desc, rec.size_spec),
      moc: rec.moc ?? null,
      size_spec: rec.size_spec ?? null,
      make: rec.make ?? null,
      qty_text: rec.qty_text ?? null,
      purchase_status: mapPurchaseStatus(rec.purchase_status),
    ...
    const configRow = classifyConfigRow(rec);
    if (configRow) {
      configs.push({ section: name.trim(), group_label: groupLabel, label: configRow.label, value: configRow.value, item: buildItem() });
      continue;
    }
  ...
  return { name: name.trim(), headerRow: headerRow + 1, columns, unmappedColumns: header.unmapped, items, configs, skipped };
}
