...
export const ACCOUNT_CODES = {
  ...
  SALARY_EXPENSE: '5200',
  FIXED_ASSETS: '1400',
  ACCUMULATED_DEPRECIATION: '1410', // asset type, always credited — nets negative, so it correctly
                                     // reduces net fixed assets on the Balance Sheet without a 6th
                                     // "contra-asset" account_type existing anywhere in this model.
  DEPRECIATION_EXPENSE: '5300',
  ASSET_DISPOSAL_GAIN_LOSS: '4200', // income type — nets negative for a loss, same convention as
                                     // Accumulated Depreciation nesting negative under asset-type.
  BANK_CHARGES: '5400', // ACCOUNTING-IMPLEMENTATION-PLAN.md Phase 8 — default counter-account
                         // suggestion for an unmatched bank-statement row's quick-create journal
                         // entry (e.g. bank charges, interest). Just a sane default in the UI
                         // dropdown; any account can still be picked instead.
  FREIGHT_EXPENSE: '5500', // Dispatch accounting integration, 2026-08-23 — posted only when the
                            // company itself bears freight cost (not when the customer pays); no
                            // GTA reverse-charge GST modeled, a stated simplification, not an
                            // oversight (see ACCOUNTING-IMPLEMENTATION-PLAN.md's Dispatch phase).
};
...
export const DEFAULT_CHART_OF_ACCOUNTS = [
  ...
  [ACCOUNT_CODES.FIXED_ASSETS, 'Fixed Assets', 'asset'],
  [ACCOUNT_CODES.ACCUMULATED_DEPRECIATION, 'Accumulated Depreciation', 'asset'],
  [ACCOUNT_CODES.DEPRECIATION_EXPENSE, 'Depreciation Expense', 'expense'],
  [ACCOUNT_CODES.ASSET_DISPOSAL_GAIN_LOSS, 'Gain/Loss on Asset Disposal', 'income'],
  [ACCOUNT_CODES.BANK_CHARGES, 'Bank Charges', 'expense'],
  [ACCOUNT_CODES.FREIGHT_EXPENSE, 'Freight & Transportation Expense', 'expense'],
];
...
// Reverse charge on an outward supply (rare — only a handful of notified categories, e.g. renting
// equipment to a body corporate): unlike purchase-side RCM, the supplier neither collects nor
// remits any GST at all — it's entirely the recipient's own liability, so no GST_OUTPUT_PAYABLE
// line gets posted and AR is for the taxable value only.
export function salesInvoiceLines({ subtotal, taxAmount, total, isReverseCharge }) {
  if (isReverseCharge) {
    return [
      { accountCode: ACCOUNT_CODES.ACCOUNTS_RECEIVABLE, debit: round2(subtotal), credit: 0 },
...
// Reverse charge (RCM): the vendor's invoice carries no GST — the buyer self-assesses it instead of
// paying it to the vendor. So Accounts Payable excludes the tax portion (vendor is only owed
// subtotal, minus TDS), and the same tax amount is both claimed as Input Credit AND booked as an
// Output Payable liability (the self-invoice) — a wash on this entry, settled with output GST like
// any other output liability at return time.
//
// `payableAmount` is always the final figure the caller already computed to store on the bill row
// (app/api/purchase-orders/[id]/record-bill's `payableAmount`) — under RCM that's already
// `subtotal - tdsAmt` (tax excluded), not `total - tdsAmt`. This function must NOT subtract
// taxAmount from it again — a real RCM+TDS vendor bill (subtotal 100000, tax 18000, TDS 2360,
// payableAmount 97640) previously threw "Journal entry not balanced: debit 118000 != credit
// 100000" because of exactly that double-subtraction; the pure-function selfcheck never caught it
// because its hand-picked payableAmount didn't match what the real route produces.
export function vendorBillLines({ subtotal, taxAmount, tdsAmount, payableAmount, isReverseCharge }) {
  const lines = [
  ...
  if (isReverseCharge && taxAmount) {
    lines.push({ accountCode: ACCOUNT_CODES.GST_OUTPUT_PAYABLE, debit: 0, credit: round2(taxAmount) });
  }
...
// Dispatch accounting integration — freight cost when the company itself bears it (paid
// immediately, mirroring the Bank Reconciliation quick-JE precedent). Only ever called when
// packing_lists.freight_paid_by = 'us'; when the customer bears the cost, nothing is posted at all.
// No GTA reverse-charge GST modeled — a stated simplification (freight paid to a transporter is
// frequently RCM-liable in India), not an oversight; see the plan doc's own non-goals.
export function dispatchFreightLines({ amount }) {
  return [
    { accountCode: ACCOUNT_CODES.FREIGHT_EXPENSE, debit: round2(amount), credit: 0 },
    { accountCode: ACCOUNT_CODES.BANK_CASH, debit: 0, credit: round2(amount) },
  ];
}
...
// Paid outright from Bank & Cash — a fixed asset bought via a Vendor Bill/Accounts Payable is a
// separate scope decision (would need a fixed-asset line type on vendor_bill_items), not attempted
// here; that's a real gap, noted where this is wired up.
export function fixedAssetPurchaseLines({ cost }) {
  return [
    { accountCode: ACCOUNT_CODES.FIXED_ASSETS, debit: round2(cost), credit: 0 },
    { accountCode: ACCOUNT_CODES.BANK_CASH, debit: 0, credit: round2(cost) },
  ];
}

// One combined entry per depreciation run (all assets' depreciation for the period, summed) — same
// "one JE per event" shape as a Payroll Run, not one JE per asset.
export function depreciationLines({ amount }) {
  return [
    { accountCode: ACCOUNT_CODES.DEPRECIATION_EXPENSE, debit: round2(amount), credit: 0 },
    { accountCode: ACCOUNT_CODES.ACCUMULATED_DEPRECIATION, debit: 0, credit: round2(amount) },
  ];
}

// Standard fixed-asset disposal entry: clear the asset's cost and whatever depreciation it had
// accumulated, record what actually came in, and plug the difference as a gain or loss. No
// "edit a fixed asset" flow exists (matches this app's immutable-posted-entry principle elsewhere)
// — disposing a wrongly-entered asset at disposalAmount 0 is also how a mistake gets corrected.
export function fixedAssetDisposalLines({ cost, accumulatedDepreciation, disposalAmount }) {
  const lines = [
    { accountCode: ACCOUNT_CODES.BANK_CASH, debit: round2(disposalAmount), credit: 0 },
    { accountCode: ACCOUNT_CODES.ACCUMULATED_DEPRECIATION, debit: round2(accumulatedDepreciation), credit: 0 },
    { accountCode: ACCOUNT_CODES.FIXED_ASSETS, debit: 0, credit: round2(cost) },
  ];
  const plug = round2(cost - disposalAmount - accumulatedDepreciation);
  if (plug > 0) lines.push({ accountCode: ACCOUNT_CODES.ASSET_DISPOSAL_GAIN_LOSS, debit: plug, credit: 0 }); // loss
  else if (plug < 0) lines.push({ accountCode: ACCOUNT_CODES.ASSET_DISPOSAL_GAIN_LOSS, debit: 0, credit: -plug }); // gain
  return lines.filter(l => l.debit || l.credit);
}
