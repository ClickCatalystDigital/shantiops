...
import { keyDim, stemOf, gradeMatches, memoryKeys, memoryTrusted, deriveDefaultMoc, deriveCategoryFieldsFromName } from './item-attributes.mjs';
...
// Single-digit tokens ("1.5" / "2.5" / "4" SQ MM) are the only difference between these lines and must not collide.
const l15 = memoryKeys({ material_description: 'INSULATED LUGS 1.5 SQ MM PIN TYPE', moc: '', size_spec: '' });
const l25 = memoryKeys({ material_description: 'INSULATED LUGS 2.5 SQ MM PIN TYPE', moc: '', size_spec: '' });
const l4 = memoryKeys({ material_description: 'INSULATED LUGS 4 SQ MM PIN TYPE', moc: '', size_spec: '' });
assert.notEqual(l15.exactKey, l25.exactKey); assert.notEqual(l25.exactKey, l4.exactKey);
// a real size cell still wins, so existing size-keyed memory keeps matching
assert.equal(memoryKeys({ material_description: 'STRAIGHT SHELL-1', moc: 'MS', size_spec: 'OD700x7000x6THK' }).size, 'od700x7000x6thk');
...
// --- Item Master standardization pass: deriving a catalog row's own defaults from its name
assert.strictEqual(deriveDefaultMoc('MS EN-8 ROD 63 MM (1 MTR 24.5 KGS)'), 'EN-8', 'EN-8 beats the generic MS also present in the name');
assert.strictEqual(deriveDefaultMoc('SS 202 PLATES 8 MM'), 'SS 202');
assert.strictEqual(deriveDefaultMoc('MS PLATES 10 MM'), 'MS');
assert.strictEqual(deriveDefaultMoc('BOLT M12'), null, 'no grade stated: no guess');
assert.deepStrictEqual(deriveCategoryFieldsFromName('plate', 'BQ PLATE 12 MM SA 516 GR 70, NORMALIZED, FORM IV TC'), { thickness: 12 });
assert.deepStrictEqual(deriveCategoryFieldsFromName('angle', 'MS ANGLE 50 X 50 X 5 MM'), { size: 'ISA 50x50x5', kg_per_m: 3.77 });
assert.strictEqual(deriveCategoryFieldsFromName('angle', 'MS ANGLE 999 X 999 X 9 MM'), null, 'no table entry: never guessed');
assert.strictEqual(deriveCategoryFieldsFromName('flat', 'MS FLATE 25 X 4/5 MM'), null, 'dual-spec: not guessed');
assert.strictEqual(deriveCategoryFieldsFromName('flat', 'MS FLATE 40 X 5 MM // 6 MM'), null, 'dual-spec, second real form: not guessed');
assert.deepStrictEqual(deriveCategoryFieldsFromName('flat', 'SS FLAT 25 MM X 5 MM'), { width: 25, thickness: 5 }, 'bare "SS" states no specific grade, so no density is guessed');
assert.deepStrictEqual(deriveCategoryFieldsFromName('round', 'SS 202 ROD 20 MM'), { diameter: 20, density: 7900 });
assert.deepStrictEqual(deriveCategoryFieldsFromName('round', 'MS ROD 20 MM (1 MTR 1.23 KGS)'), { diameter: 20 });
assert.strictEqual(deriveCategoryFieldsFromName('round', 'MS ROD 28 MM / 30 MM (1 MTR 5.5 KGS)'), null, 'a real dual-diameter row: not guessed');
assert.strictEqual(deriveCategoryFieldsFromName('round', 'STAY ROD SA 105 GR 28 MM / 30 MM (1 MTR 5.5 KGS)'), null, 'dual-diameter, no weight-annotation numbers miscounted');
assert.deepStrictEqual(deriveCategoryFieldsFromName('square', 'MS SQUARE ROD 25 MM (1 MTR 4.9 KGS)'), { side: 25 });
assert.strictEqual(deriveCategoryFieldsFromName('pipe', 'MS PIPE 50 MM'), null, 'pipe: naming-cleanup only this round, never a defaults object');
assert.strictEqual(deriveCategoryFieldsFromName('standard', 'GATE VALVE 50 MM'), null, 'standard/other never get a defaults object');
assert.deepStrictEqual(deriveCategoryFieldsFromName('beam', 'MS I. BEAM 100 X 70'), { size: 'ISMB 100', kg_per_m: 11.5 });
assert.deepStrictEqual(deriveCategoryFieldsFromName('channel', 'MS CHANELS ISMC 100 X 50'), { size: 'ISMC 100', kg_per_m: 9.56 });
