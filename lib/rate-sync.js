...
export async function syncRatesFromHub() {
  ...
  for (const row of rows) {
    ...
    // inside payload (confirmed against the real hub response) — insertGstRate/insertVendorTdsRate
    // both require effective_from as a param, so it must be merged in here. Found live on this
    // sync's first real non-empty batch: every prior "verified" run was against an empty hub, so
    // this never actually executed until now.
