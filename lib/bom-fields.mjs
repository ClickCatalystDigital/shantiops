...
export const BOM_FIELD_OWNERS = {
  ...
  // category/category_fields_json (the plate/section dimension data — the same shape the PR/BOM
  // composer already writes via app/api/purchase-requisitions) joined the owned list once
  // BomTable's own "+ Add item" dialog started collecting them directly too, so POST /api/bom-items
  // has an allowlist entry to actually store them under instead of silently dropping them.
  Engineering: ['section', 'group_label', 'material_description', 'moc', 'size_spec', 'make', 'qty_text', 'remarks', 'assembly_id',
    'category', 'category_fields_json',
...
export const RECEIVING_FIELDS = ['grn_ref', 'grn_qty_text', 'receipt_id', 'received_heat_no', 'received_mtc_no', 'received_supplier_batch_no', 'received_serial_no',
  // Phase 4 — the real linked test_certificates row an MTC requirement resolves to (received_mtc_no
  // stays alongside it, auto-derived display text). Same writable-only-through-/receive treatment
  // as every other field in this list.
  'test_certificate_id'];

export const BOM_FIELDS = [...Object.values(BOM_FIELD_OWNERS).flat(), ...RECEIVING_FIELDS];

// Stores/Inventory hardening Phase 1 — *how* a bom_items line's material need was first
// identified: bulk PMB Excel import, raised through /pr, a flat BOM template applied, or hand-
// typed/pasted. Deliberately NOT the same axis as `source` (bom/stock/sas, which answers *what the
// demand is for*) — a Stores "Build stock" request is commonly both origin='pr' (raised through
// /pr) and source='stock' (for replenishment) at once; this never conflates the two. Pure
// derivation, no stored column: every input already exists and is already correctly populated at
// every real bom_items insertion site (verified against all 9 in app/+lib/, 2026-09).
export function demandOrigin(row) {
  if (row?.pr_item_id != null) return 'pr';
  if (row?.template_id != null) return 'template';
  if (row?.import_id != null) return 'pmb';
  return 'manual';
}
...
// `pipe` added per the real-data category-taxonomy audit (SYSTEM.md) — hollow pressure-part stock
// (STAY TUBES, MS PIPE, etc.) had no home in this list at all; it's 'linear' like every other
// non-plate category here (matched by length + profile string, lib/remnant-match.js's parseDims),
// same reasoning that already applies to angle/beam/channel/tee.
export const DIMENSIONAL_CATEGORIES = ['plate', 'flat', 'round', 'square', 'octagonal', 'angle', 'beam', 'channel', 'tee', 'pipe'];

// Whether a BOM line's QC material/mounting classification (lib/qc-bom-sync.js's classify()) came
// from a real, department-set signal or the legacy moc-presence guess — surfaced on the QC document
// editor so a fallback-classified part reads as "unconfirmed, verify" rather than looking as
// trustworthy as a real category match. Lives here (not in qc-bom-sync.js, which imports lib/data.js
// and is server-only) purely so a 'use client' component can import this one pure predicate without
// dragging in the DB-touching sync logic around it. Per the explicit instruction: moc stays a
// fallback, never silently promoted to a permanent decision.
export function classificationSource(b) {
  if (b.category === 'other') return 'excluded';
  if (b.category) return 'category';
  if (b.requires_mtc) return 'requires_mtc';
  return 'fallback';
}
...
export function editableBomFields(user) {
  ...
  // Design has no BOM_FIELD_OWNERS entry of its own — it shares Engineering's (same convention as
  // VISIBLE_COLUMNS_BY_DEPT below). Without this, a Design-only head (granted 'Design' but not
  // 'Engineering') got an empty field list and every bom-items PATCH 403'd, even for their own
  // category/description edits.
  return [...new Set(depts.flatMap(d => BOM_FIELD_OWNERS[d === 'Design' ? 'Engineering' : d] || []))];
}
