...
const s = StyleSheet.create({
  ...
  tHead: { flexDirection: 'row', backgroundColor: '#eee', borderTop: 1, borderBottom: 1, borderLeft: 1, borderColor: '#999' },
  tRow: { flexDirection: 'row', borderBottom: 1, borderLeft: 1, borderColor: '#ddd', minHeight: 12 },
  ...
  cell: { paddingVertical: 2, paddingHorizontal: 2, borderRight: 1, borderColor: '#ddd', fontFamily: 'Helvetica' },
...
const MATERIAL_BASE_COLS = [
  ...
  { key: 'materialSpec', label: 'Material Spec.', get: p => p.material_spec || '—' },
...
const IIIA_WIDTHS = { partNo: 2, partOfBoiler: 14, castNo: 5, plateNo: 5, sizeT: 2, sizeW: 2, sizeL: 3,
  qty: 2, materialSpec: 5, steelMaker: 6, certNo: 12, steelMakingProcess: 10, chemC: 2, chemMn: 2,
  chemP: 2, chemS: 2, chemSi: 2, heatTreatment: 6, ys: 4, uts: 4, elongation: 4, bendTest: 4 };
...
function MaterialTableHeader({ leafCols }) {
  ...
  const cellBase = { borderRight: 1, borderColor: GT_BORDER, fontWeight: 'bold', textAlign: 'center', alignSelf: 'center' };
  return (
    <View style={{ borderTop: 1, borderLeft: 1, borderColor: GT_BORDER }} fixed>
      <View style={{ flexDirection: 'row', height: 16, borderBottom: 1, borderColor: GT_BORDER }}>
        {spans.map((sp, i) => {
          const w = leafCols.slice(sp.start, sp.start + sp.span).reduce((n, c) => n + c.width, 0);
          const label = sp.group ? MATERIAL_GROUP_TITLES[sp.group] : leafCols[sp.start].label;
          return <Text key={i} style={[cellBase, { width: `${w}%`, fontSize: 6 }]}>{label}</Text>;
        })}
      </View>
      <View style={{ flexDirection: 'row', height: 10, borderBottom: 1, borderColor: GT_BORDER }}>
        {leafCols.map((c, i) => <Text key={i} style={[cellBase, { width: `${c.width}%`, fontSize: 5.5 }]}>{c.group ? c.label : ''}</Text>)}
      </View>
      <View style={{ flexDirection: 'row', height: 9, borderBottom: 1, borderColor: GT_BORDER }}>
        {leafCols.map((c, i) => <Text key={i} style={[cellBase, { width: `${c.width}%`, fontSize: 5 }]}>{c.unit1 || ''}</Text>)}
      </View>
      <View style={{ flexDirection: 'row', height: 9, borderBottom: 1, borderColor: GT_BORDER }}>
        {leafCols.map((c, i) => <Text key={i} style={[cellBase, { width: `${c.width}%`, fontSize: 5 }]}>{c.unit2 || ''}</Text>)}
      </View>
    </View>
  );
}
...
function MaterialTable({ cols, parts }) {
  return (
    <View>
      <MaterialTableHeader leafCols={cols} />
      {parts.map((p, i) => (
        <View key={p.id ?? i} style={s.tRow} wrap={false}>
          {cols.map((c, j) => <Text key={j} style={[s.cell, { width: `${c.width}%` }]}>{c.get(p) ?? '—'}</Text>)}
        </View>
      ))}
    </View>
  );
...
const GT_BORDER = '#999';
function GroupCell({ g, height }) {
  if (!g.leaves) {
    // `alignSelf: 'center'` — the row's default cross-axis 'stretch' would otherwise force this
    // Text's own box to exactly `height`, and react-pdf truncates (drops lines/wraps) any content
    // that doesn't fit a stretched Text's box instead of letting it overflow the way a View does.
    // 'center' gives the Text its own natural (auto) height — full wrapping, no dropped text — and
    // centers it in the row for free, replacing the old paddingTop-guess approach that silently
    // clipped a longer label like "Hydrostatic test lbs./Sq. in." to "Hydrostatic test…".
    return <Text style={{ width: `${g.width}%`, borderRight: 1, borderColor: GT_BORDER, fontSize: 6, fontWeight: 'bold', textAlign: 'center', alignSelf: 'center' }}>{g.label}</Text>;
  }
  return (
    <View style={{ width: `${g.width}%`, borderRight: 1, borderColor: GT_BORDER }}>
      <Text style={{ fontSize: 6, fontWeight: 'bold', textAlign: 'center', borderBottom: 1, borderColor: GT_BORDER, paddingVertical: 2 }}>{g.label}</Text>
      <View style={{ flexDirection: 'row' }}>
        {g.leaves.map((leaf, j) => (
          <Text key={j} style={{ width: `${(leaf.width / g.width) * 100}%`, fontSize: 5.5, fontWeight: 'bold', textAlign: 'center', borderRight: j < g.leaves.length - 1 ? 1 : 0, borderColor: GT_BORDER, paddingTop: 2 }}>{leaf.label}</Text>
        ))}
      </View>
    </View>
  );
...
function GroupedEmptyTable({ groups, note, headerHeight = 26 }) {
  return (
    <View style={{ borderTop: 1, borderLeft: 1, borderRight: 1, borderBottom: 1, borderColor: GT_BORDER, marginTop: 2 }} wrap={false}>
      <View style={{ flexDirection: 'row', height: headerHeight }}>
        {groups.map((g, i) => <GroupCell key={i} g={g} height={headerHeight} />)}
      </View>
      <View style={{ flexDirection: 'row', borderTop: 1, borderColor: GT_BORDER, minHeight: 16 }}>
...
function HeadersBoxesTable({ note }) {
  // Plain spaces, not embedded "\n" — react-pdf's own auto-height text measurement drops content
  // past the first hard-break line once a Text sits directly in a fixed-`height` flex row (see
  // GroupCell's leaf-less comment); a natural wrap at the column's own width doesn't hit that.
  const dataCols = [
  ...
  return (
    <View style={{ borderTop: 1, borderLeft: 1, borderRight: 1, borderBottom: 1, borderColor: GT_BORDER, marginTop: 2 }} wrap={false}>
      <View style={{ flexDirection: 'row', height: 22 }}>
        <Text style={{ width: '10%', borderRight: 1, borderColor: GT_BORDER, fontSize: 6, fontWeight: 'bold', textAlign: 'center', alignSelf: 'center' }}>No.</Text>
        {dataCols.map((g, i) => <GroupCell key={i} g={g} height={22} />)}
      </View>
      {rows.map((r, i) => (
        <View key={i} style={{ flexDirection: 'row', borderTop: 1, borderColor: GT_BORDER, minHeight: 20 }}>
          <Text style={{ width: '10%', borderRight: 1, borderColor: GT_BORDER, fontSize: 6, fontWeight: 'bold', textAlign: 'center', paddingVertical: 4 }}>{r.label}</Text>
...
function MountingsTable({ note }) {
  ...
  return (
    <View style={{ borderTop: 1, borderLeft: 1, borderRight: 1, borderBottom: 1, borderColor: GT_BORDER, marginTop: 2 }} wrap={false}>
      <View style={{ flexDirection: 'row' }}>
        {COLS.map((c, i) => (
          <Text key={i} style={{ width: `${c.width}%`, borderRight: 1, borderBottom: 1, borderColor: GT_BORDER, fontSize: 7, fontWeight: 'bold', textAlign: 'center', paddingVertical: 4 }}>{c.label}</Text>
        ))}
      </View>
      <View style={{ flexDirection: 'row', minHeight: 30 }}>
        <Text style={{ width: `${COLS[0].width}%`, borderRight: 1, borderColor: GT_BORDER }} />
  ...
  return [
    <Page key="iii-1" size="A4" style={s.pageNoLetterhead}>
      ...
      <View style={{ marginLeft: 20, fontSize: 10 }}>
        <KV label="Constructor's name and address" value={`${entity.name}, ${entity.address}`} filled tight style={s1Row} />
...
//
// `preamble` (only true on the FIRST page of the whole form) — the real sample's mandated
// certifying sentence ("It is hereby certificate that original Steel makers certificate in Form IV
// contain...bearing makers no.X") was previously missing entirely, and would read as odd
// boilerplate if repeated once per lettered section, so it renders exactly once, embedding the
// maker's no. that the old per-page "doc_id · Maker's No." line used to show on every page instead.
//
// `isLast` (only true on the LAST page across every lettered section + the ungrouped page) — the
// sign-off + Place:/Date: previously rendered once per `FormTablePage` call, meaning a project with
// several real lettered sections (letters A-H, e.g. the true reference sample) would print several
// duplicate sign-off blocks instead of the real document's single one at the very end. Both
// booleans are computed once by the 'IVA' case below across the whole page array it builds, not
// re-derived per page.
//
// `sectionLabel` (Phase 3 — BOM-tree-driven lettered sections) stays optional: omitted, a project
// with no tree renders as a single page — preamble=true, isLast=true — the exact "no tree ->
// unchanged flat table" fallback the plan requires, just with the two real fixes above included.
function FormTablePage({ document: d, entity, sectionLabel, cols, parts, preamble, isLast }) {
  return (
    <Page size="A4" orientation="landscape" style={s.pageLNoLetterhead}>
      ...
      <MaterialTable cols={cols} parts={renumber(parts)} />
      {isLast && (
        <View wrap={false}>
          <Sign />
...
export async function renderQcFolderPdf(document, parts, mountings, project, groups, form4aGroups, seams) {
  ...
  const formPage = (f, extra) => {
    switch (f) {
      ...
      // one full page per lettered section (Table + Sign + Footer each, same per-page shape the
      // already-shipped Form III A per-group pages use), plus a trailing UNLABELED page for any part
      // QC hasn't assigned to a group yet — deliberately never "Ungrouped Materials" or any other
      // synthetic heading (direct instruction), since every real material still has to print on the
      // certificate even when a QC user hasn't (yet, or ever) organized it under a named group.
      case 'IVA': {
        // Build the page specs first (letter/label/parts only), THEN turn them into elements with
        // `preamble`/`isLast` baked in from the start via the array index — a page's own JSX props
        // are set once at creation, never patched onto an already-built React element afterward
        // (mutating `.props` on an existing element is fragile and non-idiomatic; this way there's
        // nothing to mutate).
        const sectionPages = ivaSections.sections.map(sec => (
          { key: `iva-${sec.letter}`, sectionLabel: `${sec.letter}. ${sec.name}`, parts: sec.parts }
        ));
        if (ivaSections.ungrouped.length) {
          sectionPages.push({ key: 'iva-ungrouped', sectionLabel: null, parts: ivaSections.ungrouped });
        }
        if (!sectionPages.length) {
          return <FormTablePage key={f} document={d} entity={entity} cols={IVA_MATERIAL_COLS} parts={ivaSections.ungrouped} preamble isLast />;
        }
        // preamble on the first page, sign-off/Place/Date on the last — once each across the whole
        // form, not once per lettered section (see FormTablePage's own comment for why).
        return sectionPages.map((p, i) => (
          <FormTablePage key={p.key} document={d} entity={entity} sectionLabel={p.sectionLabel} cols={IVA_MATERIAL_COLS}
            parts={p.parts} preamble={i === 0} isLast={i === sectionPages.length - 1} />
        ));
      }
