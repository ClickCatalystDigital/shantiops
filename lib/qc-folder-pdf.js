...
function FormIIIPage({ document: d, entity, project, model, parts }) {
...
function FormIIIHPage({ document: d, entity, parts, project }) {
  return (
    <Page size="A4" style={s.page}>
      ...
      <KV label="Drawing No." value={d.drawing_no} />
...
function FormIIIAGroupPage({ document: d, entity, group: g, parts }) {
  const hydroPressure = g.hydro_test_pressure || (g.design_pressure ? (Number(g.design_pressure) * 1.5).toFixed(3).replace(/0+$/, '').replace(/\.$/, '') : null);
  return (
    <Page size="A4" orientation="landscape" style={s.pageL}>
      <Header entity={entity} />
      ...
      <KV label="Drawing No." value={g.drawing_no} />
...
function Folder({ document: d, parts, mountings, groups, project }) {
  ...
  const formPage = f => {
    switch (f) {
      ...
      case 'III': return <FormIIIPage key={f} document={d} entity={entity} project={project} model={model} parts={parts} />;
...
export async function renderQcFolderPdf(document, parts, mountings, project, groups) {
  const base = await renderToBuffer(<Folder document={document} parts={parts} mountings={mountings || []} groups={groups || []} project={project} />);
