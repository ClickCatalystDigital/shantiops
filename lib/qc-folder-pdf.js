...
// always in tons) exceeds 150 — see the isBigBoiler check below.
...
function FormIIIPage({ document: d, entity, project, model, parts, mountingsPage, seams = [] }) {
  ...
  // (confirmed), compared as-entered against the literal 150-ton threshold.
  const isBigBoiler = series === 'MF' || series === 'CF' || (series === 'SF' && Number(project?.model_capacity) > 150);
