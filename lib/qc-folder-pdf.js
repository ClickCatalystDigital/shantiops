...
function FormII1Page({ document: d, entity, small }) {
  ...
  const recipient = d.recipient_name ? { name: d.recipient_name } : DIRECTOR_OF_BOILERS;
  return (
    <Page size="A4" style={s.page}>
      <Header entity={entity} />
      ...
      <Text style={{ marginBottom: 8 }}>{recipient.name}</Text>
      <Text style={s.p}>
...
function FormIIIPage({ document: d, entity, project, model, parts, mountingsPage }) {
  ...
  const drawingRange = [d.drawing_no_from, d.drawing_no_to].filter(Boolean).join(' TO ') || d.drawing_no || '—';
  return (
    <Page size="A4" style={s.page}>
      <Header entity={entity} />
