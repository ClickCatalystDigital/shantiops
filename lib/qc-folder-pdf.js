...
const s = StyleSheet.create({
  ...
  signBox: { width: '40%', borderTopWidth: 1, borderColor: '#333', paddingTop: 4, textAlign: 'center', fontSize: 7 },
...
const MATERIAL_BASE_COLS = [
  ...
  // Same zero-width-space wrap-point trick certNo already uses below — a real length spec like
  // "133.4x3.38THK(SCH-40)" has few/no spaces, so a single token can be wider than the column even
  // though the whole string would otherwise wrap fine; this gives react-pdf a break point without
  // changing what's printed.
  { key: 'sizeL', label: 'L', get: p => (p.size_l ? String(p.size_l).replace(/([x×\-(])/g, '​$1') : '—'), group: 'size' },
...
const IIIA_WIDTHS = { partNo: 2, partOfBoiler: 11, castNo: 5, plateNo: 5, sizeT: 3, sizeW: 2, sizeL: 6,
  qty: 2, materialSpec: 5, steelMaker: 6, certNo: 9, steelMakingProcess: 8, chemC: 3, chemMn: 3,
...
function GroupCell({ g, height }) {
  if (!g.leaves) {
    ...
    return <Text style={{ width: `${g.width}%`, borderRightWidth: 1, borderColor: GT_BORDER, fontSize: 6, fontWeight: 'bold', textAlign: 'center', alignSelf: 'center' }}>{g.label}</Text>;
  }
  return (
    <View style={{ width: `${g.width}%`, borderRightWidth: 1, borderColor: GT_BORDER, alignSelf: 'center' }}>
      <Text style={{ fontSize: 6, fontWeight: 'bold', textAlign: 'center', borderBottomWidth: 1, borderColor: GT_BORDER, paddingVertical: 2 }}>{g.label}</Text>
      <View style={{ flexDirection: 'row' }}>
        {g.leaves.map((leaf, j) => (
          <Text key={j} style={{ width: `${(leaf.width / g.width) * 100}%`, fontSize: 5.5, fontWeight: 'bold', textAlign: 'center', borderRightWidth: j < g.leaves.length - 1 ? 1 : 0, borderColor: GT_BORDER, paddingTop: 2 }}>{leaf.label}</Text>
        ))}
      </View>
    </View>
  );
...
function GroupedEmptyTable({ groups, note, headerHeight = 34 }) {
  return (
    <View style={{ borderTopWidth: 1, borderLeftWidth: 1, borderRightWidth: 1, borderBottomWidth: 1, borderColor: GT_BORDER, marginTop: 2 }} wrap={false}>
      ...
      <View style={{ flexDirection: 'row', borderTopWidth: 1, borderColor: GT_BORDER, minHeight: 16 }}>
  ...
  return (
    <View style={{ borderTopWidth: 1, borderLeftWidth: 1, borderRightWidth: 1, borderBottomWidth: 1, borderColor: GT_BORDER, marginTop: 2 }} wrap={false}>
      <View style={{ flexDirection: 'row', height: 30 }}>
        <Text style={{ width: '10%', borderRightWidth: 1, borderColor: GT_BORDER, fontSize: 6, fontWeight: 'bold', textAlign: 'center', alignSelf: 'center' }}>No.</Text>
        {dataCols.map((g, i) => <GroupCell key={i} g={g} height={30} />)}
      </View>
      {rows.map((r, i) => (
