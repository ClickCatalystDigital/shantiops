...
export async function renderQcFolderPdf(document, parts, mountings, project, groups, assemblies, seams) {
  const d = document;
  mountings = mountings || [];
  groups = groups || [];
    ...
    switch (f) {
      ...
      case 'IVA': {
        ...
        if (ivaSections.ungrouped.length) {
          sectionPages.push({ key: 'iva-ungrouped', sectionLabel: 'Ungrouped Materials', parts: ivaSections.ungrouped });
        }
