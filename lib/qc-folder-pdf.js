...
// QC statutory-forms plan, Phase 5 — one line per belt/ring for a given location, e.g.
// "Belt 1: ONE, Belt 2: TWO". Empty (no rows for that location yet) renders the same honest "—"
// every other still-unset field on this page already uses.
function seamLine(seams, location, label) {
  const rows = seams.filter(s => s.location === location).sort((a, b) => a.sequence - b.sequence);
  if (!rows.length) return '—';
  return rows.map(r => `${label} ${r.sequence}: ${r.seam_count || '—'}`).join(', ');
}
...
function FormIIIPage({ document: d, entity, project, model, parts, mountingsPage, seams = [] }) {
  ...
  return (
    <Page size="A4" style={s.page}>
      ...
      <Text style={{ fontSize: 8 }}>No. of longitudinal seams in shell/drum in each belt – <Text style={s.filled}>{seamLine(seams, 'belt', 'Belt')}</Text></Text>
      <Text style={{ fontSize: 8 }}>No. of longitudinal seams in Furnace in each ring – <Text style={s.filled}>{seamLine(seams, 'furnace_ring', 'Ring')}</Text></Text>
