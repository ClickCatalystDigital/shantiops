...
function KV({ label, value, filled, labelWidth, tight }) {
  const labelStyle = tight ? s.lblTight : (labelWidth ? [s.lbl, { width: labelWidth }] : s.lbl);
  return <View style={s.row}><Text style={labelStyle}>{label}</Text><Text style={filled ? [s.val, s.filled] : s.val}>: {value || '—'}</Text></View>;
}
  ...
  return [
    <Page key="iii-1" size="A4" style={s.page}>
      ...
      {/* Real sample indents every field label one tab right of its own "N. HEADING" — a two-level
          ...
          §1 now has its own dedicated page with room for it. */}
      ...
      <View style={{ marginLeft: 20, fontSize: 10 }}>
        ...
        <KV label="Contact Number" value={project?.customer_phone} filled />
