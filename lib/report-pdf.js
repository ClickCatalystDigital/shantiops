...
export function ReportFooter() {
  return h(Text, {
    ...
    render: ({ pageNumber, totalPages }) =>
      `Page ${pageNumber} of ${totalPages} · Generated ${new Date().toLocaleString('en-IN')} · Computer-generated, no signature required`,
  });
}
...
export function ReportPage({ company, title, subtitle, orientation = 'portrait', size = 'A4', children }) {
...
// Repeating table primitive — cols: [[label, width%, get?]]. Header row is `fixed` so react-pdf
// repeats it on every page (its documented pattern for multi-page tables). None of the existing
// per-document PDFs do this because none of them span more than a page or two today — report-engine
// tables (ledgers, registers) regularly will.
export function ReportTable({ cols, rows, rowKey = (r, i) => r.id ?? i }) {
  return h(
    ...
    h(
      ...
      cols.map(([label, w], i) =>
        h(Text, { key: i, style: [tokens.cell, { width: `${w}%`, fontWeight: 'bold' }] }, label)),
    ),
    rows.map((r, i) =>
      h(
        View,
        ...
        cols.map(([, w, get], j) =>
          h(Text, { key: j, style: [tokens.cell, { width: `${w}%` }] }, (get ? get(r, i) : r[j]) ?? '—')),
      )),
  );
}
