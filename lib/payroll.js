...
// How many payroll periods this employee has in the given FY (accounting for a mid-year join) and
// how many have elapsed through the given period — the two numbers YTD-accurate TDS annualizes by.
function periodsElapsedAndTotal(dateOfJoining, financialYear, periodMonth, periodYear) {
  ...
  const periodsTotal = monthsBetweenInclusive(joinMonth, joinYear, 3, fyStart + 1);
  const periodsElapsed = monthsBetweenInclusive(joinMonth, joinYear, periodMonth, periodYear);
  return { periodsElapsed: Math.max(1, periodsElapsed), periodsTotal: Math.max(periodsElapsed, periodsTotal, 1) };
}
...
async function computeTds(employee, financialYear, periodMonth, periodYear, thisMonthGross, thisMonthPf, rates) {
  const ytd = await queryOne(
    `SELECT COALESCE(SUM(gross_earnings),0) AS gross, COALESCE(SUM(pf_employee),0) AS pf, COALESCE(SUM(tds_amount),0) AS tds
  ...
  const { periodsElapsed, periodsTotal } = periodsElapsedAndTotal(employee.date_of_joining, financialYear, periodMonth, periodYear);
...
export async function computeSalarySlip(employeeId, periodMonth, periodYear, opts = {}) {
  ...
  let periodEnd = `${periodYear}-${String(periodMonth).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
  if (slipType === 'final' && relievingDate && relievingDate < periodEnd) periodEnd = relievingDate;
  ...
  const { workingDays, paymentDays } = await computePaymentDays(employeeId, periodStart, periodEnd);
  const proration = workingDays > 0 ? paymentDays / workingDays : 1;
