...
// EN-8, SS 202 and Copper added 2026 during the Item Master standardization pass — each backed by
// real, recurring Item Master usage (11/10/2 rows respectively), the same "curated from what's
// actually used" bar this list was originally built against. 'Bronze' (1 real row) deliberately
// left out — "Other / custom" already covers a true one-off.
export const STANDARD_MOC = [
  'MS', 'CS', 'IS 2062 E250', 'IS 2062 E350', 'SA 516 GR 60', 'SA 516 GR 70', 'SA 210 GR A1',
  'SA 106 GR B', 'EN-8', 'SS 202', 'SS 304', 'SS 304L', 'SS 316', 'SS 316L', 'GI', 'Aluminium',
  'Copper',
];
...
export const STANDARD_SECTIONS = {
  angle: [
    // 15x15x3, 40x40x4, 100x100x6 added 2026 during the Item Master standardization pass — real
    // sizes already present in the live catalog that this table was missing. Values derived from
    // this table's own implied rectangular-strip approximation (kg/m ≈ 0.00785 × t × (2a − t)),
    // which reproduces every other entry here within ~1-2% — same "spot-check before relying on
    // for costing" caveat as the rest of this list.
    { size: 'ISA 15x15x3', kg_per_m: 0.65 },
    ...
    { size: 'ISA 35x35x5', kg_per_m: 2.40 }, { size: 'ISA 40x40x4', kg_per_m: 2.42 },
    { size: 'ISA 40x40x5', kg_per_m: 2.95 },
    ...
    { size: 'ISA 100x100x6', kg_per_m: 9.00 },
...
const CATEGORY_PATTERNS = [
  ...
  // `TUBE(S)?` excludes a following "SHEET": "TUBE SHEET MATERIAL" (real, recurring — APH sheet,
  // 3 files) is the flat perforated plate tubes pass THROUGH, not a tube itself — an actual
  // transposed-description plate row (§ last-resort fallback below), confirmed by its own real
  // "1250X3000X8 MM THICK" spec. Found live: this rule firing first was silently overriding that
  // fallback and mis-tagging every real TUBE SHEET row as pipe.
  ['pipe', /\bPIPE(S)?\b|\bTUBE(S)?\b(?!\s*SHEET)|\bNECK\b|SAMPLING PORT/],
  ...
  ['pipe', /\d\s*NB\b/],
  // Last-resort numeric fallback (2026-09, PMB standardization round): a real, recurring
  // source-column transposition — "BODY SHELL MATERIAL 1250X2500X3.15 MM THICK" is genuine MS
  // sheet stock, but the description names the *use*, not the material, so no keyword above ever
  // matches. An L x W x T triple immediately followed by MM THK/THICK has no other plausible
  // reading in this domain. Checked against every real bom_items row containing "THICK"/"THK"
  // before adding this: 118 real matches, 75 already `plate`, the rest genuinely uncategorized or
  // this same transposed-description pattern (BODY SHELL/TUBE SHEET/FLANGES MATERIAL, bare
  // "SHEET"/"PLATE") — zero false positives, since this rule only ever runs once every earlier,
  // more specific keyword rule (PIPE/FLANGE/etc.) has already had first refusal.
  ['plate', /\d+(?:\.\d+)?\s*[Xx]\s*\d+(?:\.\d+)?\s*[Xx]\s*\d+(?:\.\d+)?\s*MM\s*(?:THK|THICK(?:NESS)?)\b/],
];
...
// never match inside "GASKET", they're different whole tokens). A trailing-"s" plural/singular
// variant counts as the same word (real gap found stress-testing against SB-1040's real PMB: the
// catalog's own "V BELTS" group, bom_category='standard', never matched a real "V' BELT" line item
// purely because of the plural — the match then silently fell through to suggestSpellingCorrection,
// which produced a wrong "did you mean BOLT" for a word that was never a typo at all). On a tie
// between two equally-specific (same word count) groups that disagree on category, refuses rather
// than guessing — same "ties refused outright" precedent lib/tc-match.js already established.
function wordOrPlural(descWords, w) {
  return descWords.has(w) || descWords.has(w + 's') || (w.endsWith('s') && descWords.has(w.slice(0, -1)));
}
export function suggestCategoryFromGroups(description, groups) {
  ...
  for (const g of groups) {
    ...
    if (!groupWords.every(w => wordOrPlural(descWords, w))) continue; // full containment required, plural-tolerant
...
// Best-effort structured dimensions from a free-text size ("1250X2500X3.15 MM THICK", "Φ63.5 x 3.66 THK - 2780 LG",
// "SQ.10x10- 5000 LG", "ISA 50x50x5T"). Used only to PRE-FILL the edit form of a line that has text but no structured
// fields (PMB imports), so the person sees and confirms the numbers — never saved by itself. Returns {} when unsure.
export function fieldsFromSizeSpec(category, text) {
  const t = String(text ?? '').replace(/\s+/g, ' ').trim();
  if (!t) return {};
  const nums = s => [...s.matchAll(/\d+(?:\.\d+)?/g)].map(m => Number(m[0]));
  // more than one length / diameter / section in one text = a bundled line: never guess which one is meant
  if ((t.match(/(?:lg|long|length)\b/gi) || []).length > 1 || (t.match(/[Φφø∅]|\bdia\b/gi) || []).length > 1 || (/ISMC/i.test(t) && /ISA/i.test(t))) return {};
  const lg = t.match(/(\d+(?:\.\d+)?)\s*(?:mm\s*)?(?:lg|long|length)\b/i);
  const length = lg ? Number(lg[1]) : null;
  const clean = t.replace(/(\d+(?:\.\d+)?)\s*(?:mm\s*)?(?:lg|long|length)\b/gi, ' ');
  if (category === 'plate') {
    const n = nums(clean);
    if (n.length !== 3) return {};
    const th = clean.match(/(\d+(?:\.\d+)?)\s*(?:mm\s*)?(?:thk|thick)/i) || clean.match(/(?:thk|thick)\.?\s*(\d+(?:\.\d+)?)/i);
    const thickness = th ? Number(th[1]) : Math.min(...n);
    const rest = n.filter((v, i) => !(v === thickness && n.indexOf(v) === i));
    if (rest.length !== 2) return {};
    return { thickness, length: Math.max(...rest), width: Math.min(...rest) };
  }
  if (category === 'flat') { const n = nums(clean); return n.length === 2 && Math.min(...n) <= 20 ? { width: Math.max(...n), thickness: Math.min(...n), ...(length ? { length } : {}) } : {}; }
  // "65X65-400Lg" / "8 x 8 - 300 lg" with no diameter mark is a square section, not a round bar: leave it to a person
  if (category === 'round' && /(\d+(?:\.\d+)?)\s*[xX]\s*\1(?!\d)/.test(clean) && !/[Φφø∅]|\bdia\b/i.test(t)) return {};
  if (category === 'round') { const n = nums(clean.replace(/^[^\d]*/, '')); return n.length >= 1 ? { diameter: n[0], ...(length ? { length } : n[1] ? { length: n[1] } : {}) } : {}; }
  if (category === 'square') { const n = nums(clean); return n.length >= 2 && n[0] === n[1] ? { side: n[0], ...(length ? { length } : n[2] ? { length: n[2] } : {}) } : {}; }
  if (['angle', 'beam', 'channel'].includes(category)) {
    const norm = s => s.toLowerCase().replace(/\s+/g, '').replace(/(\d)(?:thk|t)\b/g, '$1').replace(/x5thk/g, 'x5');
    const key = norm(clean);
    const hits = (STANDARD_SECTIONS[category] || []).filter(s => key.includes(norm(s.size)));
    const hit = hits.length === 1 ? hits[0] : null;
    return hit ? { size: hit.size, kg_per_m: hit.kg_per_m, ...(length ? { length } : {}) } : {};
  }
  return {};
}
