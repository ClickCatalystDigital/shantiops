...
export function fieldsFromSizeSpec(category, text) {
  ...
  if (category === 'flat') { const n = nums(clean); return n.length === 2 ? { width: Math.max(...n), thickness: Math.min(...n), ...(length ? { length } : {}) } : {}; }
  ...
  if (['angle', 'beam', 'channel'].includes(category)) {
    ...
    const hit = (STANDARD_SECTIONS[category] || []).find(s => key.includes(norm(s.size)));
